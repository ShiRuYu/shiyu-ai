...
public class UserController {
    ...
    @GetMapping("/detail") public Result<UserVO> getUserInfo() { Long id=LoginContextHolder.getUserId(); if(id==null)return Result.fail("用户未登录"); UserVO v=userService.detailView(id); if(v==null)return Result.fail("用户不存在"); v.setTenants(authService.getUserTenants(id)); if(LoginContextHolder.getCurrentTenantId()!=null)v.setCurrentTenantId(LoginContextHolder.getCurrentTenantId()); if(LoginContextHolder.getHomeTenantId()!=null)v.setHomeTenantId(LoginContextHolder.getHomeTenantId()); if(LoginContextHolder.getSwitchMode()!=null)v.setSwitchMode(LoginContextHolder.getSwitchMode()); return Result.success(v); }
