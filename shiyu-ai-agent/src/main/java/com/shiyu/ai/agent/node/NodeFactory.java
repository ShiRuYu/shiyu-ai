...
public class NodeFactory {
     ...
     */
    /** 
     * 使用外部注册的 NodeCreator bean 创建需要 DI 的节点
     * 消除了原有的 switch 表达式，符合开闭原则
     */
    ...
    private BaseNode createNodeWithDependencies(NodeType nodeType, NodeConfig config) {
        ...
        for (com.shiyu.ai.agent.node.creator.NodeCreator creator : beanNodeCreators) {
            if (creator.getType() == nodeType) {
                try {
                    log.debug("使用 Bean NodeCreator 创建节点: type={}", nodeType);
                    return creator.create(config);
                } catch (Exception e) {
                    log.error("节点创建失败: type={}", nodeType, e);
                    return null;
                }
            }
        }
        log.debug("无匹配的 Bean NodeCreator: type={}", nodeType);
        return null;
    }
