...
import com.shiyu.ai.chat.product.CharacterCardV2;
...
public final class CharacterImportPreviewStore {
    ...
    public Preview issue(long tenantId, long userId, byte[] payload, String filename, CharacterCardV2 card) {
    ...
    public CharacterCardV2 consume(long tenantId, long userId, String token, byte[] payload, String filename) {
        if (token == null || token.isBlank()) throw new IllegalArgumentException("previewToken is required");
        Entry entry = entries.remove(token);
        if (entry == null || entry.expires().isBefore(Instant.now()) || entry.tenantId() != tenantId || entry.userId() != userId
    ...
    private record Entry(long tenantId, long userId, String digest, String filename, CharacterCardV2 card, Instant expires) { }
