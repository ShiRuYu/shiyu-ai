...
public class ConversationController {
    ...
    private Result<GenerationRun> messageInternal(String id, String idempotencyKey, MessageRequest request) {
        if (request == null || request.content == null || request.content.isBlank()) throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "message content is required");
        if (idempotencyKey != null && !idempotencyKey.isBlank()) {
    ...
    public Result<Void> activeLeaf(@PathVariable String id, @RequestParam(required = false) String messageId, @RequestBody(required = false) ActiveLeafRequest body) {
        ...
        conversationRepository.findMessage(messageId, currentTenant(), currentUser()).filter(m -> id.equals(m.conversationId())).orElseThrow(() -> new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "message does not belong to conversation"));
    ...
    public Object export(@PathVariable String id, @RequestParam(required = false) String format) {
        ...
        if ("jsonl".equalsIgnoreCase(format)) return org.springframework.http.ResponseEntity.ok().contentType(MediaType.TEXT_PLAIN).body(com.shiyu.ai.chat.product.ConversationExchangeCodec.toJsonl(messages));
        if ("markdown".equalsIgnoreCase(format) || "md".equalsIgnoreCase(format)) return org.springframework.http.ResponseEntity.ok().contentType(MediaType.TEXT_MARKDOWN).body(com.shiyu.ai.chat.product.ConversationExchangeCodec.toMarkdown(messages));
    ...
    public Result<Conversation> importConversation(@RequestBody ImportRequest request) {
        ...
        List<com.shiyu.ai.chat.product.ConversationExchangeCodec.ImportedMessage> imported = importPreviews.consume(
    ...
    public Result<ConversationImportPreviewStore.Preview> importPreview(@RequestBody ImportRequest request) {
        ...
        List<com.shiyu.ai.chat.product.ConversationExchangeCodec.ImportedMessage> imported = "markdown".equals(format) || "md".equals(format)
                ? com.shiyu.ai.chat.product.ConversationExchangeCodec.fromMarkdown(request.content)
                : com.shiyu.ai.chat.product.ConversationExchangeCodec.fromJsonl(request.content);
    ...
    private void validateImportPayload(ImportRequest request) {
        if (request == null || request.content == null || request.content.isBlank()) {
            throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "import content is required");
        }
        if (request.content.getBytes(java.nio.charset.StandardCharsets.UTF_8).length > 4 * 1024 * 1024) {
            throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "import payload exceeds 4 MiB");
        }
    }

    private long currentTenant() { Long id = UserContextHolder.getCurrentTenantId(); if (id == null) throw new IllegalStateException("tenant context is required"); return id; }
    private long currentUser() { Long id = UserContextHolder.getUserId(); if (id == null) throw new IllegalStateException("login is required"); return id; }
