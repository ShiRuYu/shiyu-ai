...
public class ChatProductController {
    ...
    @GetMapping("/characters") public Result<List<CharacterAsset>> characters() { return Result.success(repository.listCharacters(tenant(), user())); }
    @GetMapping("/characters/{id}") public Result<CharacterAsset> character(@PathVariable String id) { return Result.success(repository.findCharacterForAccess(tenant(), user(), id).orElseThrow(() -> new IllegalArgumentException("character not found"))); }
    @DeleteMapping("/characters/{id}") public Result<Void> deleteCharacter(@PathVariable String id) { repository.deleteCharacter(tenant(), user(), id); return Result.success(); }
    ...
    public Result<CharacterImportPreviewStore.Preview> previewCharacterImport(@RequestPart("file") MultipartFile file) throws Exception {
        ...
        return Result.success(characterImportPreviews.issue(tenant(), user(), original, file.getOriginalFilename(), card));
    }
    ...
    public Result<CharacterAsset> importCharacter(@RequestPart("file") MultipartFile file, @RequestParam String previewToken) throws Exception {
        byte[] original = file.getBytes();
        CharacterCardV2 card = characterImportPreviews.consume(tenant(), user(), previewToken, original, file.getOriginalFilename());
    ...
    public ResponseEntity<byte[]> exportCharacter(@PathVariable String id) throws Exception {
        CharacterAsset asset = repository.findCharacterForAccess(tenant(), user(), id).orElseThrow(() -> new IllegalArgumentException("character not found"));
    ...
    @GetMapping("/personas") public Result<List<PersonaAsset>> personas() { return Result.success(repository.listPersonas(tenant(), user())); }
    @GetMapping("/personas/{id}") public Result<PersonaAsset> persona(@PathVariable String id) { return Result.success(repository.findPersona(tenant(), user(), id).orElseThrow(() -> new IllegalArgumentException("persona not found"))); }
    @DeleteMapping("/personas/{id}") public Result<Void> deletePersona(@PathVariable String id) { repository.deletePersona(tenant(), user(), id); return Result.success(); }
    ...
    @GetMapping("/lorebooks") public Result<List<LorebookAsset>> lorebooks() { return Result.success(repository.listLorebooks(tenant(), user())); }
    @GetMapping("/lorebooks/{id}") public Result<LorebookAsset> lorebook(@PathVariable String id) { return Result.success(repository.findLorebook(tenant(), user(), id).orElseThrow(() -> new IllegalArgumentException("lorebook entry not found"))); }
    @DeleteMapping("/lorebooks/{id}") public Result<Void> deleteLorebook(@PathVariable String id) { repository.deleteLorebook(tenant(), user(), id); return Result.success(); }
    ...
    public Result<PromptTemplateVersion> createPrompt(@RequestBody PromptRequest request) {
        ...
        List<PromptTemplateVersion> revisions = repository.listPrompts(tenant(), user(), request.templateId);
        ...
        return Result.success(repository.savePrompt(version, tenant(), user()));
    }
    @GetMapping("/prompt-studio/templates") public Result<List<PromptTemplateVersion>> prompts(@RequestParam(required = false) String templateId) { return Result.success(repository.listPrompts(tenant(), user(), templateId)); }
    ...
    public Result<PromptTemplateVersion> publishPrompt(@PathVariable String templateId, @RequestBody PublishRequest request) {
        if (request == null || request.version < 1) throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "draft version is required");
        PromptTemplateVersion draft = repository.listPrompts(tenant(), user(), templateId).stream()
        ...
        int nextVersion = repository.listPrompts(tenant(), user(), templateId).stream().mapToInt(PromptTemplateVersion::version).max().orElse(0) + 1;
        ...
        return Result.success(repository.savePrompt(published, tenant(), user()));
    }
    ...
    public Result<PromptDiff> diffPrompt(@PathVariable String templateId, @RequestBody DiffRequest request) {
        if (request == null || request.fromVersion < 1 || request.toVersion < 1) throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "prompt versions are required");
        List<PromptTemplateVersion> revisions = repository.listPrompts(tenant(), user(), templateId);
    ...
    public Result<PromptTestRun> testPrompt(@PathVariable String templateId, @RequestBody PromptTestRequest request) {
        if (request == null || request.version < 1) throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "prompt version is required");
        PromptTemplateVersion version = repository.listPrompts(tenant(), user(), templateId).stream()
    ...
    @GetMapping("/groups") public Result<List<GroupChatAsset>> groups() { return Result.success(repository.listGroups(tenant(), user())); }
    @GetMapping("/groups/{id}") public Result<GroupChatAsset> group(@PathVariable String id) { return Result.success(repository.findGroup(tenant(), user(), id).orElseThrow(() -> new IllegalArgumentException("group not found"))); }
    @DeleteMapping("/groups/{id}") public Result<Void> deleteGroup(@PathVariable String id) { repository.deleteGroup(tenant(), user(), id); return Result.success(); }
    ...
    public Result<GroupTurnPlanner.TurnDecision> nextSpeaker(@PathVariable String id, @RequestBody(required = false) TurnRequest request) {
        GroupChatAsset asset = repository.findGroup(tenant(), user(), id).orElseThrow(() -> new IllegalArgumentException("group not found"));
    ...
    public Result<GroupTurnRun> runTurn(@PathVariable String id, @RequestBody TurnRunRequest request) {
        if (request == null || request.conversationId == null || request.conversationId.isBlank()) {
            throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "conversationId is required");
        }
        if (request.content == null || request.content.isBlank()) {
            throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "content is required");
        }
        GroupChatAsset asset = repository.findGroup(tenant(), user(), id)
                .orElseThrow(() -> new IllegalArgumentException("group not found"));
        Conversation conversation = conversations.findConversation(request.conversationId, tenant(), user())
                .orElseThrow(() -> new IllegalArgumentException("conversation not found"));
        if (generationRepository.hasRunningConversation(request.conversationId, tenant())) {
        ...
        List<com.shiyu.ai.conversation.domain.GenerationRun> priorRuns = generationRepository.listConversation(request.conversationId, tenant(), 1000);
        ...
        if (platform == null || platform.isBlank() || model == null || model.isBlank()) {
            throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "platform and model are required");
        }
        GenerationRun run;
        try {
        ...
        try { generationRunner.start(run, tenant(), user()); }
    ...
    private static void validateVariables(String body, java.util.Map<String, String> schema) {
        ...
        while (matcher.find()) if (!declared.contains(matcher.group(1))) throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNPROCESSABLE_ENTITY, "undeclared prompt variable: " + matcher.group(1));
    }
}
