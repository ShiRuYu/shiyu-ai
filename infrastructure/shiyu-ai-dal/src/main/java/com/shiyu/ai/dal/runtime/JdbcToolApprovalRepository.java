...
public class JdbcToolApprovalRepository implements ToolApprovalRepository {
    ...
    @Override
    public int expirePending(long tenantId, long ownerUserId) {
        return jdbc.update("UPDATE AI_TOOL_APPROVAL SET STATUS='EXPIRED',DECIDED_AT=CURRENT_TIMESTAMP WHERE TENANT_ID=? AND OWNER_USER_ID=? AND STATUS='PENDING' AND EXPIRES_AT<CURRENT_TIMESTAMP", tenantId, ownerUserId);
    }
