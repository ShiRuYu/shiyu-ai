...
public class JdbcGenerationRepository implements GenerationRepository {
    ...
    @Override public boolean hasRunning(String conversationId, String inputMessageId) {
        Integer count = jdbc.queryForObject("SELECT COUNT(*) FROM CHAT_GENERATION_ACTIVE WHERE CONVERSATION_ID=? AND INPUT_MESSAGE_ID=?", Integer.class, conversationId, inputMessageId);
        return count != null && count > 0;
    }
    @Override public boolean hasRunningConversation(String conversationId, long tenantId) {
        Integer count = jdbc.queryForObject("SELECT COUNT(*) FROM CHAT_GENERATION_ACTIVE WHERE CONVERSATION_ID=? AND TENANT_ID=?", Integer.class, conversationId, tenantId);
        return count != null && count > 0;
    }
    ...
    public int recoverStaleGenerations(long timeoutMs) {
        ...
        for (StaleGeneration abandoned : stale) {
            ...
            if (failed.runtimeRunId() != null && !failed.runtimeRunId().isBlank()) {
                try {
                    AiRun run = runtimeRuns.find(failed.runtimeRunId(), currentTenant(current), abandoned.ownerUserId()).orElse(null);
    ...
    @Override
    public void appendEvent(GenerationEvent e, long tenantId) {
    ...
    public List<GenerationEvent> listEvents(String generationId, int afterSequence, int limit) {
        return jdbc.query("SELECT SEQ,TYPE,PAYLOAD,CREATED_AT FROM AI_RUN_EVENT WHERE GENERATION_ID=? AND SEQ>? ORDER BY SEQ LIMIT ?",
    ...
    public int nextEventSequence(String generationId) {
        Integer max = jdbc.queryForObject("SELECT COALESCE(MAX(SEQ),-1)+1 FROM AI_RUN_EVENT WHERE GENERATION_ID=?", Integer.class, generationId);
        return max == null ? 0 : max;
    }

    private GenerationRuntimeLink runtimeLink(String generationId, long tenantId) {
