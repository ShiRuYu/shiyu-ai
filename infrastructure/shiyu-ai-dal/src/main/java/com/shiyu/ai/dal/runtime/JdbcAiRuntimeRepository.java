...
public class JdbcAiRuntimeRepository implements AiRunRepository {
    ...
    @Override public List<AiRun> list(long tenant,long owner,int limit) { return jdbc.query("SELECT * FROM AI_RUN WHERE TENANT_ID=? AND OWNER_USER_ID=? ORDER BY CREATED_AT DESC LIMIT ?", new Object[]{tenant, owner, Math.max(1, Math.min(limit, 500))}, (rs, rowNum) -> mapRun(rs)); }
    ...
    @Override public int linkGeneration(String runId, long tenant, long owner, String generationId) {
        return jdbc.update("UPDATE AI_RUN SET GENERATION_ID=? WHERE ID=? AND TENANT_ID=? AND OWNER_USER_ID=? AND GENERATION_ID IS NULL", generationId, runId, tenant, owner);
    }
    ...
    @Override
    @Transactional
    public long appendNextEvent(String runId, long tenantId, long ownerUserId, AiRunEventType type,
    ...
    public long appendNextEvent(String runId, long tenantId, long ownerUserId, AiRunEventType type,
        ...
        Long current = jdbc.query("SELECT LAST_EVENT_SEQ FROM AI_RUN WHERE ID=? AND TENANT_ID=? AND OWNER_USER_ID=? FOR UPDATE",
                rs -> rs.next() ? rs.getLong(1) : null, runId, tenantId, ownerUserId);
        ...
        int updated = jdbc.update("UPDATE AI_RUN SET LAST_EVENT_SEQ=? WHERE ID=? AND TENANT_ID=? AND OWNER_USER_ID=? AND LAST_EVENT_SEQ=?",
                seq, runId, tenantId, ownerUserId, current);
    ...
    public long appendEvent(AiRunEvent e) {
        ...
        Long runLast = jdbc.query("SELECT LAST_EVENT_SEQ FROM AI_RUN WHERE ID=? AND TENANT_ID=? FOR UPDATE",
                rs -> rs.next() ? rs.getLong(1) : null, e.runId(), e.tenantId());
        Long max = jdbc.queryForObject("SELECT COALESCE(MAX(SEQ),0) FROM AI_RUN_EVENT WHERE RUN_ID=? AND TENANT_ID=?", Long.class, e.runId(), e.tenantId());
        List<AiRunEvent> existing = jdbc.query("SELECT * FROM AI_RUN_EVENT WHERE RUN_ID=? AND TENANT_ID=? AND SEQ=?",
                this::mapEvent, e.runId(), e.tenantId(), e.seq());
        if (!existing.isEmpty()) {
        ...
        Integer terminalCount = jdbc.queryForObject("SELECT COUNT(*) FROM AI_RUN_EVENT WHERE RUN_ID=? AND TENANT_ID=? AND TYPE IN ('RUN_COMPLETED','RUN_FAILED','RUN_CANCELLED')", Integer.class, e.runId(), e.tenantId());
        ...
        jdbc.update("INSERT INTO AI_RUN_EVENT (RUN_ID,TENANT_ID,SEQ,TYPE,SCHEMA_VERSION,TURN_ID,STEP_ID,PARENT_EVENT_SEQ,CONVERSATION_ID,GENERATION_ID,EXECUTION_ID,APP_ID,APP_VERSION_ID,PROVIDER_REQUEST_ID,TRACE_ID,PAYLOAD,REDACTED,CREATED_AT) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                e.runId(), e.tenantId(), e.seq(), e.type().name(), e.schemaVersion(), e.turnId(), e.stepId(), e.parentEventSeq(),
        ...
        if (runLast != null) {
            if (jdbc.update("UPDATE AI_RUN SET LAST_EVENT_SEQ=? WHERE ID=? AND TENANT_ID=? AND LAST_EVENT_SEQ=?",
                    e.seq(), e.runId(), e.tenantId(), runLast) != 1) {
                throw new IllegalStateException("run event sequence was modified");
            }
        }
        return e.seq();
    }
    @Override public List<AiRunEvent> events(String id,long tenant,long owner,long after,int limit) { find(id,tenant,owner).orElseThrow(() -> new IllegalArgumentException("run not found")); return jdbc.query("SELECT * FROM AI_RUN_EVENT WHERE RUN_ID=? AND TENANT_ID=? AND SEQ>? ORDER BY SEQ LIMIT ?",this::mapEvent,id,tenant,after,Math.max(1,Math.min(limit,100_000))); }
    private AiRunEvent mapEvent(java.sql.ResultSet rs, int n) throws java.sql.SQLException {
        return new AiRunEvent(rs.getString("RUN_ID"), rs.getLong("TENANT_ID"), rs.getLong("SEQ"), AiRunEventType.valueOf(rs.getString("TYPE")),
