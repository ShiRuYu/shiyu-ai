...
public class JVectorMemorySemanticIndex implements MemorySemanticIndex {
    ...
    public List<MemoryPath> search(MemoryQuery query,int limit){Map<String,Object> filter=new HashMap<>();filter.put("tenantId",query.tenantId());filter.put("namespace",query.namespace());if(query.subjectType()!=null&&!query.subjectType().isBlank())filter.put("subjectType",query.subjectType());if(query.subjectId()!=null&&!query.subjectId().isBlank())filter.put("subjectId",query.subjectId());List<VectorRecord> records=store.search(VectorSearchRequest.builder().queryVector(embed(query.text())).topK(limit).filter(filter).build());List<MemoryPath> out=new ArrayList<>();for(VectorRecord r:records){repository.findEvent(query.tenantId(),r.id()).filter(e->e.status()==MemoryEventStatus.ACTIVE).ifPresent(e->out.add(new MemoryPath(e,((Number)r.metadata().getOrDefault("_score",0d)).doubleValue(),List.of())));}return out;}
    public void delete(String id){store.delete(id);store.flush();}
    public void rebuild(String namespace){
        ...
        repository.findByNamespace(namespace,100000).forEach(this::upsert);
    }
