...
public class JdbcAiAppRepository implements AiAppRepository {
    ...
    @Override public int publishVersion(String appId,String id,long tenant) { int n=jdbc.update("UPDATE AI_APP_VERSION SET STATUS='PUBLISHED',PUBLISHED_AT=CURRENT_TIMESTAMP WHERE APP_ID=? AND ID=? AND TENANT_ID=? AND STATUS<>'ARCHIVED'",appId,id,tenant); if(n==1) jdbc.update("UPDATE AI_APP SET PUBLISHED_VERSION_ID=?,UPDATED_AT=CURRENT_TIMESTAMP WHERE ID=? AND TENANT_ID=?",id,appId,tenant); return n; }
