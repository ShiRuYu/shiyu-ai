...
public class JdbcConversationRepository implements ConversationRepository {
    ...
    @Override
    public Optional<Conversation> findConversation(String id, long tenantId, long ownerUserId) {
        return jdbc.query("SELECT * FROM CHAT_CONVERSATION WHERE ID=? AND TENANT_ID=? AND OWNER_USER_ID=? AND STATUS<>'DELETED'", this::mapConversation, id, tenantId, ownerUserId).stream().findFirst();
    }

    @Override
    public List<Conversation> listConversations(long tenantId, long ownerUserId, int limit, int offset) {
        return jdbc.query("SELECT * FROM CHAT_CONVERSATION WHERE TENANT_ID=? AND OWNER_USER_ID=? AND STATUS<>'DELETED' ORDER BY UPDATED_AT DESC LIMIT ? OFFSET ?", this::mapConversation, tenantId, ownerUserId, Math.min(Math.max(limit, 1), 100), Math.max(offset, 0));
    }

    @Override public List<Conversation> listBranches(String parentConversationId, long tenantId, long ownerUserId) {
        return jdbc.query("SELECT * FROM CHAT_CONVERSATION WHERE PARENT_CONVERSATION_ID=? AND TENANT_ID=? AND OWNER_USER_ID=? AND STATUS<>'DELETED' ORDER BY UPDATED_AT DESC", this::mapConversation, parentConversationId, tenantId, ownerUserId);
    }
    ...
    @Override
    public Optional<ConversationMessage> findMessage(String id, long tenantId, long ownerUserId) {
        return jdbc.query("SELECT m.* FROM CHAT_MESSAGE m JOIN CHAT_CONVERSATION c ON c.ID=m.CONVERSATION_ID WHERE m.ID=? AND m.TENANT_ID=? AND c.OWNER_USER_ID=?", this::mapMessage, id, tenantId, ownerUserId).stream().findFirst();
    }

    @Override
    public List<ConversationMessage> listMessages(String conversationId, long tenantId, long ownerUserId, int limit) {
        return jdbc.query("SELECT m.* FROM CHAT_MESSAGE m JOIN CHAT_CONVERSATION c ON c.ID=m.CONVERSATION_ID WHERE m.CONVERSATION_ID=? AND m.TENANT_ID=? AND c.OWNER_USER_ID=? ORDER BY m.SEQUENCE DESC LIMIT ?", this::mapMessage, conversationId, tenantId, ownerUserId, Math.min(Math.max(limit, 1), 1000));
    }
    @Override public int deleteConversation(String id, long tenantId, long ownerUserId) {
        return jdbc.update("UPDATE CHAT_CONVERSATION SET STATUS='DELETED',VERSION=VERSION+1,UPDATED_AT=CURRENT_TIMESTAMP WHERE ID=? AND TENANT_ID=? AND OWNER_USER_ID=? AND STATUS<>'DELETED'", id, tenantId, ownerUserId);
    }
    @Override public int deleteMessage(String id, long tenantId, long ownerUserId) {
        return jdbc.update("DELETE FROM CHAT_MESSAGE WHERE ID=? AND TENANT_ID=? AND CONVERSATION_ID IN (SELECT ID FROM CHAT_CONVERSATION WHERE TENANT_ID=? AND OWNER_USER_ID=?)",
                id, tenantId, tenantId, ownerUserId);
    }
    ...
    private ConversationMessage mapMessage(ResultSet r, int n) throws java.sql.SQLException {
        ...
        java.util.Map<String, Object> toolCall = java.util.Optional.ofNullable(r.getString("TOOL_CALL"))
                .map(json -> JSONUtils.parseObject(json, java.util.Map.class)).orElse(java.util.Map.of());
