...
public class ConversationService {
    ...
    public Conversation create(long tenantId, long ownerUserId, String sceneType, String title,
                               String platform, String model, String systemPrompt) {
        Instant now = Instant.now();
        Conversation conversation = new Conversation(
                UUID.randomUUID().toString(), tenantId, ownerUserId, sceneType,
    ...
    public ConversationMessage appendMessage(Conversation conversation, String parentMessageId, MessageRole role,
        ...
        List<ConversationMessage> current = repository.listMessages(
                conversation.id(), conversation.tenantId(), conversation.ownerUserId(), 1000);
    ...
    public GenerationRun recordCompletedGeneration(Conversation conversation, ConversationMessage input,
        ...
        Conversation latest = repository.findConversation(conversation.id(), conversation.tenantId(), conversation.ownerUserId()).orElse(conversation);
