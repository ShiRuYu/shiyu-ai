...
public class MagmaContextRetrievalAdapter implements ContextRetrievalPort {
    ...
    @Override public List<ContextItem> retrieve(ContextQuery query) {
        ...
        MemoryQuery magmaQuery = new MemoryQuery(query.tenantId(), query.namespace(), query.filters().get("subjectType"), query.filters().get("subjectId"), query.text(), null, null, null, 2, query.topK(), 2000);
        return memory.retrieve(magmaQuery).stream().map(path -> new ContextItem("MEMORY_EVENT", path.event().id(), path.event().content(), path.score(), new ContextCitation(path.event().eventType(), path.event().sourceId(), path.event().occurredAt().toString(), null), path.edges().stream().map(e -> e.graphType().name() + ":" + e.relationType()).toList(), "tenant-subject", path.event().occurredAt())).toList();
    }
}
