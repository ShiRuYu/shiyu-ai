...
public class InMemoryToolApprovalRepository implements ToolApprovalRepository {
    ...
    @Override public List<ToolApproval> listAll(long tenantId, long ownerUserId) { return values.values().stream().filter(a -> a.tenantId() == tenantId && a.ownerUserId() == ownerUserId).sorted(java.util.Comparator.comparing(ToolApproval::createdAt)).toList(); }
