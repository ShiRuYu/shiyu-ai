...
public class ContextAssemblyService {
    ...
    public ContextResult retrieve(ContextQuery query) {
        List<ContextItem> items = providers.stream()
                ...
                .limit(query.topK())
                .toList();
        return new ContextResult(items, new ContextTrace(UUID.randomUUID().toString(), query.tenantId(), query.text(),
                items.stream().map(ContextItem::sourceId).toList(), query.namespace(), Instant.now()));
    }

    public record ContextResult(List<ContextItem> items, ContextTrace trace) {
