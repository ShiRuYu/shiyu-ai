...
/** Adapts agent execution lifecycle events to the durable AI runtime run. */
@Slf4j
final class AgentRuntimeEventBridge {
    ...
    AiRun start(Execution execution, Map<String, Object> input) {
        ...
        long owner = execution.getUserId() == null
                ? number(input == null ? null : input.get("userId")) : execution.getUserId();
        ...
        try {
            ...
            AiRun run = runtime.startRun(
                    new AiRunContext(new TenantId(tenant), owner, appId, appVersionId, null, null,
    ...
    void finish(AiRun run, Execution execution) {
        ...
        try {
            AiRunStatus status = execution.getStatus() == ExecutionStatus.COMPLETED ? AiRunStatus.COMPLETED
                    : execution.getStatus() == ExecutionStatus.CANCELLED ? AiRunStatus.CANCELLED
                    : AiRunStatus.FAILED;
            runtime.finish(run.id(), run.tenantId(), run.ownerUserId().value(), status,
                    status == AiRunStatus.FAILED ? "AGENT_EXECUTION_FAILED" : null);
        } catch (RuntimeException ignored) {
            // The execution result remains authoritative once a terminal event is attempted.
        }
    }
