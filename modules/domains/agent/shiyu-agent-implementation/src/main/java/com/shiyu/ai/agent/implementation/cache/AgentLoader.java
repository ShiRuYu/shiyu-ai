...
/**
 * 实现 智能体 Loader 相关的业务处理、协作逻辑或基础设施能力。
 */
@Slf4j
@Component
public class AgentLoader {
    ...
    /**
     * 执行 智能体 Loader 相关业务操作，并维护必要的状态和协作关系。
     *
    ...
    /**
     * 执行 智能体 Loader 相关业务数据，并返回处理结果。
     *
     * @param actor 当前操作主体上下文，用于确定租户、用户和访问权限。
     * @param agentId 用于定位agent的标识。
     * @return 返回 智能体 Loader 相关操作生成的结果数据。
     */
    public AgentDefinition loadFromDb(ActorContext actor, String agentId) {
        ...
        if (agentDef == null || agentDef.getStatus() == null || agentDef.getStatus() != 1) {
            log.warn("Agent 不存在或已停用: agentIdPresent={}", agentId != null);
            return null;
        }

        String versionNumber = agentDef.getCurrentVersion();
        if (versionNumber == null || versionNumber.isEmpty()) {
            log.warn("Agent 没有当前版本: agentIdPresent={}", agentId != null);
            return null;
        }
        ...
        try {
            ...
            try {
                ...
                if (!extInfoJson.equals(agentDef.getExtInfo())) {
                    ...
                    agentAdminRepository.update(actor.tenantId(), agentDef);
    ...
    /**
     * 构建或转换 智能体 Loader 相关业务数据，并返回处理结果。
     *
     * @param agentId 用于定位agent的标识。
     * @param graphConfig 用于完成本次业务处理的 graphConfig 参数。
     * @return 返回 智能体 Loader 相关操作生成的结果数据。
     */
    public Graph buildGraph(String agentId, GraphConfigRequest graphConfig) {
        ...
        graph.validate();
        log.info("Graph 构建并验证成功: graphNamePresent={}", graphName != null);
        return graph;
    }
