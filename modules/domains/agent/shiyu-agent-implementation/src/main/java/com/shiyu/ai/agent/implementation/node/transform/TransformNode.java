...
public class TransformNode extends BaseNode {
    ...
    private static final FieldKey[] GET_INPUT_KEYS = {
        FieldKey.INPUT, FieldKey.DATA, FieldKey.CONTENT, FieldKey.TEXT, FieldKey.QUERY
    };
    ...
    /**
     * 执行 Transform 相关业务数据，并返回处理结果。
     *
     * @param input 用于完成本次业务处理的 input 参数。
     * @return 返回 Transform 相关操作生成的结果数据。
     */
    @Override
    protected NodeOutput doExecute(NodeInput input) throws Exception {
        log.info("执行数据转换节点：{}", config.getNodeName());
        log.debug(
                "转换配置：transformType={}, inputFormat={}, outputFormat={}",
                config.getTransformType(),
        ...
        try {
            ...
            Object result =
                    transformData(
                            inputData,
                            input.getParameter(FieldKey.TRANSFORM_TYPE, config.getTransformType()));
            ...
            output.setMsg("数据转换节点执行失败，请稍后重试");
            return output;
        }
    }
    ...
    private Map<String, Object> parseJsonToMap(String json) {
        try {
            ...
            log.warn(
                    "JSON 解析失败，返回空 Map: errorType={}, errorMessageLength={}",
    ...
    private String convertMapToJson(String mapData) {
        try {
            ...
            log.warn(
                    "Map 转换 JSON 失败，尝试视为 Map 字符串: errorType={}, errorMessageLength={}",
                    e.getClass().getSimpleName(),
                    e.getMessage() == null ? 0 : e.getMessage().length());
            return "{\"data\": \"" + mapData + "\"}";
        }
