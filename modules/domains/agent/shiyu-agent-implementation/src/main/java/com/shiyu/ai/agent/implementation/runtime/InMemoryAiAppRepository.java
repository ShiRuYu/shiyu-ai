...
public class InMemoryAiAppRepository implements AiAppRepository {
    ...
    @Override public List<AiApp> list(TenantId tenantId, long ownerUserId, int limit) { return apps.values().stream().filter(a -> a.tenantId().equals(requireTenant(tenantId)) && a.ownerUserId().value() == ownerUserId).sorted(Comparator.comparing(AiApp::updatedAt).reversed().thenComparing(AiApp::id)).limit(Math.max(1, Math.min(limit, 100))).toList(); }
    ...
    @Override public List<AiAppVersion> versions(String appId, TenantId tenantId) { return versions.values().stream().filter(v -> v.tenantId().equals(requireTenant(tenantId)) && v.appId().equals(appId)).sorted(Comparator.comparing(AiAppVersion::createdAt).reversed().thenComparing(AiAppVersion::id)).toList(); }
    @Override public synchronized int publishVersion(String appId, String versionId, TenantId tenantId) {
        ...
        versions.replaceAll((id, version) -> version.appId().equals(appId)
                && version.tenantId().equals(requireTenant(tenantId))
                && "PUBLISHED".equals(version.status())
                ? new AiAppVersion(version.id(), version.appId(), version.tenantId(), version.version(), version.configJson(), "ARCHIVED", version.createdAt(), version.publishedAt())
                : version);
        versions.put(versionId, new AiAppVersion(current.id(), current.appId(), current.tenantId(), current.version(), current.configJson(), "PUBLISHED", current.createdAt(), publishedAt));
        apps.computeIfPresent(appId, (id, app) -> new AiApp(app.id(), app.tenantId(), app.ownerUserId(), app.name(), app.description(), app.status(), versionId, app.createdAt(), publishedAt));
        return 1;
    }
    @Override public synchronized int archiveVersion(String appId, String versionId, TenantId tenantId) {
        AiAppVersion current = findVersion(appId, versionId, tenantId).orElseThrow(() -> new IllegalArgumentException("app version not found"));
