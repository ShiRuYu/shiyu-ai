...
public class RagEnhancementConfig extends NodeConfig {
    ...
    @Builder.Default private String enhancementStrategy = "SUMMARIZATION";

    /** 是否添加上下文（默认 true） */
    @Builder.Default private Boolean addContext = true;

    /** 上下文窗口大小（默认 3，用于 RE_RANK 和 SUMMARIZATION） */
    @Builder.Default private Integer contextWindowSize = 3;

    /** 最大文本长度（默认 2000） */
    @Builder.Default private Integer maxLength = 2000;

    /** 相似度阈值（默认 0.5，用于 FILTER 策略） */
    @Builder.Default private Double similarityThreshold = 0.5;
}
