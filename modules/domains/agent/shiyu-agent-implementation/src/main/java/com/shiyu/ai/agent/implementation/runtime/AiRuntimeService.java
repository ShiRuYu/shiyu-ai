...
public class AiRuntimeService implements AiRuntimePort {
    ...
    /**
     * A published executable app must fail closed before its first run. The light-weight runtime
     * does not own Agent/Knowledge repositories, so it validates the immutable binding contract
     * here and leaves object-level authorization to the owning platform at execution time.
     */
