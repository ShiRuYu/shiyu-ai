...
public class ToolApprovalService {
    ...
    public ToolApproval request(String runId, TenantId tenantId, long ownerUserId, String toolName, String argumentsRedacted) { Instant now = Instant.now(); ToolApproval value = new ToolApproval(UUID.randomUUID().toString(), runId, tenantId.value(), ownerUserId, toolName, argumentsRedacted, ToolApprovalStatus.PENDING, now, null, now.plusSeconds(300)); approvals.insert(value); return value; }
    public List<ToolApproval> list(String runId, TenantId tenantId, long ownerUserId) { approvals.expirePending(tenantId, ownerUserId); return approvals.list(runId, tenantId, ownerUserId); }
    public List<ToolApproval> listAll(TenantId tenantId, long ownerUserId) { approvals.expirePending(tenantId, ownerUserId); return approvals.listAll(tenantId, ownerUserId); }
    public ToolApproval require(String id, TenantId tenantId, long ownerUserId) { approvals.expirePending(tenantId, ownerUserId); return approvals.find(id, tenantId, ownerUserId).orElseThrow(() -> new IllegalArgumentException("approval not found")); }
