...
public final class ToolExecutionPipeline {
    ...
    public Result execute(AiRun run, Request request, Function<Map<String, Object>, Object> executor) {
        ...
        if (request.highRisk()) {
            if (request.approvalId() == null || request.approvalId().isBlank()) {
                approval = approvals.request(run.id(), run.tenantId(), run.ownerUserId().value(), request.toolName(), redactedArgs);
            ...
            approval = approvals.require(request.approvalId(), run.tenantId(), run.ownerUserId().value());
    ...
    public Result executeById(String runId, TenantId tenantId, long ownerUserId, Request request,
                              Function<Map<String, Object>, Object> executor) {
        return execute(runtime.requireRun(runId, Objects.requireNonNull(tenantId, "tenantId must not be null"), ownerUserId), request, executor);
    }
