...
public class AgentStateMachine {
    ...
    static {
        TRANSITIONS.put(
                ...
                Set.of(ExecutionStatus.RUNNING, ExecutionStatus.CANCELLED));
        TRANSITIONS.put(
        ...
        TRANSITIONS.put(
                ExecutionStatus.PAUSED, Set.of(ExecutionStatus.RUNNING, ExecutionStatus.CANCELLED));
    ...
    public static void transition(ExecutionStatus current, ExecutionStatus target) {
        ...
        if (allowed == null || !allowed.contains(target)) {
            throw new IllegalStateException(String.format("非法状态转换: %s → %s", current, target));
        }
    }
