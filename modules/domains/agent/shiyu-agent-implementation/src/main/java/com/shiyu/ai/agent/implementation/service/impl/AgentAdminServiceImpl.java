...
public class AgentAdminServiceImpl implements AgentAdminService {
    ...
    public Pair<Long, List<AgentVO>> getPage(
        ...
        Pair<Long, List<AgentDefBO>> result =
                agentAdminRepository.selectPage(actor.tenantId(), pageNo, pageSize, name, status);
    ...
    public AgentDetailVO getById(ActorContext actor, Long id) {
        ...
        List<AgentVersionBO> versions =
                agentAdminRepository.selectVersionsByAgentId(actor.tenantId(), def.getAgentId());
        List<AgentVersionVO> versionVOs =
                versions.stream().map(this::toVersionVO).collect(Collectors.toList());
        return AgentDetailVO.builder()
                .id(def.getId())
                .agentId(def.getAgentId())
                .name(def.getName())
                .description(def.getDescription())
                .currentVersion(def.getCurrentVersion())
    ...
    public AgentVO create(ActorContext actor, AgentRequest request) {
        AgentDefBO existing =
                agentAdminRepository.selectByAgentId(actor.tenantId(), request.getAgentId());
        if (existing != null) {
    ...
    private List<NodeTypeMetaVO.FieldMeta> buildFieldMetas(NodeType type) {
        ...
        if (type == NodeType.RAG_RETRIEVAL) {
            fields.add(
                    fieldWithSource(
                            field(
                                    ...
                                    false,
                                    "为空时检索当前用户所有有权限空间"),
                            new NodeTypeMetaVO.DataSourceConfig(
                                    "api",
            ...
            fields.add(
                    field(
                            ...
                            "内容来源",
                            "select",
                            List.of("DOCUMENT", "KNOWLEDGE_ENTRY"),
                            false,
                            Map.of(
                                    "multiple",
                                    true,
                                    "options",
                                    Map.of("DOCUMENT", "文档", "KNOWLEDGE_ENTRY", "知识条目"))));
            fields.add(
                    field(
                            "retrievalMode",
                            "检索模式",
                            ...
                            Map.of(
                                    "options",
                                    Map.of("KEYWORD", "关键词", "VECTOR", "向量", "HYBRID", "混合"))));
        ...
        switch (type) {
            case INTENT:
                fields.add(
                        fieldWithSource(
                                ...
                                new NodeTypeMetaVO.DataSourceConfig(
                                        ...
                                        null)));
                fields.add(
                        field("confidenceThreshold", "置信度阈值", "number", 0.75, false, "意图识别的最低置信度"));
                fields.add(
                        fieldWithSource(
                                field("platform", "AI平台", "select", "", false, "选择AI平台"),
                                new NodeTypeMetaVO.DataSourceConfig(
                                        "api",
                                        DataSourceApiConstants.PLATFORM_ENABLED,
                                        null,
                                        "name",
                                        "code",
                                        null)));
                fields.add(
                        fieldWithSource(
                                field("modelName", "模型名称", "select", "", false, "选择模型"),
                                new NodeTypeMetaVO.DataSourceConfig(
                                        "api",
                                        DataSourceApiConstants.MODEL_BY_PLATFORM,
                                        null,
                                        "displayName",
                                        "modelName",
                                        "platform")));
            ...
            case RAG_ENHANCEMENT:
                fields.add(
                        field(
                                ...
                                "SUMMARIZATION",
                                false,
                                Map.of("options", List.of("SUMMARIZATION", "CHUNKING", "HYBRID"))));
            ...
            case OUTPUT_FORMAT:
                fields.add(
                        field(
                                ...
                                "TEXT",
                                false,
                                Map.of("options", List.of("TEXT", "JSON", "MARKDOWN", "HTML"))));
            ...
            case AGENT_CALL:
                fields.add(
                        fieldWithSource(
                                ...
                                new NodeTypeMetaVO.DataSourceConfig(
                                        "api",
                                        DataSourceApiConstants.AGENT_LIST_ALL,
                                        null,
                                        "name",
                                        "code",
                                        null)));
    ...
    private NodeTypeMetaVO.FieldMeta field(
            String key,
            String label,
            String type,
            Object defaultValue,
    ...
    private NodeTypeMetaVO.FieldMeta fieldWithSource(
            NodeTypeMetaVO.FieldMeta meta, NodeTypeMetaVO.DataSourceConfig source) {
