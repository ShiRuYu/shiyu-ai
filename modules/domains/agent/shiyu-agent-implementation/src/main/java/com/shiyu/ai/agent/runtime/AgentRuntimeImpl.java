...
public class AgentRuntimeImpl implements AgentRuntime {
    ...
    public Execution execute(ActorContext actor, String agentId, String version, Map<String, Object> input) {
        ...
        try {
            ...
            eventPublisher.publish(new AgentExecutionFailedEvent(
                    execution.getExecutionId(), agentId, publicFailureCode()));
        ...
        if (result.getStatus() == ExecutionStatus.COMPLETED) {
            ...
            eventPublisher.publish(new AgentExecutionFailedEvent(
                    result.getExecutionId(), agentId, publicFailureCode()));
        }
        ...
        return Flux.<Map<String, Object>>create(sink -> {
            ...
            try {
                ...
                log.error("Agent 流式执行失败: agentIdPresent={}, executionIdPresent={}, errorType={}, errorMessageLength={}",
                        agentId != null, execution.getExecutionId() != null,
    ...
    public void pause(ActorContext actor, String executionId) {
        ...
        execution.pause();
        saveExecution(actor, execution);
        log.info("Agent 执行已暂停: executionIdPresent={}", executionId != null);
    }
    ...
    public void cancel(ActorContext actor, String executionId) {
        ...
        execution.cancel();
        saveExecution(actor, execution);
        log.info("Agent 执行已取消: executionIdPresent={}", executionId != null);
    }
    ...
    private void finishRuntime(AiRun run, Execution execution) {
        ...
        try {
            ...
            runtime.finish(run.id(), run.tenantId(), run.ownerUserId().value(), status,
                    status == AiRunStatus.FAILED ? publicFailureCode() : null);
        } catch (RuntimeException ignored) { }
    }

    private static String publicFailureCode() {
