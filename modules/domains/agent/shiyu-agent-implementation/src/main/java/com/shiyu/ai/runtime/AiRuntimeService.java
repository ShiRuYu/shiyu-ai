...
public class AiRuntimeService implements AiRuntimePort {
    ...
    public AiApp createApp(TenantId tenantId, long ownerUserId, String name, String description) {
        AiApp app = new AiApp(UUID.randomUUID().toString(), tenant(tenantId), new UserId(ownerUserId), name, description, "ACTIVE", null, Instant.now(), Instant.now());
        apps.insert(app);
        return app;
    }

    public AiApp requireApp(String id, TenantId tenantId, long ownerUserId) { return apps.find(id, tenant(tenantId), ownerUserId).orElseThrow(() -> new IllegalArgumentException("app not found")); }
    public List<AiApp> listApps(TenantId tenantId, long ownerUserId, int limit) { return apps.list(tenant(tenantId), ownerUserId, limit); }
    public AiAppVersion createVersion(String appId, TenantId tenantId, long ownerUserId, String version, String configJson) {
        requireApp(appId, tenantId, ownerUserId);
        AiAppVersion value = new AiAppVersion(UUID.randomUUID().toString(), appId, tenant(tenantId), version, configJson, "DRAFT", Instant.now(), null);
        apps.insertVersion(value);
        return value;
    }
    public List<AiAppVersion> versions(String appId, TenantId tenantId, long ownerUserId) { requireApp(appId, tenantId, ownerUserId); return apps.versions(appId, tenant(tenantId)); }
    public AiAppVersion publish(String appId, String versionId, TenantId tenantId, long ownerUserId) {
        requireApp(appId, tenantId, ownerUserId);
        AiAppVersion version = apps.findVersion(appId, versionId, tenant(tenantId)).orElseThrow(() -> new IllegalArgumentException("app version not found"));
        ...
        if (apps.publishVersion(appId, versionId, tenant(tenantId)) != 1) throw new IllegalStateException("app version publish conflict");
        return apps.findVersion(appId, versionId, tenant(tenantId)).orElseThrow();
    }
    public AiAppVersion archive(String appId, String versionId, TenantId tenantId, long ownerUserId) {
    ...
    public AiAppPreview preview(String appId, String versionId, TenantId tenantId, long ownerUserId, String prompt) {
        AiApp app = requireApp(appId, tenantId, ownerUserId);
        AiAppVersion version = apps.findVersion(appId, versionId, tenant(tenantId)).orElseThrow(() -> new IllegalArgumentException("app version not found"));
        Map<String,Object> config;
        try { config = version.configJson() == null || version.configJson().isBlank() ? Map.of() : JSONUtils.parseMap(version.configJson()); }
    ...
    public AiAppVersion requirePublishedVersion(String appId, TenantId tenantId, long ownerUserId) {
        ...
        AiAppVersion version = apps.findVersion(appId, versionId, tenant(tenantId)).orElseThrow(() -> new IllegalStateException("published app version not found"));
    ...
    public AiRun startRun(AiRunContext context, AiRunSource source, String sourceId, String model, String prompt) {
        ...
        AiRun run = new AiRun(UUID.randomUUID().toString(), context.tenantId(), new UserId(context.ownerUserId()), context.appId(), context.appVersionId(), source, sourceId,
        ...
        try {
            ...
            try { finish(running.id(), running.tenantId(), running.ownerUserId().value(), AiRunStatus.FAILED, "RUN_EVENT_WRITE_FAILED"); }
    ...
    public AiRun finish(String id, TenantId tenantId, long ownerUserId, AiRunStatus terminal, String errorCode) {
        AiRun current = runs.find(id, tenant(tenantId), ownerUserId).orElseThrow(() -> new IllegalArgumentException("run not found"));
    ...
    public List<AiRunEvent> events(String id, TenantId tenantId, long ownerUserId, long afterSeq, int limit) { return runs.events(id, tenant(tenantId), ownerUserId, afterSeq, Math.max(1, Math.min(limit, 1000))); }
    public List<AiRun> listRuns(TenantId tenantId, long ownerUserId, int limit) { return runs.list(tenant(tenantId), ownerUserId, limit); }
    public AiRun requireRun(String id, TenantId tenantId, long ownerUserId) { return runs.find(id, tenant(tenantId), ownerUserId).orElseThrow(() -> new IllegalArgumentException("run not found")); }
    public AiRun requireGenerationRun(String generationId, TenantId tenantId, long ownerUserId) { return runs.findByGeneration(generationId, tenant(tenantId), ownerUserId).orElseThrow(() -> new IllegalArgumentException("runtime run not found")); }
    public AiRun linkGeneration(AiRun run, String generationId) {
        ...
        if (run.generationId() == null && runs.linkGeneration(run.id(), run.tenantId(), run.ownerUserId().value(), generationId) != 1) {
    ...
    public AiRun requireExecutionRun(String executionId, TenantId tenantId, long ownerUserId) { return runs.findByExecution(executionId, tenant(tenantId), ownerUserId).orElseThrow(() -> new IllegalArgumentException("runtime run not found")); }
    public AiRun recordUsage(String id, TenantId tenantId, long ownerUserId, long promptTokens, long completionTokens, boolean estimated, String costSnapshot) {
    ...
    public long append(AiRun run, AiRunEventType type, String payload, boolean redacted) {
        long seq = runs.appendNextEvent(run.id(), run.tenantId(), run.ownerUserId().value(), type,
    ...
    public long append(AiRun run, AiRunEventType type, String payload, boolean redacted,
        ...
        return runs.appendNextEvent(run.id(), run.tenantId(), run.ownerUserId().value(), type,
    ...
    private static TenantId tenant(TenantId tenantId) { return Objects.requireNonNull(tenantId, "tenantId must not be null"); }
}
