...
public class ConditionNode extends BaseNode {
    ...
    /**
     * 执行 Condition 相关业务数据，并返回处理结果。
     *
     * @param input 用于完成本次业务处理的 input 参数。
     * @return 返回 Condition 相关操作生成的结果数据。
     */
    @Override
    protected NodeOutput doExecute(NodeInput input) throws Exception {
        log.info("执行条件判断节点：{}", config.getNodeName());
        log.debug(
                "条件配置：conditionType={}, conditionExpression={}, defaultBranch={}",
                config.getConditionType(),
                config.getConditionExpression(),
        ...
        try {
            ...
            String conditionExpression =
                    input.getParameter(
                            FieldKey.CONDITION_EXPRESSION, config.getConditionExpression());
            String conditionType =
                    input.getParameter(
                            FieldKey.CONDITION_TYPE,
                            config.getConditionType() != null
                                    ? config.getConditionType()
                                    : "EXPRESSION");
            ...
            String nextBranch =
                    result
                            ? input.getParameter(FieldKey.TRUE_BRANCH, config.getTrueBranch())
                            : input.getParameter(
                                    FieldKey.DEFAULT_BRANCH, config.getDefaultBranch());
            ...
            output.setMsg("条件判断节点执行失败，请稍后重试");
            // 返回默认分支
     ...
     *
     * <p>支持 #variableName 引用 state 中的任意字段，支持布尔表达式如 {@code #score > 0.5}
     */
    private boolean evaluateScript(String script, NodeInput input) {
        try {
            ...
            log.warn(
                    "脚本条件评估失败: scriptPresent={}, errorType={}, errorMessageLength={}",
                    script != null,
