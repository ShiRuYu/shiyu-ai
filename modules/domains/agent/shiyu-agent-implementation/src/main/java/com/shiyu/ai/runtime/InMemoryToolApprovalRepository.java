...
public class InMemoryToolApprovalRepository implements ToolApprovalRepository {
    ...
    @Override public List<ToolApproval> list(String runId, TenantId tenantId, long ownerUserId) { long value = tenant(tenantId); return values.values().stream().filter(a -> a.runId().equals(runId) && a.tenantId() == value && a.ownerUserId() == ownerUserId).sorted(java.util.Comparator.comparing(ToolApproval::createdAt).thenComparing(ToolApproval::id)).toList(); }
    @Override public List<ToolApproval> listAll(TenantId tenantId, long ownerUserId) { long value = tenant(tenantId); return values.values().stream().filter(a -> a.tenantId() == value && a.ownerUserId() == ownerUserId).sorted(java.util.Comparator.comparing(ToolApproval::createdAt).reversed().thenComparing(ToolApproval::id)).toList(); }
