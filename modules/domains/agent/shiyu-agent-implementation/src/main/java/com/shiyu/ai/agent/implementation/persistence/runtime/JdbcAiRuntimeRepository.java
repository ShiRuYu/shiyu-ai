...
public class JdbcAiRuntimeRepository implements AiRunRepository {
    ...
    @Override public List<AiRun> list(TenantId tenant,long owner,int limit) { return jdbc.query("SELECT * FROM AI_RUN WHERE TENANT_ID=? AND OWNER_USER_ID=? ORDER BY CREATED_AT DESC,ID ASC LIMIT ?", (rs, rowNum) -> mapRun(rs), tenant(tenant), owner, Math.max(1, Math.min(limit, 500))); }
    @Override public Optional<AiRun> findByGeneration(String generationId,TenantId tenant,long owner) { return jdbc.query("SELECT * FROM AI_RUN WHERE GENERATION_ID=? AND TENANT_ID=? AND OWNER_USER_ID=? ORDER BY CREATED_AT DESC,ID ASC LIMIT 1", rs -> rs.next()?Optional.of(mapRun(rs)):Optional.empty(), generationId,tenant(tenant),owner); }
    ...
    @Override public Optional<AiRun> findByExecution(String executionId,TenantId tenant,long owner) { return jdbc.query("SELECT * FROM AI_RUN WHERE EXECUTION_ID=? AND TENANT_ID=? AND OWNER_USER_ID=? ORDER BY CREATED_AT DESC,ID ASC LIMIT 1", rs -> rs.next()?Optional.of(mapRun(rs)):Optional.empty(), executionId,tenant(tenant),owner); }
    ...
    public long appendNextEvent(String runId, TenantId tenantId, long ownerUserId, AiRunEventType type,
        ...
        if (!terminalEvents.isEmpty()) {
                ...
                if (existing.type() != type || !java.util.Objects.equals(existing.payload(), normalizedPayload) || existing.redacted() != redacted) {
        ...
        try {
            jdbc.update("INSERT INTO AI_RUN_EVENT (RUN_ID,TENANT_ID,SEQ,TYPE,SCHEMA_VERSION,TURN_ID,STEP_ID,PARENT_EVENT_SEQ,CONVERSATION_ID,GENERATION_ID,EXECUTION_ID,APP_ID,APP_VERSION_ID,PROVIDER_REQUEST_ID,TRACE_ID,PAYLOAD,REDACTED,CREATED_AT) " +
                    ...
                    runId, seq, type.name(), turnId, stepId, seq, seq, providerRequestId, normalizedPayload, redacted,
