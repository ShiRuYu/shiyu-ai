...
public class JdbcAiAppRepository implements AiAppRepository {
    ...
    @Override public List<AiApp> list(TenantId tenant,long owner,int limit) { return jdbc.query("SELECT * FROM AI_APP WHERE TENANT_ID=? AND OWNER_USER_ID=? ORDER BY UPDATED_AT DESC,ID ASC LIMIT ?",(rs,n)->mapApp(rs),tenant(tenant),owner,Math.max(1,Math.min(limit,100))); }
    ...
    @Override public List<AiAppVersion> versions(String appId,TenantId tenant) { return jdbc.query("SELECT * FROM AI_APP_VERSION WHERE APP_ID=? AND TENANT_ID=? ORDER BY CREATED_AT DESC,ID ASC",(rs,n)->mapVersion(rs),appId,tenant(tenant)); }
    @Override
    @Transactional
    public int publishVersion(String appId,String id,TenantId tenant) { long tenantValue=tenant(tenant); int n=jdbc.update("UPDATE AI_APP_VERSION SET STATUS='PUBLISHED',PUBLISHED_AT=CURRENT_TIMESTAMP WHERE APP_ID=? AND ID=? AND TENANT_ID=? AND STATUS='DRAFT'",appId,id,tenantValue); if(n==1) jdbc.update("UPDATE AI_APP_VERSION SET STATUS='ARCHIVED' WHERE APP_ID=? AND TENANT_ID=? AND STATUS='PUBLISHED' AND ID<>?",appId,tenantValue,id); if(n==1) jdbc.update("UPDATE AI_APP SET PUBLISHED_VERSION_ID=?,UPDATED_AT=CURRENT_TIMESTAMP WHERE ID=? AND TENANT_ID=?",id,appId,tenantValue); return n; }
    @Override public int archiveVersion(String appId,String id,TenantId tenant) { return jdbc.update("UPDATE AI_APP_VERSION SET STATUS='ARCHIVED' WHERE APP_ID=? AND ID=? AND TENANT_ID=? AND STATUS='DRAFT'",appId,id,tenant(tenant)); }
