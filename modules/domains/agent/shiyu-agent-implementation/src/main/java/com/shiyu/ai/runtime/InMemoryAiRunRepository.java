...
public class InMemoryAiRunRepository implements AiRunRepository {
    ...
    private static final Comparator<AiRun> NEWEST_FIRST = Comparator.comparing(AiRun::createdAt).reversed().thenComparing(AiRun::id);
    ...
    @Override public List<AiRun> list(TenantId tenantId, long ownerUserId, int limit) { return runs.values().stream().filter(r -> r.tenantId().equals(requireTenant(tenantId)) && r.ownerUserId().value() == ownerUserId).sorted(NEWEST_FIRST).limit(Math.max(1, Math.min(limit, 500))).toList(); }
    @Override public Optional<AiRun> findByGeneration(String generationId, TenantId tenantId, long ownerUserId) { return runs.values().stream().filter(r -> r.tenantId().equals(requireTenant(tenantId)) && r.ownerUserId().value() == ownerUserId && java.util.Objects.equals(r.generationId(), generationId)).sorted(NEWEST_FIRST).findFirst(); }
    ...
    @Override public Optional<AiRun> findByExecution(String executionId, TenantId tenantId, long ownerUserId) { return runs.values().stream().filter(r -> r.tenantId().equals(requireTenant(tenantId)) && r.ownerUserId().value() == ownerUserId && java.util.Objects.equals(r.executionId(), executionId)).sorted(NEWEST_FIRST).findFirst(); }
    ...
    @Override public synchronized AiRun updateTerminalAndAppend(AiRun run, long expectedVersion, AiRunEventType eventType,
        ...
        AiRun previous = find(run.id(), run.tenantId(), run.ownerUserId().value())
        ...
        synchronized (stream) {
            ...
            if (terminal != null) {
                if (type == terminal.type()) {
                    if (!java.util.Objects.equals(terminal.payload(), normalizedPayload) || terminal.redacted() != redacted) {
            ...
            AiRunEvent event = new AiRunEvent(runId, tenantId, seq, type, 1, turnId, stepId,
                    ...
                    run.appId(), run.appVersionId(), providerRequestId, run.traceId(), normalizedPayload, redacted, createdAt);
            stream.add(event);
    ...
    @Override public long appendEvent(AiRunEvent event) {
        ...
        synchronized (stream) {
            ...
            if (stream.stream().anyMatch(e -> e.type() == AiRunEventType.RUN_COMPLETED
            ...
            stream.add(event);
            runs.computeIfPresent(event.runId(), (id, current) -> current.withLastEventSeq(event.seq()));
            return event.seq();
        }
    }
    @Override public List<AiRunEvent> events(String runId, TenantId tenantId, long ownerUserId, long afterSeq, int limit) {
        ...
        return snapshot.stream().filter(e -> e.seq() > afterSeq).sorted(Comparator.comparingLong(AiRunEvent::seq)).limit(Math.max(1, Math.min(limit, 100_000))).toList();
    }
