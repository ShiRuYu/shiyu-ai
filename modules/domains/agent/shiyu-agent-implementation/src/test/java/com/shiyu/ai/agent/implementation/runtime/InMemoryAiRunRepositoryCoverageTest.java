...
class InMemoryAiRunRepositoryCoverageTest {
    ...
    void treatsNullTerminalPayloadAsTheCanonicalEmptyPayloadOnReplay() {
        ...
        long sequence = repository.appendNextEvent(run.id(), new TenantId(1), 2,
                AiRunEventType.RUN_COMPLETED, null, false, Instant.now());

        assertEquals(sequence, repository.appendNextEvent(run.id(), new TenantId(1), 2,
                AiRunEventType.RUN_COMPLETED, null, false, Instant.now()));
    }
    ...
    void externalEventsAdvanceRunSequenceAndCannotFollowTerminalState() {
        ...
        AiRunEvent delta = new AiRunEvent(run.id(), new TenantId(1), 2, AiRunEventType.MODEL_DELTA, "{}", true, Instant.now());
        ...
        AiRun completed = runtime.finish(run.id(), new TenantId(1), 2, AiRunStatus.COMPLETED, "done");
        assertEquals(3, completed.lastEventSeq());
        assertThrows(IllegalStateException.class, () -> repository.appendEvent(
                new AiRunEvent(run.id(), new TenantId(1), 4, AiRunEventType.MODEL_DELTA, "{}", true, Instant.now())));
    }
    ...
    void rollsBackTerminalStateWhenEventAppendFails() {
        ...
        AiRun run = runtime.startRun(new AiRunContext(new TenantId(1), 2, null, null, null, null, null, null, Map.of()),
                AiRunSource.API, "api", "model", "prompt");
        repository.appendNextEvent(run.id(), new TenantId(1), 2, AiRunEventType.RUN_FAILED,
                "existing", true, Instant.now());

        assertThrows(IllegalStateException.class,
                () -> repository.updateTerminalAndAppend(run.transition(AiRunStatus.COMPLETED), run.version(),
                        AiRunEventType.RUN_COMPLETED, "different", true));
        assertEquals(AiRunStatus.RUNNING, repository.find(run.id(), new TenantId(1), 2).orElseThrow().status());
    }
    ...
    void readsEventStreamSafelyWhileEventsAreAppended() throws Exception {
        ...
        for (int i = 0; i < 10_000; i++) {
            repository.appendNextEvent(run.id(), new TenantId(1), 2, AiRunEventType.MODEL_DELTA,
                    "{}", true, Instant.now());
        }
        ...
        Future<?> reader = pool.submit(() -> {
            start.await();
            for (int i = 0; i < 100; i++) repository.events(run.id(), new TenantId(1), 2, 0, 20_000);
            return null;
        });
        start.countDown();
        assertDoesNotThrow(() -> {
    ...
    void ordersRunsAndLatestLookupsByIdWhenTimestampsTie() {
        ...
        assertEquals("run-a", repository.findByGeneration("generation", new TenantId(1), 2).orElseThrow().id());
        assertEquals("run-a", repository.findByExecution("execution", new TenantId(1), 2).orElseThrow().id());
    }

    private static AiRun run(String id, String generationId, String executionId) {
        return new AiRun(id, new TenantId(1), new com.shiyu.ai.kernel.context.UserId(2), null, null,
                AiRunSource.API, "api-" + id, null, null, null, generationId, executionId,
                "model", "prompt", AiRunStatus.RUNNING, 0, 0, false, null, CREATED_AT, null, null, 0);
    }
}
