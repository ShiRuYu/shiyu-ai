...
class ExecutionHistoryServiceImplCoverageTest {
    ...
    void startsAndCompletesExecutionInTenantScope() {
        ...
        String id =
                service.startExecution(
                        ACTOR, "agent", "v1", "session", "node", "HTTP", "{\"q\":1}");
        assertNotNull(id);
        AgentExecutionBO row = new AgentExecutionBO();
        row.setExecutionId(id);
        row.setStartTime(java.time.LocalDateTime.now().minusSeconds(1));
    ...
    void completionRequiresAnExistingExecution() {
        ...
        assertThrows(
                ...
                () -> service.completeExecution(ACTOR, "missing", "", 2, "not found"));
    }
}
