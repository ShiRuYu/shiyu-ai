...
/**
 * 验证 Jdbc AI Runtime Repository 相关功能、边界条件、异常路径和协作行为。
 */
...
class JdbcAiRuntimeRepositoryTest {
    ...
    void appendsEventsIdempotentlyAndProtectsTerminalRuns() {
        ...
        assertEquals(
                ...
                repository.appendNextEvent(
                        "run", new TenantId(7), 9, AiRunEventType.MODEL_STARTED, null, false, now));
        assertEquals(
                1,
                repository
                        .updateTerminalAndAppend(
                                run, 0, AiRunEventType.RUN_COMPLETED, "done", false)
                        .lastEventSeq());
        ...
        when(jdbc.query(contains("TYPE IN"), any(RowMapper.class), any(Object[].class)))
                .thenReturn(List.of(event(2, AiRunEventType.RUN_COMPLETED, "{}", false)));
        assertEquals(
                2,
                repository.appendNextEvent(
                        "run", new TenantId(7), 9, AiRunEventType.RUN_COMPLETED, null, false, now));
        ...
        when(jdbc.queryForObject(contains("COUNT(*)"), eq(Integer.class), any(Object[].class)))
                .thenReturn(0);
        assertEquals(
                1, repository.appendEvent(event(1, AiRunEventType.MODEL_STARTED, "start", false)));
        assertThrows(
                IllegalStateException.class,
    ...
    void mapsRowsAndReportsMissingRunsOrConcurrentChanges() throws Exception {
        ...
        when(rs.getTimestamp("CREATED_AT")).thenReturn(Timestamp.from(now));
        Method mapEvent =
                JdbcAiRuntimeRepository.class.getDeclaredMethod(
        ...
        doAnswer(
                        (Answer<Object>)
                                invocation -> {
                                    String sql = invocation.getArgument(0);
                                    return sql.startsWith("SELECT LAST_EVENT_SEQ")
                                            ? null
                                            : Optional.empty();
                                })
                .when(jdbc)
        ...
        assertThrows(
                ...
                () ->
                        repository.appendNextEvent(
                                ...
                                false,
                                now));
        when(jdbc.update(startsWith("UPDATE AI_RUN SET STATUS"), any(Object[].class)))
                .thenReturn(0);
        assertThrows(
                IllegalStateException.class,
                () ->
                        repository.updateTerminalAndAppend(
                                run(AiRunStatus.COMPLETED),
                                4,
                                AiRunEventType.RUN_COMPLETED,
                                "",
                                false));
    ...
    @Test
    void rejectsExplicitEventsWhenTheRunDoesNotExist() {
        ...
        assertThrows(
                ...
                () ->
                        repository.appendEvent(
                                event(1, AiRunEventType.MODEL_STARTED, "start", false)));
        verify(jdbc, never()).update(startsWith("INSERT INTO AI_RUN_EVENT"), any(Object[].class));
    }
    ...
    void preservesDuplicateSequenceOnlyWhenPayloadMatches() {
        ...
        when(jdbc.queryForObject(
                        startsWith("SELECT * FROM AI_RUN_EVENT"),
                        any(RowMapper.class),
                        any(Object[].class)))
    ...
    void validatesExplicitEventSequenceAndDuplicateAppendBranches() {
        ...
        assertThrows(
                IllegalStateException.class,
                () -> repository.appendEvent(event(2, AiRunEventType.MODEL_DELTA, "delta", false)));
    ...
    private static AiRunEvent event(
            long seq, AiRunEventType type, String payload, boolean redacted) {
