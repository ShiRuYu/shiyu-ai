...
/**
 * 验证 智能体 Admin Repository Coverage 相关功能、边界条件、异常路径和协作行为。
 */
class AgentAdminRepositoryCoverageTest {
    ...
    void queriesAgentsAndVersionsWithTenantFilters() {
        ...
        AgentDefDO def = def(1L);
        AgentVersionDO version = version(2L);
        ...
        AgentDefBO defBo = new AgentDefBO();
        defBo.setId(1L);
        defBo.setAgentId("agent");
        AgentVersionBO versionBo = new AgentVersionBO();
        versionBo.setId(2L);
        versionBo.setAgentId("agent");
        try (var conversion = mockStatic(MapstructUtils.class)) {
            conversion
                    .when(() -> MapstructUtils.convert(anyList(), eq(AgentDefBO.class)))
                    .thenReturn(List.of(defBo));
            conversion
                    .when(() -> MapstructUtils.convert(anyList(), eq(AgentVersionBO.class)))
                    .thenReturn(List.of(versionBo));
            conversion
                    .when(() -> MapstructUtils.convert(any(AgentDefDO.class), eq(AgentDefBO.class)))
                    .thenReturn(defBo);
            conversion
                    .when(
                            () ->
                                    MapstructUtils.convert(
                                            any(AgentVersionDO.class), eq(AgentVersionBO.class)))
                    .thenReturn(versionBo);
            ...
            assertEquals(
                    2L, repository.selectVersionByAgentIdAndNumber(TENANT, "agent", "v1").getId());
    ...
    void createsUpdatesDeletesAndRejectsCrossTenantRows() {
        ...
        doAnswer(
                        invocation -> {
                            AgentDefDO row = invocation.getArgument(0);
                            row.setId(10L);
                            return 1;
                        })
                .when(defs)
                .insertSelective(any(AgentDefDO.class));
        doAnswer(
                        invocation -> {
                            AgentVersionDO row = invocation.getArgument(0);
                            row.setId(20L);
                            return 1;
                        })
                .when(versions)
                .insertSelective(any(AgentVersionDO.class));
        ...
        def.setAgentId("agent");
        def.setName("Demo");
        AgentDefDO mappedDef = def(10L);
        AgentVersionDO mappedVersion = version(20L);
        try (var conversion = mockStatic(MapstructUtils.class)) {
            conversion
                    .when(() -> MapstructUtils.convert(any(AgentDefBO.class), eq(AgentDefDO.class)))
                    .thenReturn(mappedDef);
            conversion
                    .when(
                            () ->
                                    MapstructUtils.convert(
                                            any(AgentVersionBO.class), eq(AgentVersionDO.class)))
                    .thenReturn(mappedVersion);
            AgentDefBO created = repository.create(TENANT, def);
            assertEquals(10L, created.getId());
            assertEquals(11L, created.getTenantId());
            when(defs.selectOneByQuery(any(QueryWrapper.class))).thenReturn(def(10L));
            created.setId(10L);
            assertSame(created, repository.update(TENANT, created));
            repository.deleteById(TENANT, 10L);
            repository.deleteByAgentId(TENANT, "agent");
            AgentVersionBO vb = new AgentVersionBO();
            vb.setAgentId("agent");
            vb.setVersionNumber("v1");
            ...
            vb.setId(20L);
            assertSame(vb, repository.updateVersion(TENANT, vb));
            repository.deleteVersionById(TENANT, 20L);
    ...
    void mapsMissingRowsToNull() {
        ...
        when(defs.selectOneByQuery(any(QueryWrapper.class))).thenReturn(null);
        when(versions.selectOneByQuery(any(QueryWrapper.class))).thenReturn(null);
        assertNull(repository.selectById(TENANT, 1L));
        assertNull(repository.selectByAgentId(TENANT, "none"));
        assertNull(repository.selectVersionById(TENANT, 1L));
        assertNull(repository.selectVersionByAgentIdAndNumber(TENANT, "none", "v1"));
        repository.deleteById(TENANT, 1L);
        repository.deleteByAgentId(TENANT, "none");
        repository.deleteVersionById(TENANT, 1L);
    }
    ...
    private static AgentAdminRepositoryImpl repository(
            AgentDefMapper defs, AgentVersionMapper versions) {
        AgentAdminRepositoryImpl repository = new AgentAdminRepositoryImpl();
        try {
            var f = AgentAdminRepositoryImpl.class.getDeclaredField("agentDefMapper");
            f.setAccessible(true);
            f.set(repository, defs);
            f = AgentAdminRepositoryImpl.class.getDeclaredField("agentVersionMapper");
            f.setAccessible(true);
            f.set(repository, versions);
            return repository;
    ...
    private static AgentDefDO def(long id) {
        AgentDefDO row = new AgentDefDO();
        row.setId(id);
        row.setTenantId(11L);
    ...
    private static AgentVersionDO version(long id) {
        AgentVersionDO row = new AgentVersionDO();
        row.setId(id);
        row.setTenantId(11L);
        row.setAgentId("agent");
        row.setVersionNumber("v1");
        row.setDelFlag(0);
        return row;
    }
}
