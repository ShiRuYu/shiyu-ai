...
class NodeFactoryTest {
    ...
    void createsBatchesAndRejectsInvalidConfiguration() {
        ...
        factory.clearNodes();
        assertTrue(factory.getAllRegisteredNodes().isEmpty());
    }
    ...
    void createsAllFallbackNodeTypesAndConvertsGenericConfig() {
        ...
        for (NodeType type : fallbackTypes) {
            ...
            assertNotNull(node);
            assertEquals(
                    type == NodeType.DEFAULT ? NodeType.DEFAULT : type,
                    node.getConfig().getNodeType());
        }

        NodeConfig genericCondition =
                NodeConfig.builder()
                        .nodeId("converted")
                        .nodeName("converted")
    ...
    void usesBeanCreatorsInjectsServicesAndReportsBatchFailures() {
        ...
        BaseNode created =
                withBeans.createNode(
                        NodeConfig.builder().nodeId("intent-1").nodeType(NodeType.INTENT).build());
        ...
        assertNotNull(
                withBeans.createNodesWithServices(
                        Map.of(
                                "ok-2",
                                NodeConfig.builder()
                                        .nodeId("ok-2")
    ...
    void beanCreatorFailureIsReportedWhenFallbackRequiresDependencies() {
        ...
        when(failing.create(any(NodeConfig.class)))
        ...
        RuntimeException failure =
                assertThrows(
                        ...
                        () ->
                                withFailingBean.createNode(
                                        NodeType.INTENT, "intent-fallback", "intent"));
