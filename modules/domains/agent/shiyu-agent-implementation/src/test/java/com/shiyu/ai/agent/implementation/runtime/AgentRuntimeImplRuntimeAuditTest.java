...
class AgentRuntimeImplRuntimeAuditTest {
    ...
    void admitsAndFinishesDurableRuntimeRunForAppExecution() {
        ...
        assertEquals(com.shiyu.ai.agent.implementation.execution.ExecutionStatus.COMPLETED,
                service.execute(ACTOR, "agent-1", Map.of("__appId", "app-1", "message", "hello")).getStatus());
        verify(runtime).append(run, com.shiyu.ai.agent.contract.runtime.AiRunEventType.MODEL_STARTED, "{}", true);
