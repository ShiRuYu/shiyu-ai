...
import com.shiyu.ai.agent.implementation.service.IntentService.IntentRecognitionResult;
...
class IntentServiceImplTest {
    ...
    void recognizesSupportedSlotIntentAndBuildsModelRequest() {
        ...
        when(chat.chat(any(ChatRequest.class)))
                .thenReturn(
                        ChatResponse.builder()
                                .success(true)
                                .content(
                                        "```json"
                                            + " {\"intentCode\":\"WEATHER\",\"intentName\":\"Weather\",\"confidence\":\"0.95\",\"slots\":{\"city\":\"Beijing\"}}"
                                            + " ```")
                                .build());

        IntentRecognitionResult result =
                service.recognize(
                        "agent-1", "conversation", "weather in Beijing", "CUSTOM", "model-1");
    ...
    void rejectsUnsupportedLowConfidenceAndInvalidResponses() {
        ...
        when(chat.chat(any(ChatRequest.class)))
                .thenReturn(
                        ChatResponse.builder()
                                .success(true)
                                .content(
                                        "{\"intentCode\":\"UNKNOWN\",\"intentName\":\"x\",\"confidence\":0.9}")
                                .build());
        ...
        when(chat.chat(any(ChatRequest.class)))
                .thenReturn(
                        ChatResponse.builder()
                                .success(true)
                                .content(
                                        "{\"intentCode\":\"KNOWN\",\"intentName\":\"x\",\"confidence\":0.2}")
                                .build());
        ...
        when(chat.chat(any(ChatRequest.class)))
                .thenReturn(ChatResponse.builder().success(true).content("not json").build());
        ...
        assertTrue(
                service.recognize("agent-1", "missing", "hello")
                        .errorMessage()
                        .contains("model down"));
    }
    ...
    void mapsModelFailuresNullResponsesAndConfidenceVariants() {
        ...
        assertEquals(
                ...
                service.recognize("agent-1", "conversation", "hello").errorMessage());
        ...
        when(chat.chat(any(ChatRequest.class)))
                .thenReturn(
                        ChatResponse.builder()
                                .success(true)
                                .content(
                                        "{\"intentCode\":\"KNOWN\",\"intentName\":\"x\",\"confidence\":\"bad\",\"slots\":[]}")
                                .build());
        IntentRecognitionResult badConfidence =
                service.recognize("agent-1", "conversation", "hello");
        ...
        when(chat.chat(any(ChatRequest.class)))
                .thenReturn(
                        ChatResponse.builder()
                                .success(true)
                                .content(
                                        "{\"intentCode\":\"KNOWN\",\"intentName\":\"x\",\"confidence\":1}")
                                .build());
    ...
    void buildsSlotPromptWhenSchemaAndExamplesAreAbsent() throws Exception {
        ...
        when(chat.chat(any(ChatRequest.class)))
                .thenReturn(
                        ChatResponse.builder()
                                .success(true)
                                .content(
                                        "{\"intentCode\":\"SPARSE\",\"intentName\":\"Sparse\",\"confidence\":0.8}")
                                .build());
        ...
        IntentDefinition noExamples =
                IntentDefinition.builder()
                        ...
                        .build();
        Method buildPrompt =
                IntentServiceImpl.class.getDeclaredMethod(
        ...
        assertTrue(
                ((String) buildPrompt.invoke(service, "q", (Object) null))
