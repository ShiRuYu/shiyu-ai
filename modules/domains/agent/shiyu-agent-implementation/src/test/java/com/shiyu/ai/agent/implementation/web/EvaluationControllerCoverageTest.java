...
/**
 * 验证 Evaluation Controller Coverage 相关功能、边界条件、异常路径和协作行为。
 */
class EvaluationControllerCoverageTest {
    ...
    void mapsDatasetCaseRunAndResultEndpointsIncludingMissingOutput() {
        ...
        EvalCase testCase =
                new EvalCase("c1", "d1", 7, "input", "expected", Map.of(), Instant.now());
        ...
        when(service.addCase(eq("d1"), eq(tenant), eq(9L), eq("input"), eq("expected"), anyMap()))
                .thenReturn(testCase);
        when(service.cases(eq("d1"), eq(tenant), eq(9L))).thenReturn(List.of(testCase));
        when(service.run(eq("d1"), eq(tenant), eq(9L), eq("v1"), eq(EvalMetric.EXACT_MATCH), any()))
                .thenReturn(run);
        ...
        try (var ignored = mockStatic(ActorContextHttpAdapter.class)) {
            ...
            EvaluationController.DatasetRequest datasetRequest =
                    new EvaluationController.DatasetRequest();
            datasetRequest.setName("Demo");
            datasetRequest.setDescription("desc");
            ...
            caseRequest.setInput("input");
            caseRequest.setExpected("expected");
            caseRequest.setMetadata(Map.of());
            ...
            runRequest.setDatasetId("d1");
            runRequest.setAppVersionId("v1");
            runRequest.setMetric(EvalMetric.EXACT_MATCH);
