...
class AgentAdminServiceImplTest {
    ...
    private final AgentAdminServiceImpl service =
            new AgentAdminServiceImpl(repository, agentService);
    ...
    void pagesGetsCreatesUpdatesAndDeletesAgentsInTenantScope() {
        ...
        version.setAgentId("math");
        version.setVersionNumber("v1");
        when(repository.selectPage(ACTOR.tenantId(), 1, 10, "", 1))
                .thenReturn(Pair.of(1L, List.of(def)));
        when(repository.selectById(ACTOR.tenantId(), 1L))
                .thenReturn(def)
                .thenReturn(def)
                .thenReturn(null);
        when(repository.selectVersionsByAgentId(ACTOR.tenantId(), "math"))
        ...
        when(repository.create(eq(ACTOR.tenantId()), any(AgentDefBO.class)))
                .thenAnswer(invocation -> invocation.getArgument(1));
        when(repository.update(eq(ACTOR.tenantId()), any(AgentDefBO.class)))
                .thenAnswer(invocation -> invocation.getArgument(1));
        ...
        assertEquals("Renamed", service.update(ACTOR, 1L, request).getName());
    ...
    void rejectsDuplicateAndMissingAgentsAndExposesNodeMetadata() {
        AgentDefBO existing = new AgentDefBO();
        existing.setAgentId("math");
        when(repository.selectByAgentId(ACTOR.tenantId(), "math")).thenReturn(existing);
        AgentRequest request = new AgentRequest();
        request.setAgentId("math");
