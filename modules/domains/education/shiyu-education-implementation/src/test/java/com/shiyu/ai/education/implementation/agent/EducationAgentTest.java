...
class EducationAgentTest {
    ...
    void examAgentCollectsKnowledgePersistsAndSurfacesLlmFailure() {
        ...
        com.shiyu.ai.education.implementation.application.ExamService service =
                mock(com.shiyu.ai.education.implementation.application.ExamService.class);
        ...
        when(chat.chat(any()))
                .thenReturn(ChatResponse.builder().success(true).content("ok").build());
        doAnswer(
                        invocation -> {
                            ((ExamBO) invocation.getArgument(1)).setId(99L);
                            return 1;
                        })
                .when(repo)
                .insert(eq(new TenantId(1L)), any(ExamBO.class));
        ...
        assertThrows(
                ...
                () -> agent.generateExam(ACTOR, "MATH", 8, List.of(), 30, 7L));
    }
    ...
    void plannerAgentValidatesTargetAndDistributesDailyItems() {
        ...
        doAnswer(
                        invocation -> {
                            ((StudyPlanBO) invocation.getArgument(1)).setId(55L);
                            return 1;
                        })
                .when(plans)
                .insert(eq(new TenantId(1L)), any(StudyPlanBO.class));

        PlannerAgent agent = new PlannerAgent(chat, points, paths, plans, items);
        StudyPlanBO result =
                agent.generatePlan(
                        ACTOR, 2L, 10L, LocalDate.of(2026, 1, 1), LocalDate.of(2026, 1, 5));
