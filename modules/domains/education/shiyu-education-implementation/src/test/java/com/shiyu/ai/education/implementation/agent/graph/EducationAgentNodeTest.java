...
class EducationAgentNodeTest {
    ...
    private static final KnowledgeResponse KNOWLEDGE =
    ...
    void practiceNodeCoversValidationDifficultyAndJsonParsing() throws Exception {
        ...
        assertEquals("AI 出题失败，请稍后重试", failed.getMsg());
        ...
        when(engine.chat(any()))
                .thenReturn(
                        ChatResponse.builder()
                                .success(true)
                                .content("{\"type\":\"FILL\",\"title\":\"x\",\"answer\":\"A\"}")
                                .build());
        for (double score : new double[] {10.0, 45.0, 70.0, 90.0}) {
            NodeOutput threshold =
                    node.execute(
                            NodeInput.fromMap(
                                    Map.of(
                                            "knowledge",
    ...
    void teachNodeCoversMissingSuccessAndFailure() throws Exception {
        ...
        when(engine.chat(any()))
                .thenReturn(ChatResponse.builder().success(true).content("lesson").build());
        NodeOutput success =
                node.execute(
                        NodeInput.fromMap(
                                Map.of(
        ...
        when(engine.chat(any()))
                .thenReturn(
                        ChatResponse.builder().success(false).errorMessage("unavailable").build());
        NodeOutput failure =
                node.execute(
                        NodeInput.fromMap(
                                Map.of("knowledge", KNOWLEDGE, "tenantId", 1L, "userId", 2L)));
        assertFalse(failure.isSuccess());
        assertEquals("LLM 教学调用失败，请稍后重试", failure.getMsg());
        assertFalse(failure.getMsg().contains("unavailable"));
    ...
    void prereqNodeReturnsResultsAndDegradesOptionalLookups() throws Exception {
        ...
        when(path.findMissingPrerequisites(any(), eq(10L), any()))
                .thenThrow(new RuntimeException("path"));
        NodeOutput degraded =
                node.execute(
                        NodeInput.fromMap(
                                Map.of("knowledgeId", 10L, "__knowledgeAccessContext", actor)));
        ...
        assertThrows(
                ...
                () -> node.execute(NodeInput.fromMap(Map.of("knowledgeId", 10L))));
    }
    ...
    void scoreAnalysisCalculatesAnswersAndRequiresActorForMutation() throws Exception {
        ...
        NodeOutput defaults =
                node.execute(NodeInput.fromMap(Map.of("practiceQuestions", List.of(question))));
                ...
                node.execute(
                        NodeInput.fromMap(
                                Map.of(
                                        ...
                                        List.of(question),
                                        "answerResults",
                                        List.of(Map.of("correct", true), Map.of("correct", false)),
                                        "studentId",
                                        2L,
                                        "knowledgeId",
                                        10L,
    ...
    void reviewSchedulePersistsScheduledTasksAndValidatesInputs() throws Exception {
        ...
        TestReviewScheduleNode node =
                new TestReviewScheduleNode(
                        new ReviewScheduler(), mock(ReviewService.class), repository);
        ActorContext actor = new ActorContext(new TenantId(1L), new UserId(2L), false);
        NodeOutput missing =
                node.execute(NodeInput.fromMap(Map.of("__knowledgeAccessContext", actor)));
        assertFalse(missing.isSuccess());
        NodeOutput output =
                node.execute(
                        NodeInput.fromMap(
    ...
    void abilityQueryRequiresActorAndProjectsKnowledgePrerequisitesAndMastery() throws Exception {
        ...
        when(relations.getPrerequisites(actor, 10L)).thenReturn(List.of());
        when(abilities.get(actor, 2L, 10L))
                .thenReturn(new AbilityValue(2L, 10L, 100, 100, 100, 100, 100, 100, null));
        NodeOutput output =
                node.execute(
                        NodeInput.fromMap(
                                Map.of(
                                        ...
                                        actor)));
        assertTrue(output.isSuccess());
        assertEquals(100.0, output.getData("overallScore", 0.0));
        when(abilities.get(actor, 2L, 10L)).thenReturn(null);
        assertEquals(
                0.0,
                node.execute(
                                NodeInput.fromMap(
                                        Map.of(
                                                ...
                                                "__knowledgeAccessContext",
                                                actor)))
                        .getData("overallScore", -1.0));
        when(points.getResponse(actor, 10L)).thenReturn(null);
        assertFalse(
                node.execute(
                                NodeInput.fromMap(
                                        Map.of(
    ...
    private static final class TestPracticeNode extends PracticeNode {
        ...
        private NodeOutput execute(NodeInput input) throws Exception {
    ...
    private static final class TestAbilityQueryNode extends AbilityQueryNode {
        private TestAbilityQueryNode(
                ...
                KnowledgeRelationPort relations,
                AbilityService abilities) {
            super(points, relations, abilities);
        }
