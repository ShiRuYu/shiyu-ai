...
public class MagmaConsolidationWorker {
    ...
    private void consolidate(long jobId) {
        ...
        jdbc.query("SELECT ID,CONTENT FROM MEMORY_EVENT WHERE TENANT_ID=? AND NAMESPACE=? AND STATUS<>'REVOKED' AND ID<>? ORDER BY OCCURRED_AT DESC, ID ASC LIMIT 100",
                (rs, row) -> {
        ...
        Map<String, Object> attributes = JSONUtils.parseObject((String) event.get("ATTRIBUTES"), new TypeReference<Map<String, Object>>() { });
