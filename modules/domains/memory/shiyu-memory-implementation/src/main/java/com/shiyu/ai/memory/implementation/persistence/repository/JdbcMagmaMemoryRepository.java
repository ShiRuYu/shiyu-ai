...
public class JdbcMagmaMemoryRepository implements MagmaMemoryRepository {
    ...
    @Override public Optional<MemoryEvent> findPreviousEvent(TenantId tenantId,String ns,String st,String sid,Instant occurredAt) {
        return jdbc.query("SELECT * FROM MEMORY_EVENT WHERE TENANT_ID=? AND NAMESPACE=? AND SUBJECT_TYPE=? AND SUBJECT_ID=? AND STATUS IN ('ACTIVE','CANDIDATE') AND OCCURRED_AT<=? ORDER BY OCCURRED_AT DESC, CREATED_AT DESC, ID ASC LIMIT 1",this::mapEvent,tenantId.value(),ns,st,sid,ts(occurredAt)).stream().findFirst();
    }
    @Override public Optional<MemoryEvent> findNextEvent(TenantId tenantId,String ns,String st,String sid,Instant occurredAt) {
        return jdbc.query("SELECT * FROM MEMORY_EVENT WHERE TENANT_ID=? AND NAMESPACE=? AND SUBJECT_TYPE=? AND SUBJECT_ID=? AND STATUS IN ('ACTIVE','CANDIDATE') AND OCCURRED_AT>? ORDER BY OCCURRED_AT, CREATED_AT, ID ASC LIMIT 1",this::mapEvent,tenantId.value(),ns,st,sid,ts(occurredAt)).stream().findFirst();
    }
    public List<MemoryEvent> findCandidates(TenantId tenantId,String ns,String st,String sid,int limit) { return jdbc.query("SELECT * FROM MEMORY_EVENT WHERE TENANT_ID=? AND NAMESPACE=? AND SUBJECT_TYPE=? AND SUBJECT_ID=? AND STATUS IN ('ACTIVE','CANDIDATE') ORDER BY OCCURRED_AT DESC, ID ASC LIMIT ?",this::mapEvent,tenantId.value(),ns,st,sid,Math.min(Math.max(limit,1),200)); }
    @Override public List<MemoryEvent> findByNamespace(TenantId tenantId,String ns,int limit) { return jdbc.query("SELECT * FROM MEMORY_EVENT WHERE TENANT_ID=? AND NAMESPACE=? AND STATUS<>'REVOKED' ORDER BY OCCURRED_AT, ID ASC LIMIT ?",this::mapEvent,tenantId.value(),ns,Math.min(Math.max(limit,1),100000)); }
