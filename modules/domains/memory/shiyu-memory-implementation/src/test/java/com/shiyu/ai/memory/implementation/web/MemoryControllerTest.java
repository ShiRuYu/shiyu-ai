...
/**
 * 验证 记忆 Controller 相关功能、边界条件、异常路径和协作行为。
 */
class MemoryControllerTest {
    ...
    void handlesEventQueryAndLifecycleEndpointsWithSubjectIsolation() {
        ...
        request.setSubjectId("8");
        request.setEventType("STUDY");
        request.setContent("progress");
        request.setOccurredAt(event.occurredAt());
        request.setSourceType("conversation");
        ...
        query.setNamespace("magma");
        query.setSubjectType("USER");
        query.setSubjectId("8");
        query.setText("progress");
        when(memory.retrieveWithTrace(any(MemoryQuery.class)))
                .thenReturn(mock(MemoryRetrievalResult.class));
        assertNotNull(controller.query(query));
        when(memory.requireAccessibleEvent(TENANT, "event-1")).thenReturn(event);
        assertNotNull(controller.confirm("event-1"));
        assertNotNull(controller.revoke("event-1"));
        ...
        when(memory.supersede(eq(TENANT), eq("event-1"), any(IngestMemoryCommand.class)))
                .thenReturn(event);
        assertNotNull(controller.supersede("event-1", request));
        assertNotNull(controller.trace("trace-1"));
        controller.rebuild("magma");
        verify(index).rebuild(TENANT, "magma");
    }
    ...
    void rejectsCrossSubjectRequestsBeforeCallingMemory() {
        ...
        request.setNamespace("magma");
        request.setSubjectType("USER");
        request.setSubjectId("other-user");
