...
class MemoryValueInvariantTest {
    ...
    void normalizesOptionalCollectionsAndRejectsInvalidEntities() {
        MemoryEntity entity =
                new MemoryEntity("e1", new TenantId(7L), "USER", "u1", "User", null, null, true);
        assertEquals(Map.of(), entity.attributes());
        assertThrows(
                IllegalArgumentException.class,
                () -> new MemoryEntity("", new TenantId(7L), "USER", "u1", "", "", Map.of(), true));
        assertThrows(
                IllegalArgumentException.class,
                () -> new MemoryEntity("e1", new TenantId(7L), "", "u1", "", "", Map.of(), true));
        assertThrows(
                IllegalArgumentException.class,
                () -> new MemoryEntity("e1", new TenantId(7L), "USER", "", "", "", Map.of(), true));
    }
    ...
    void normalizesTraceAndResultDefaults() {
        ...
        MemoryRetrievalTrace trace =
                new MemoryRetrievalTrace(
                        ...
                        "q",
                        null,
                        Map.of(GraphType.ENTITY, 0.5d),
                        Arrays.asList(List.of("a"), null),
                        null,
                        null,
                        now);
        ...
        MemoryRetrievalTrace shorthand =
                new MemoryRetrievalTrace("t2", new TenantId(7L), "default", "q", List.of("e"), now);
