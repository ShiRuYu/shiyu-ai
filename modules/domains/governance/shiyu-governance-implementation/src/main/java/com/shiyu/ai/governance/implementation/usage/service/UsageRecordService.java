...
public class UsageRecordService implements UsageGovernance {
    ...
    public UsageRecordResult record(
            ActorContext actor, DomainEventEnvelope<UsageMeasurement> envelope) {
        ...
        record.setOccurredAt(
                LocalDateTime.ofInstant(envelope.occurredAt(), java.time.ZoneOffset.UTC));
                ...
                publishEmbeddingEvent(
                        ...
                        safeInt(
                                measurement
                                        ...
                                        .getOrDefault(
                                                "estimatedTokens",
                                                Long.toString(measurement.inputTokens()))),
    ...
    public void recordUsage(
        ...
        ModelPricing pricing =
                pricingMap.getOrDefault(
                        platform + ":" + model, new ModelPricing(platform, model, 0.0, 0.0));
        BillingPriceProvider.PriceSnapshot snapshot =
                billingPriceProvider == null ? null : billingPriceProvider.price(platform, model);
        double cost =
                snapshot == null
                        ? pricing.calculateCost(promptTokens, completionTokens)
                        : snapshot.promptPerToken()
                                .multiply(BigDecimal.valueOf(promptTokens))
                                .add(
                                        snapshot.completionPerToken()
        ...
        if (generationRunId != null && !generationRunId.isBlank())
            usage.put("generationRunId", generationRunId);
        ...
        record.setSourceType(
                generationRunId == null || generationRunId.isBlank()
                        ? "MODEL_CALL"
                        : "GENERATION_RUN");
        ...
        log.debug(
                "LLM 用量已保存: platform={}, model={}, tokens={}, cost={}, latency={}ms",
                platform,
                model,
                totalTokens,
                cost,
    ...
    public void recordEmbedding(
        ...
        UsageRecordBO record =
                buildRecord("EMBEDDING", latencyMs, tenantId, userId, sessionId, extInfo);
        ...
        usageRecordRepository.insert(record);
        log.debug(
                "Embedding 用量已保存: model={}, tokens≈{}, vectors={}, latency={}ms",
                model,
                estimatedTokens,
                vectorCount,
                latencyMs);
        ...
        if (realtimePublisher != null) {
            try {
                realtimePublisher.publishEmbeddingUsage(
                        model, textLength, estimatedTokens, vectorCount, latencyMs);
            } catch (Exception e) {
                log.warn(
                        "WebSocket 推送 Embedding 用量失败: errorType={}, errorMessageLength={}",
    ...
    private UsageRecordBO buildRecord(
            String usageType,
            long latencyMs,
            TenantId tenantId,
            Long userId,
    ...
    private void publishUsageEvent(
        ...
        if (realtimePublisher != null) {
            try {
                realtimePublisher.publishUsageRecord(
                        platform, model, promptTokens, completionTokens, latencyMs, cost);
            } catch (Exception e) {
                log.warn(
                        "WebSocket 推送用量失败: errorType={}, errorMessageLength={}",
                        e.getClass().getSimpleName(),
                        e.getMessage() == null ? 0 : e.getMessage().length());
            }
        }
    ...
    private void publishEmbeddingEvent(
