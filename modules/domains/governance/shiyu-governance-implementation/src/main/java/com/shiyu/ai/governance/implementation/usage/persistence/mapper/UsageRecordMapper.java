...
public interface UsageRecordMapper extends BaseMapperFlex<UsageRecordDO> {
    ...
    /**
     * Custom {@code @Select} statements do not use MyBatis-Flex's generated column mapping. Alias
     * every snake-case column explicitly so a {@link UsageRecordDO} retains its JSON metadata and
     * latency fields.
     */
    ...
    /**
     * H2 does not expose the MySQL JSON extraction functions used by the old aggregation queries.
     * The repository parses the JSON payloads after loading these bounded record sets instead.
     */
    @Select("SELECT " + RECORD_COLUMNS + " FROM governance_usage_record WHERE usage_type = 'LLM'")
    List<UsageRecordDO> selectLlmRecords();

    /**
     * Portable bounded read used by the Java aggregation adapter. Keeping the time boundary as a
     * bound value avoids vendor-specific DATEADD and FORMATDATETIME functions when the application
