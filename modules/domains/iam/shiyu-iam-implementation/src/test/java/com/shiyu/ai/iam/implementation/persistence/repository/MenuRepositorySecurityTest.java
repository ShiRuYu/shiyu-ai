...
/**
 * 验证 Menu Repository 安全 相关功能、边界条件、异常路径和协作行为。
 */
...
class MenuRepositorySecurityTest {
    ...
    void coversTenantScopedMenuQueriesCrudAndSubtreeDeletion() throws Exception {
        ...
        MenuDO root = menu(1L, null);
        MenuDO child = menu(2L, 1L);
        ...
        try (MockedStatic<MapstructUtils> conversions = mockStatic(MapstructUtils.class)) {
            conversions
                    .when(() -> MapstructUtils.convert(any(List.class), eq(MenuBO.class)))
                    .thenReturn(List.of(new MenuBO()));
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(MenuBO.class)))
                    .thenReturn(new MenuBO());
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(MenuDO.class)))
                    .thenReturn(new MenuDO());
            ...
            menu.setTenantId(21L);
            when(menus.insertSelective(any(MenuDO.class)))
                    .thenAnswer(
                            i -> {
                                ((MenuDO) i.getArgument(0)).setId(9L);
                                return 1;
                            });
            assertEquals(9L, repository.insert(menu).getId());
            ...
            assertEquals(
                    1L, repository.selectPage(TENANT, 2, 5, "Home", "home", "menu", 1).getLeft());
            ...
            assertEquals(1, repository.selectByParentId(TENANT, null).size());
            assertEquals(1, repository.selectByParentIdAndType(TENANT, null, "MENU").size());
            assertEquals(
                    1,
                    repository.selectMenusByUserId(TENANT, 8L, "member", false, "BUTTON").size());
            assertEquals(
                    1, repository.selectMenusByUserId(TENANT, 8L, "member", true, null).size());
        }
        ...
        assertThrows(
                ...
                () -> repository.selectMenusByUserId(null, 8L, null, false, null));
    }

    private static MenuDO menu(Long id, Long parent) {
        MenuDO value = new MenuDO();
        value.setId(id);
        value.setParentId(parent);
        value.setDelFlag(0);
        return value;
    }
