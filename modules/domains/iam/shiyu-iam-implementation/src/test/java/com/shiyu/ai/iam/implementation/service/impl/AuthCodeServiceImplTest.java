...
/**
 * 验证 认证 Code Service Impl 相关功能、边界条件、异常路径和协作行为。
 */
class AuthCodeServiceImplTest {
    private static final ActorContext ACTOR =
            new ActorContext(new TenantId(5), new UserId(9), false);
    ...
    private final AuthCodeServiceImpl service =
            new AuthCodeServiceImpl(repository, roleRepository, tenantRepository);
    ...
    void listsPagesCreatesUpdatesAndDeletesTenantCodes() {
        ...
        page.setPageSize(10);
        page.setCode("agent");
        assertEquals(1, service.list(ACTOR).size());
        assertEquals(1, service.page(ACTOR, page).getItems().size());
        AuthCodeRequest request = request("agent:write");
        AuthCodeBO saved = code(2L, "agent:write");
        when(repository.existsByCode("agent:write", null)).thenReturn(false);
        when(repository.insert(any())).thenReturn(saved);
        assertEquals(2L, service.create(ACTOR, request).getId());
        verify(repository).insertTenantCode(any(TenantAuthCodeBO.class));
        when(repository.selectById(1L)).thenReturn(code);
        when(repository.isAvailable(1L, ACTOR.tenantId())).thenReturn(true);
        when(repository.existsByCode("agent:update", 1L)).thenReturn(false);
        assertTrue(service.update(ACTOR, 1L, request("agent:update")));
        when(repository.hasRoleAssignments(1L)).thenReturn(false);
        when(repository.countActiveTenantLinks(1L)).thenReturn(0L);
        assertTrue(service.delete(ACTOR, 1L));
        verify(repository).deleteTenantCode(ACTOR.tenantId(), 1L);
    }
    ...
    void grantsReplacesRevokesOnlyWithinTenantRoleScope() {
        ...
        AuthCodeBO code = code(3L, "agent:read");
        when(repository.selectAvailableByIds(List.of(3L), ACTOR.tenantId()))
                .thenReturn(List.of(code));
        when(repository.selectByRoleIdAndTenantId(7L, ACTOR.tenantId())).thenReturn(List.of());
        assertTrue(service.grant(ACTOR, 7L, ACTOR.tenantId(), List.of(3L, 3L)));
        verify(repository).insertRoleAssignments(any());
        when(repository.selectByTenantId(ACTOR.tenantId())).thenReturn(List.of(code));
        assertTrue(
                service.replace(ACTOR, 7L, ACTOR.tenantId(), List.of("agent:read", "agent:read")));
        assertTrue(service.revoke(ACTOR, 7L, ACTOR.tenantId(), 3L));
        verify(repository).deleteRoleAssignments(7L, ACTOR.tenantId(), 3L);
        when(tenantRepository.selectDescendantIds(new TenantId(5L))).thenReturn(List.of());
    ...
    void rejectsInvalidScopeAndDuplicateCodes() {
        ...
        assertThrows(
                IllegalArgumentException.class, () -> service.create(ACTOR, request("agent:read")));
    ...
    void coversAuthCodeValidationPaginationAndIdempotentGrantBranches() {
        ...
        when(repository.selectByTenantId(ACTOR.tenantId()))
                .thenReturn(List.of(resourceCode, moduleOnly, nullCode));
        AuthCodePageRequest page = new AuthCodePageRequest();
        ...
        when(tenantRepository.selectById(5L)).thenReturn(tenant);
        when(tenantRepository.selectDescendantIds(new TenantId(5L))).thenReturn(List.of(5L));
        when(repository.selectByRoleIdAndTenantId(7L, ACTOR.tenantId()))
                .thenReturn(List.of(code(3L, "existing")));
        when(repository.selectAvailableByIds(List.of(3L, 4L), ACTOR.tenantId()))
                .thenReturn(List.of(code(3L, "existing")));
        ...
        assertThrows(
                ...
                () -> service.listRoleAuthCodes(ACTOR, 7L, ACTOR.tenantId()));
        TenantBO disabled = new TenantBO();
        disabled.setStatus(0);
        disabled.setDelFlag(0);
        when(roleRepository.isRoleOwnedByTenant(7L, new TenantId(5L))).thenReturn(true);
    ...
    private static AuthCodeBO code(Long id, String value) {
    ...
    private static AuthCodeRequest request(String value) {
