...
/**
 * 验证 角色 Repository 安全 相关功能、边界条件、异常路径和协作行为。
 */
...
class RoleRepositorySecurityTest {
    ...
    void coversRoleQueriesLifecycleAndTenantScopeChecks() throws Exception {
        ...
        try (MockedStatic<MapstructUtils> conversions = mockStatic(MapstructUtils.class)) {
            conversions
                    .when(() -> MapstructUtils.convert(any(List.class), eq(RoleBO.class)))
                    .thenReturn(List.of(new RoleBO()));
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(RoleBO.class)))
                    .thenReturn(new RoleBO());
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(RoleDO.class)))
                    .thenReturn(new RoleDO());
            ...
            assertNotNull(repository.selectById(2L, TENANT));
            assertNull(repository.selectById(null, TENANT));
            assertThrows(
                    IllegalArgumentException.class,
                    () -> repository.selectById(2L, new TenantId(0L)));
            RoleBO role = new RoleBO();
            role.setTenantId(21L);
            role.setName("operator");
            when(roles.insertSelective(any(RoleDO.class)))
                    .thenAnswer(
                            i -> {
                                ((RoleDO) i.getArgument(0)).setId(4L);
                                return 1;
                            });
            ...
            RoleBO zeroTenant = new RoleBO();
            zeroTenant.setTenantId(0L);
            assertThrows(IllegalArgumentException.class, () -> repository.insert(zeroTenant));
            RoleBO coded = new RoleBO();
            coded.setTenantId(21L);
            coded.setName("coded");
            RoleDO codedDo = new RoleDO();
            codedDo.setCode("existing");
            codedDo.setName("coded");
            ...
            RoleBO invalidRole = new RoleBO();
            invalidRole.setId(4L);
            assertFalse(repository.update(invalidRole));
            RoleBO zeroRole = new RoleBO();
            zeroRole.setId(4L);
            zeroRole.setTenantId(0L);
            assertFalse(repository.update(zeroRole));
            when(roles.deleteByQuery(any(QueryWrapper.class))).thenReturn(1);
            assertTrue(repository.deleteRoleAndRelations(4L, TENANT));
            assertFalse(repository.deleteRoleAndRelations(null, TENANT));
            assertThrows(
                    IllegalArgumentException.class,
                    () -> repository.deleteRoleAndRelations(4L, null));
            assertThrows(
                    IllegalArgumentException.class,
                    () -> repository.deleteRoleAndRelations(4L, new TenantId(0L)));
            assertTrue(repository.isRoleInScope(4L, TENANT));
            assertFalse(repository.isRoleInScope(null, TENANT));
            assertTrue(repository.isRoleOwnedByTenant(4L, TENANT));
            assertThrows(
                    IllegalArgumentException.class, () -> repository.isRoleOwnedByTenant(4L, null));
            assertTrue(repository.areMenusInTenantScope(null, List.of(21L)));
            assertTrue(repository.areMenusInTenantScope(List.<Long>of(), List.<Long>of()));
            assertFalse(repository.areMenusInTenantScope(List.of(1L), List.<Long>of()));
            assertFalse(repository.areMenusInTenantScope(List.of(1L), null));
            assertFalse(
                    repository.areMenusInTenantScope(
                            java.util.Arrays.asList(1L, null), List.of(21L)));
            ...
            RoleScopeMenuDO assignment = new RoleScopeMenuDO();
            assignment.setRoleId(4L);
            assignment.setMenuId(8L);
            when(roleMenus.selectListByQuery(any(QueryWrapper.class)))
                    .thenReturn(List.of(assignment));
            assertEquals(
                    Map.of(4L, List.of(8L)),
                    repository.selectMenuIdsByRoleIds(TENANT, List.of(4L)));
            when(roleMenus.selectListByQuery(any(QueryWrapper.class)))
                    .thenReturn(List.of(assignment));
            ...
            repository.insertRoleMenus(4L, TENANT, TENANT, List.of(8L));
            repository.insertRoleMenus(4L, TENANT, TENANT, List.of());
            assertThrows(
                    IllegalArgumentException.class,
