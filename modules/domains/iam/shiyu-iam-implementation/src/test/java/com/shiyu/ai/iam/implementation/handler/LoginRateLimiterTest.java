...
class LoginRateLimiterTest {
    ...
    @Test
    void usesRealIpWhenForwardedHeadersAreUnavailable() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        ...
        assertEquals("203.0.113.11", requestLimiter.getClientIp());
        requestLimiter.destroy();
    }
    ...
    void fallsBackAcrossEmptyProxyHeadersBeforeUsingRemoteAddress() {
        ...
        assertEquals(
                "198.51.100.9",
                new LoginRateLimiter(new HttpClientIpResolver(request)).getClientIp());
        limiter.destroy();
    }
    ...
    void cleansExpiredLocksAndWindowEntriesWithoutRemovingLiveEntries() throws Exception {
        ...
        Map<String, Object> attempts = (Map<String, Object>) attemptsField.get(limiter);
        Class<?> entryType =
                Class.forName(
                        "com.shiyu.ai.iam.implementation.handler.LoginRateLimiter$RateLimitEntry");
        ...
        @SuppressWarnings("unchecked")
        Queue<Long> stale = (Queue<Long>) queue.get(staleWindow);
        stale.add(1L);
        attempts.put("stale-window", staleWindow);
        Object live = entryConstructor.newInstance();
        @SuppressWarnings("unchecked")
        Queue<Long> liveAttempts = (Queue<Long>) queue.get(live);
