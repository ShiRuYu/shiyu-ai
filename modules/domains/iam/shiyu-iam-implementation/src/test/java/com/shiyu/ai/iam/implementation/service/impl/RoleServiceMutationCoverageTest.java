...
class RoleServiceMutationCoverageTest {
    ...
    void listsAndLoadsRolesOnlyWithinAssignableTenantScope() {
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(anyList(), eq(com.shiyu.ai.iam.implementation.vo.RoleVO.class)))
                    .thenAnswer(invocation -> ((List<?>) invocation.getArgument(0)).isEmpty()
                            ? List.of() : List.of(mock(com.shiyu.ai.iam.implementation.vo.RoleVO.class)));
            ...
            mapper.when(() -> MapstructUtils.convert(current, com.shiyu.ai.iam.implementation.vo.RoleVO.class))
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(anyList(), eq(com.shiyu.ai.iam.implementation.vo.RoleVO.class)))
                    .thenAnswer(invocation -> ((List<?>) invocation.getArgument(0)).stream()
                            .map(value -> mock(com.shiyu.ai.iam.implementation.vo.RoleVO.class)).toList());
