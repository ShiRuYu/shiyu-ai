...
class RoleServiceMutationCoverageTest {
    ...
    void createsAndUpdatesRoleAndPersistsValidatedMenuAssignments() {
        ...
        when(roles.insert(any(RoleBO.class)))
                .thenAnswer(
                        invocation -> {
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            ...
            verify(roles, times(2))
                    .insertRoleMenus(50L, new TenantId(9L), new TenantId(9L), List.of(1L, 2L));
        }
    }
    ...
    void rejectsInvalidCreateUpdateScopesAndMenuAssignments() {
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(missingTenant, RoleBO.class))
                    .thenReturn(role(null, null));
            assertFalse(service.createRole(ACTOR, missingTenant));
            when(tenants.selectById(9L)).thenReturn(inactiveTenant(9L));
            mapper.when(() -> MapstructUtils.convert(empty, RoleBO.class))
                    .thenReturn(role(null, 9L));
            ...
            mapper.when(() -> MapstructUtils.convert(invalidMenu, RoleBO.class))
                    .thenReturn(invalidRole);
            assertThrows(
                    IllegalArgumentException.class, () -> service.createRole(ACTOR, invalidMenu));

            mapper.when(() -> MapstructUtils.convert((RoleRequest) null, RoleBO.class))
                    .thenReturn(null);
            ...
            mapper.when(() -> MapstructUtils.convert(foreign, RoleBO.class))
                    .thenReturn(role(null, 10L));
    ...
    void listsAndLoadsRolesOnlyWithinAssignableTenantScope() {
        ...
        when(roles.selectAllByTenant("1", new TenantId(9L)))
                .thenReturn(List.of(current, malformed));
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(
                            () ->
                    ...
                    .thenAnswer(
                            invocation ->
                                    ((List<?>) invocation.getArgument(0)).isEmpty()
                                            ? List.of()
                                            : List.of(
                                                    mock(
                                                            com.shiyu.ai.iam.implementation.vo
                                                                    .RoleVO.class)));
            ...
            when(roles.selectMenuIdsByRoleId(1L, new TenantId(9L), new TenantId(9L)))
                    .thenReturn(List.of(4L));
            mapper.when(
                            () ->
                                    MapstructUtils.convert(
                                            current,
                                            com.shiyu.ai.iam.implementation.vo.RoleVO.class))
                    .thenReturn(mock(com.shiyu.ai.iam.implementation.vo.RoleVO.class));
            ...
            when(roles.selectAllByTenant(null, new TenantId(11L)))
                    .thenReturn(List.of(role(3L, 11L)));
    ...
    void handlesEmptyAssignmentsAndStopsBeforeMutationForForeignUsers() {
        ...
        verify(assignments, never())
                .deleteByUserIdRoleIdAndTenantId(anyLong(), anyLong(), any(TenantId.class));
    }
    ...
    void coversInactiveDeletedAndCrossTenantRoleScopeDecisions() {
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            ...
            ActorContext parentAdmin =
                    new ActorContext(
                            ...
                            "PARENT_SUPER_ADMIN",
                            false);
            when(roles.selectAllByTenant(null, new TenantId(10L)))
                    .thenReturn(List.of(role(2L, 10L)));
        ...
        verify(roles, never())
                .insertRoleMenus(
                        eq(20L), eq(new TenantId(9L)), eq(new TenantId(9L)), eq(List.of()));
    }
    ...
    void coversMutationFalseAndMissingRoleBranches() {
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            ...
            verify(roles, never())
                    .deleteRoleMenus(anyLong(), any(TenantId.class), any(TenantId.class));
        }
