...
class AuthorizationRepositoriesTest {
    ...
    void coversAuthorizationCodeTenantAndRoleAssignments() throws Exception {
        ...
        inject(repository, "authCodeMapper", codes);
        ...
        assertThrows(
                NullPointerException.class, () -> repository.selectAvailableByIds(List.of(), null));
        ...
        TenantAuthCodeDO link = new TenantAuthCodeDO();
        link.setAuthCodeId(4L);
        when(tenantCodes.selectListByQuery(any(QueryWrapper.class))).thenReturn(List.of(link));
        when(codes.selectListByQuery(any(QueryWrapper.class)))
                .thenReturn(List.of(new AuthCodeDO()));
        try (MockedStatic<MapstructUtils> conversions = mockStatic(MapstructUtils.class)) {
            conversions
                    .when(() -> MapstructUtils.convert(any(List.class), eq(AuthCodeBO.class)))
                    .thenReturn(List.of(new AuthCodeBO()));
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(AuthCodeBO.class)))
                    .thenReturn(new AuthCodeBO());
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(AuthCodeDO.class)))
                    .thenReturn(new AuthCodeDO());
            conversions
                    .when(
                            () ->
                                    MapstructUtils.convert(
                                            any(Object.class), eq(TenantAuthCodeDO.class)))
                    .thenReturn(new TenantAuthCodeDO());
            conversions
                    .when(
                            () ->
                                    MapstructUtils.convert(
                                            any(Object.class), eq(RoleScopeAuthCodeDO.class)))
                    .thenReturn(new RoleScopeAuthCodeDO());
            ...
            when(codes.selectOneById(4L)).thenReturn(new AuthCodeDO());
            assertNotNull(repository.selectById(4L));
            AuthCodeBO code = new AuthCodeBO();
            assertNotNull(repository.insert(code));
            repository.update(code);
            when(codes.selectCountByQuery(any(QueryWrapper.class))).thenReturn(1L, 0L);
            assertTrue(repository.existsByCode("read", null));
            assertFalse(repository.existsByCode("read", 2L));
            when(tenantCodes.selectCountByQuery(any(QueryWrapper.class))).thenReturn(1L, 0L);
            assertTrue(repository.isAvailable(4L, TENANT));
            assertEquals(0L, repository.countActiveTenantLinks(4L));
            when(roleCodes.selectCountByQuery(any(QueryWrapper.class))).thenReturn(1L);
            assertTrue(repository.hasRoleAssignments(4L));
            repository.insertTenantCode(new TenantAuthCodeBO());
            repository.deleteTenantCode(TENANT, 4L);
            repository.insertRoleAssignments(List.of(new RoleScopeAuthCodeBO()));
            repository.insertRoleAssignments(List.of());
            repository.deleteRoleAssignments(2L, TENANT, null);
            repository.deleteRoleAssignments(2L, TENANT, 4L);
        }
    }

    @Test
    void coversDictionaryTenantScopedCrudAndValidation() throws Exception {
        DictMapper mapper = mock(DictMapper.class);
        DictRepositoryImpl repository = new DictRepositoryImpl();
        inject(repository, "dictMapper", mapper);
        ...
        try (MockedStatic<MapstructUtils> conversions = mockStatic(MapstructUtils.class)) {
            conversions
                    .when(() -> MapstructUtils.convert(any(List.class), eq(DictBO.class)))
                    .thenReturn(List.of(new DictBO()));
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(DictBO.class)))
                    .thenReturn(new DictBO());
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(DictDO.class)))
                    .thenReturn(new DictDO());
            ...
            assertEquals(1, repository.selectByDictType(TENANT, "role").size());
            when(mapper.selectOneByQuery(any(QueryWrapper.class))).thenReturn(new DictDO());
            assertNotNull(repository.selectById(TENANT, 1L));
            DictBO dict = new DictBO();
            when(mapper.insertSelective(any(DictDO.class)))
                    .thenAnswer(
                            i -> {
                                ((DictDO) i.getArgument(0)).setId(8L);
                                return 1;
                            });
            assertEquals(8L, repository.create(dict).getId());
            repository.update(dict);
            assertNull(repository.update(null));
            dict.setId(null);
            assertNull(repository.update(dict));
            repository.deleteById(TENANT, 8L);
            repository.deleteByIds(TENANT, List.of(8L, 9L));
        }
