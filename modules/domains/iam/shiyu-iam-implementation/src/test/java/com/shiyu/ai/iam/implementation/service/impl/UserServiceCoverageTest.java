...
class UserServiceCoverageTest {
    ...
    void coversTenantScopedDetailsListsCreateUpdateAndDeletion() {
        ...
        UserServiceImpl service =
                new UserServiceImpl(users, roles, assignments, tenants, tenantRoles, menu);
        UserBO user = user(20L, "alice");
        RoleBO role = role(30L, 7L, "teacher");
        UserScopeRoleBO assignment = assignment(20L, 30L, 7L, 1, 0);
        user.setExtInfo("{\"currentRole\":{\"roleId\":30}}");
        when(users.isUserInScope(20L, new TenantId(7L))).thenReturn(true, true, true, false);
        ...
        vo.setId(20L);
        when(users.selectPage(eq(ACTOR.tenantId()), any(), any(), eq("ali")))
                .thenReturn(Pair.of(1L, List.of(user)));
        when(assignments.selectByUserIds(List.of(20L))).thenReturn(List.of(assignment));
        UserBO inserted = user(21L, "new");
        when(users.insert(any(UserBO.class))).thenReturn(inserted);
        when(users.selectById(21L)).thenReturn(inserted);
        when(users.update(any(UserBO.class))).thenReturn(true);
        when(users.deleteById(20L)).thenReturn(true);
        when(roles.isRoleOwnedByTenant(30L, new TenantId(7L))).thenReturn(true);
        UserRequest request = new UserRequest();
        request.setUsername("new");
        request.setPassword("secret");
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(any(UserBO.class), eq(UserVO.class)))
                    .thenReturn(vo);
            mapper.when(() -> MapstructUtils.convert(any(List.class), eq(UserVO.class)))
                    .thenReturn(List.of(vo));
            mapper.when(() -> MapstructUtils.convert(any(UserRequest.class), eq(UserBO.class)))
                    .thenReturn(user(21L, "new"));
            assertNotNull(service.detailView(ACTOR, 20L));
            assertEquals(1, service.getUserList(ACTOR, "ali", 1, 10).getTotal());
            assertNotNull(service.createUser(ACTOR, request, new Long[] {30L, 30L}, 7L));
            assertTrue(service.updateUser(ACTOR, 20L, request, null, 7L));
        }
        assertTrue(service.deleteUser(ACTOR, 20L));
        assertFalse(service.deleteUser(ACTOR, 20L));
        verify(users).deleteById(20L);
    }
    ...
    void coversPasswordAndTenantAssignmentGovernance() {
        ...
        UserServiceImpl service =
                new UserServiceImpl(users, roles, assignments, tenants, tenantRoles, menu);
        UserBO user = user(11L, "self");
        user.setPassword(PasswordUtils.encode("old"));
        when(users.isUserInScope(11L, new TenantId(7L))).thenReturn(true);
        when(users.selectById(11L)).thenReturn(user);
        when(users.update(any(UserBO.class))).thenReturn(true);
        assertFalse(service.changePassword(ACTOR, 12L, "old", "new"));
        assertFalse(service.changePassword(ACTOR, 11L, "bad", "new"));
        assertTrue(service.changePassword(ACTOR, 11L, "old", "new"));
        ...
        when(assignments.selectByUserId(11L)).thenReturn(List.of(assignment(11L, 30L, 7L, 1, 0)));
        when(tenantRoles.selectTenantById(new TenantId(7L))).thenReturn(tenant);
        when(tenantRoles.selectRoleById(30L)).thenReturn(role);
        when(roles.isRoleOwnedByTenant(30L, new TenantId(7L))).thenReturn(true);
        assertEquals(1, service.getTenantAssignments(ACTOR, 11L).size());
        UserTenantRoleRequest item = new UserTenantRoleRequest();
        item.setTenantId(7L);
        item.setRoleId(30L);
        assertTrue(service.replaceTenantAssignments(ACTOR, 11L, List.of(item, item)));
        verify(assignments).deleteByUserIdAndTenantId(11L, new TenantId(7L));
    ...
    void parentAdminAndInvalidScopesAreRejectedOrDelegatedSafely() {
        ...
        ActorContext parent =
                new ActorContext(
                        ...
                        "PARENT_SUPER_ADMIN",
                        true);
        UserBO user = user(20L, "child");
        user.setExtInfo("{}");
        RoleBO delegated = role(99L, 7L, "tenant_super");
        when(users.isUserInScope(20L, new TenantId(7L))).thenReturn(true);
        when(users.selectById(20L)).thenReturn(user);
        when(assignments.selectByUserId(20L)).thenReturn(List.of());
        when(tenantRoles.selectTenantSuperRole(new TenantId(7L))).thenReturn(delegated);
        UserVO vo = new UserVO();
        vo.setId(20L);
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(any(RoleBO.class), eq(RoleBO.class)))
                    .thenReturn(delegated);
            mapper.when(() -> MapstructUtils.convert(any(UserBO.class), eq(UserVO.class)))
                    .thenReturn(vo);
            assertNotNull(service.detailView(parent, 20L));
        }
    ...
    void preservesExistingFieldsOnPartialUpdateAndRejectsInvalidRoleScopes() {
        ...
        assertFalse(
                service.replaceTenantAssignments(ACTOR, 20L, List.of(new UserTenantRoleRequest())));
    ...
    void rejectsMissingContextsTenantsAndRoleOwnershipBeforeMutation() {
        ...
        UserServiceImpl service =
                new UserServiceImpl(
                        users, roles, assignments, tenants, tenantRoles, mock(MenuService.class));
        UserRequest request = new UserRequest();
        assertThrows(NullPointerException.class, () -> service.detailView(null, 1L));
        assertThrows(
                IllegalArgumentException.class,
                () -> service.createUser(ACTOR, request, null, null));
        assertThrows(
                IllegalArgumentException.class,
                () -> service.updateUser(ACTOR, 1L, request, null, 0L));
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(request, UserBO.class))
                    .thenReturn(user(null, "created"));
            assertThrows(
                    IllegalArgumentException.class,
    ...
    void coversUserDetailRoleFallbackAndManagementBoundaryBranches() {
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            ...
            when(users.insert(any(UserBO.class))).thenReturn(created);
            mapper.when(() -> MapstructUtils.convert(createRequest, UserBO.class))
                    .thenReturn(created);
        ...
        childAssignment.setTenantId(8L);
        childAssignment.setRoleId(31L);
    ...
    void coversDelegatedDetailFallbackAndSelectedRoleTrustBoundaries() {
        ...
        fallback.setExtInfo(
                "{\"switchMode\":\"PARENT_SUPER_ADMIN\",\"currentRole\":{\"roleId\":99,\"roleKey\":123}}");
        ...
        assigned.setExtInfo(
                "{\"switchMode\":\"PARENT_SUPER_ADMIN\",\"currentRole\":{\"roleId\":30,\"roleKey\":\"teacher\"}}");
    ...
    void preservesExplicitUpdateFieldsAndHandlesUpdateConflict() {
        ...
        patch.setNickName("New");
        patch.setGender("F");
        patch.setAvatar("avatar");
        patch.setAddress("address");
        patch.setEmail("new@example.test");
        patch.setPhone("123");
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(conflictRequest, UserBO.class))
                    .thenReturn(conflictPatch);
        ...
        completeExisting.setGender("F");
        completeExisting.setAvatar("avatar");
        completeExisting.setAddress("address");
        completeExisting.setEmail("mail@example.test");
        completeExisting.setPhone("123");
        completeExisting.setRemark("remark");
        completeExisting.setExtInfo("{}");
        UserBO sparsePatch = new UserBO();
        sparsePatch.setPassword(" ");
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(sparseRequest, UserBO.class))
                    .thenReturn(sparsePatch);
    ...
    void filtersInactiveAssignmentsAndResolvesOnlyCurrentTenantRole() {
        ...
        when(users.selectPage(eq(ACTOR.tenantId()), any(), any(), isNull()))
                .thenReturn(Pair.of(2L, List.of(first, noId)));
        ...
        when(assignments.selectByUserIds(List.of(50L)))
                .thenReturn(List.of(current, nullStatus, deleted, disabled, otherTenant));
        UserVO firstVo = new UserVO();
        firstVo.setId(50L);
        UserVO noIdVo = new UserVO();
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(any(List.class), eq(UserVO.class)))
                    .thenReturn(List.of(firstVo, noIdVo));
    ...
    void exercisesDelegatedRoleVariantsTenantAssignmentFiltersAndNullTargets() {
        ...
        when(assignments.selectByUserId(60L))
                .thenReturn(List.of(assignment(60L, 30L, 7L, 1, null)));
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(superRole, RoleBO.class))
                    .thenReturn(superRole);
            mapper.when(() -> MapstructUtils.convert(any(UserBO.class), eq(UserVO.class)))
                    .thenReturn(new UserVO());
        ...
        RoleBO disabledRole = role(32L, 7L, "disabled");
        disabledRole.setStatus(0);
        RoleBO roleWithoutStatus = role(31L, 7L, "unknown");
        roleWithoutStatus.setStatus(null);
        ...
        when(assignments.selectByUserId(70L))
                .thenReturn(
                        List.of(
        ...
        assertTrue(
                service.replaceTenantAssignments(
                        ACTOR, 80L, java.util.Arrays.asList(null, new UserTenantRoleRequest())));
    ...
    private static UserBO user(Long id, String username) {
    ...
    private static TenantBO tenant(Long id, Integer status, Integer delFlag) {
        TenantBO value = new TenantBO();
        value.setId(id);
        value.setStatus(status);
        value.setDelFlag(delFlag);
        value.setName("tenant-" + id);
        return value;
    }

    private static RoleBO role(Long id, Long tenant, String code) {
    ...
    private static UserScopeRoleBO assignment(
            Long user, Long role, Long tenant, Integer status, Integer delFlag) {
        UserScopeRoleBO value = new UserScopeRoleBO();
        value.setUserId(user);
        value.setRoleId(role);
        value.setTenantId(tenant);
        value.setStatus(status);
        value.setDelFlag(delFlag);
        return value;
