...
class TenantServiceImplTest {
    ...
    void scopesViewsAndPageToActorTenant() {
        ...
        tenant.setId(11L);
        tenant.setParentId(10L);
        tenant.setName("child");
        TenantVO view = mock(TenantVO.class);
        when(repository.selectAll()).thenReturn(List.of(tenant));
        when(repository.selectDescendantIds(new TenantId(10L))).thenReturn(List.of(11L));
        when(repository.selectPage(ACTOR.tenantId(), 1, 10, "", "", 1))
                .thenReturn(Pair.of(1L, List.of(tenant)));
    ...
    void enforcesTenantCreationUpdateAndDeletionRules() {
        ...
        assertThrows(
                ...
                () -> service.getTenantPage(null, 1, 10, "", "", 1));
    }
    ...
    void rejectsInvalidTargetTenantBeforeRepositoryAccess() {
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(childRequest, TenantBO.class))
                    .thenReturn(child);
            when(repository.existsByCode("child", null)).thenReturn(false);
