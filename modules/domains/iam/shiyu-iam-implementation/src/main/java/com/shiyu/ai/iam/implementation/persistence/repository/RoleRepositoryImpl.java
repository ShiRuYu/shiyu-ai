...
public class RoleRepositoryImpl
    ...
    /** 分页查询角色列表 */
    public Pair<Long, List<RoleBO>> selectPage(
            TenantId tenantId, Number pageNo, Number pageSize, String name) {
        requireTenant(tenantId);
        QueryWrapper countWrapper = new QueryWrapper();
        countWrapper.eq(RoleDO::getTenantId, tenantId.value()).and(RoleDO::getDelFlag).eq(0);
        ...
        queryWrapper.eq(RoleDO::getTenantId, tenantId.value()).and(RoleDO::getDelFlag).eq(0);
        if (name != null && !name.isEmpty()) {
    ...
    /** 查询指定租户的角色。调用方必须先校验目标租户属于当前操作租户的 自身或后代，本方法只负责按明确的 tenant_id 查询角色。 */
    public List<RoleBO> selectAllByTenant(String status, TenantId tenantId) {
        long tenantValue = requireTenant(tenantId);
        QueryWrapper queryWrapper =
                QueryWrapper.create().where(RoleDO::getTenantId).eq(tenantValue);
        if (status != null && !status.isEmpty()) {
            queryWrapper.eq(RoleDO::getStatus, status);
        }
        List<RoleDO> roleDOs =
    ...
    public boolean update(RoleBO roleBO) {
        if (roleBO == null
                || roleBO.getId() == null
                || roleBO.getTenantId() == null
                || roleBO.getTenantId() <= 0) {
            return false;
        }
        RoleDO roleDO = MapstructUtils.convert(roleBO, RoleDO.class);
    ...
    public boolean deleteRoleAndRelations(Long roleId, TenantId tenantId) {
        ...
        userScopeRoleMapper.deleteByQuery(
                QueryWrapper.create()
                        .where(UserScopeRoleDO::getRoleId)
                        .eq(roleId)
                        .and(UserScopeRoleDO::getTenantId)
                        .eq(tenantValue));
        roleScopeMenuMapper.deleteByQuery(
                QueryWrapper.create()
                        .where(RoleScopeMenuDO::getRoleId)
                        .eq(roleId)
                        .and(RoleScopeMenuDO::getTenantId)
                        .eq(tenantValue));
        roleScopeAuthCodeMapper.deleteByQuery(
                QueryWrapper.create()
                        .where(RoleScopeAuthCodeDO::getRoleId)
                        .eq(roleId)
    ...
    public boolean areMenusInTenantScope(List<Long> menuIds, List<Long> allowedTenantIds) {
        ...
        List<Long> distinctMenuIds =
                menuIds.stream().filter(java.util.Objects::nonNull).distinct().toList();
    ...
    public Map<Long, List<Long>> selectMenuIdsByRoleIds(TenantId tenantId, List<Long> roleIds) {
        ...
        QueryWrapper qw =
                QueryWrapper.create()
                        .select(
                                column(RoleScopeMenuDO::getRoleId),
                                column(RoleScopeMenuDO::getMenuId))
        ...
        return list.stream()
                .collect(
                        Collectors.groupingBy(
                                ...
                                Collectors.mapping(
                                        RoleScopeMenuDO::getMenuId, Collectors.toList())));
    }
    ...
    public void insertRoleMenus(
            Long roleId, TenantId roleTenantId, TenantId tenantId, List<Long> menuIds) {
        long tenantValue = requireTenant(tenantId);
        requireTenant(roleTenantId);
        if (!roleTenantId.equals(tenantId)) {
