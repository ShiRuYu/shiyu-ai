...
public class AuthCodeRepositoryImpl
    ...
    public List<AuthCodeBO> selectByTenantId(TenantId tenantId) {
        ...
        return MapstructUtils.convert(
                authCodeMapper.selectListByQuery(
                        QueryWrapper.create()
                                .in(AuthCodeDO::getId, ids)
                                .eq(AuthCodeDO::getStatus, 1)
                                .eq(AuthCodeDO::getDelFlag, 0)),
                AuthCodeBO.class);
    }
    ...
    public boolean existsByCode(String code, Long excludeId) {
        QueryWrapper query =
                QueryWrapper.create().eq(AuthCodeDO::getCode, code).eq(AuthCodeDO::getDelFlag, 0);
    ...
    public boolean isAvailable(Long authCodeId, TenantId tenantId) {
        return tenantAuthCodeMapper.selectCountByQuery(
                        QueryWrapper.create()
    ...
    public boolean hasRoleAssignments(Long authCodeId) {
        return roleScopeAuthCodeMapper.selectCountByQuery(
                        QueryWrapper.create()
    ...
    public void insertTenantCode(TenantAuthCodeBO assignment) {
        tenantAuthCodeMapper.insertSelective(
                MapstructUtils.convert(assignment, TenantAuthCodeDO.class));
    }
    ...
    public void insertRoleAssignments(List<RoleScopeAuthCodeBO> assignments) {
        ...
        roleScopeAuthCodeMapper.insertBatch(
                assignments.stream()
                        .map(item -> MapstructUtils.convert(item, RoleScopeAuthCodeDO.class))
                        .toList());
    }
