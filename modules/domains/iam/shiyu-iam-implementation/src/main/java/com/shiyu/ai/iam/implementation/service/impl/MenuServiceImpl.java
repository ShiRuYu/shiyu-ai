...
public class MenuServiceImpl implements MenuService {
    ...
    private List<MenuBO> getMenuPermissionsTree(ActorContext actor) {
        ...
        return getAllTree(actor);
    }

    private List<MenuBO> getMenuTree(ActorContext actor) {
        log.info("获取权限树 - 菜单");
        List<MenuBO> menus = menuRepository.selectAllByType(actor.tenantId(), "MENU");
        return buildMenuTree(menus);
    }

    private List<MenuBO> getAllTree(ActorContext actor) {
        log.info("获取权限树 all");
        List<MenuBO> allMenuBOs = menuRepository.selectAll(actor.tenantId());
    ...
    public boolean deleteMenu(ActorContext actor, Long id) {
        ...
        actor = requireActor(actor);
        if (menuRepository.selectById(actor.tenantId(), id) == null) {
            return false;
        }
        boolean result = menuRepository.deleteById(actor.tenantId(), id);
        if (result) {
    ...
    private boolean createMenu(ActorContext actor, MenuBO menuBO) {
        log.info("新增菜单");
        ...
        if (!isParentMenuInTenant(actor, menuBO.getParentId())) return false;
    ...
    private boolean updateMenu(ActorContext actor, Long id, MenuBO menuBO) {
        log.info("修改菜单，menuIdPresent: {}", id != null);

        MenuBO existingMenu = menuRepository.selectById(actor.tenantId(), id);
        ...
        normalizeMenuType(menuBO);
        Long currentTenantId = actor.tenantId().value();
        if (!isParentMenuInTenant(actor, menuBO.getParentId())) {
            return false;
        }
    ...
    private List<MenuBO> getMenuRoots(ActorContext actor) {
    ...
    private List<MenuBO> getChildrenByParentId(ActorContext actor, Long parentId) {
        log.info("获取子菜单，parentIdPresent: {}", parentId != null);
        return menuRepository.selectByParentId(actor.tenantId(), parentId);
    }
    ...
    public boolean isMenuPathExists(ActorContext actor, String path, Long id) {
        ...
        // SQL COUNT，替代原全表加载 + 内存过滤
        return menuRepository.existsByPath(requireActor(actor).tenantId(), path, id);
    }

    private List<MenuBO> getRouteMenusByUserId(ActorContext actor, Long userId) {
        log.info("获取用户路由菜单，userIdPresent: {}", userId != null);
        String cacheKey = buildRouteMenuCacheKey(actor, userId);

        // 1. 查缓存
        List<MenuBO> cached = routeMenuCache.getIfPresent(cacheKey);
        if (cached != null) {
            log.debug("路由菜单缓存命中，cacheKeyPresent: {}", cacheKey != null);
            return cached;
        }
        ...
        log.info("路由菜单已缓存，cacheKeyPresent: {}", cacheKey != null);
        return tree;
    }
    ...
    public void evictRouteMenuCache(Long userId) {
        if (userId != null) {
            ...
            log.debug("路由菜单缓存已清除，userIdPresent: {}", userId != null);
        }
    }
    ...
    private boolean isParentMenuInTenant(ActorContext actor, Long parentId) {
        if (parentId == null) {
            return true;
        }
        MenuBO parent = menuRepository.selectById(actor.tenantId(), parentId);
        return parent != null && actor.tenantId().value() == parent.getTenantId();
    }

    private ActorContext requireActor(ActorContext actor) {
        if (actor == null) throw new IllegalArgumentException("actor context is required");
        return actor;
    }
}
