...
public class AuthRepositoryImpl
     ...
     *
     * <p>JOIN: auth_user_scope_role → auth_role
     *
     * @param userId 用户ID
     ...
     *
     * <p>JOIN: auth_user_scope_role → auth_role → auth_role_scope_auth_code → auth_auth_code
     *
     * <p>仅返回用户在指定租户下、指定角色的权限码，避免跨角色越权。
     *
     * @param userId 用户ID
     * @param tenantId 当前租户ID
     ...
     * @return 权限码列表
     */
    public List<String> selectCodesByUserIdAndRoleCode(
            UserId userId, TenantId tenantId, String roleCode) {
        long tenantValue = requireTenant(tenantId);
        long userValue = requireUser(userId);
        ...
        QueryWrapper qw =
                QueryWrapper.create()
                        ...
                        .on(
                                column(AuthCodeDO::getId)
                                        .eq(column(RoleScopeAuthCodeDO::getAuthCodeId)))
                        ...
                        .eq(tenantValue)
                        .and(
                                column(RoleScopeAuthCodeDO::getTenantId)
                                        .eq(column(UserScopeRoleDO::getTenantId)))
                        .and(column(UserScopeRoleDO::getTenantId).eq(tenantValue))
                        .and(AuthCodeDO::getStatus)
                        .eq(1)
                        .and(AuthCodeDO::getDelFlag)
                        .eq(0)
                        .and(RoleScopeAuthCodeDO::getStatus)
                        .eq(1)
                        .and(RoleScopeAuthCodeDO::getDelFlag)
                        .eq(0)
                        .and(RoleDO::getStatus)
                        .eq(1)
                        .and(RoleDO::getDelFlag)
    ...
    public List<String> selectCodesByUsername(String username, TenantId tenantId) {
        ...
        QueryWrapper qw =
                QueryWrapper.create()
                        ...
                        .innerJoin(UserScopeRoleDO.class)
                        .on(
                                column(RoleScopeAuthCodeDO::getRoleId)
                                        .eq(column(UserScopeRoleDO::getRoleId)))
