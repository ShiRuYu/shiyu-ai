...
import com.shiyu.ai.iam.implementation.persistence.mapper.MenuMapper;
...
public class MenuRepositoryImpl
    ...
    public List<MenuBO> selectAllByType(TenantId tenantId, String type) {
        QueryWrapper queryWrapper = new QueryWrapper().where(MenuDO::getType).eq(type);
    ...
    public List<MenuBO> selectAllExcludingType(TenantId tenantId, String type) {
        QueryWrapper queryWrapper = new QueryWrapper().where(MenuDO::getType).ne(type);
        addMenuTenantFilter(queryWrapper, tenantId);
        List<MenuDO> menuDOs = menuMapper.selectListByQuery(queryWrapper);
        return MapstructUtils.convert(menuDOs, MenuBO.class);
    }
    ...
    public MenuBO selectById(TenantId tenantId, Long id) {
        QueryWrapper qw =
                QueryWrapper.create().where(MenuDO::getId).eq(id).and(MenuDO::getDelFlag).eq(0);
        addMenuTenantFilter(qw, tenantId);
    ...
    public boolean deleteById(TenantId tenantId, Long id) {
        ...
        roleScopeMenuMapper.deleteByQuery(
                QueryWrapper.create().where(RoleScopeMenuDO::getMenuId).in(ids));
        tenantMenuMapper.deleteByQuery(
                QueryWrapper.create().where(TenantMenuDO::getMenuId).in(ids));
        QueryWrapper deleteQuery = QueryWrapper.create().where(MenuDO::getId).in(ids);
        addMenuTenantFilter(deleteQuery, tenantId);
        menuMapper.deleteByQuery(deleteQuery);
        return true;
    }

    public Pair<Long, List<MenuBO>> selectPage(
            TenantId tenantId,
            Number pageNo,
            Number pageSize,
            String name,
        ...
        return Pair.of(
                total, MapstructUtils.convert(menuMapper.selectListByQuery(query), MenuBO.class));
    }

    private QueryWrapper buildManageQuery(
            TenantId tenantId, String name, String code, String type, Integer status) {
        QueryWrapper query = QueryWrapper.create().where(MenuDO::getDelFlag).eq(0);
    ...
    private void collectSubtreeIds(TenantId tenantId, Long parentId, List<Long> ids) {
        ...
        ids.add(parentId);
        QueryWrapper childrenQuery =
                QueryWrapper.create()
                        .where(MenuDO::getParentId)
                        .eq(parentId)
                        .and(MenuDO::getDelFlag)
                        .eq(0);
     ...
     *
     * <p>一次性 JOIN user_scope_role → role_scope_menu → menu， 替代原来的 查角色→遍历查菜单→内存过滤 流程
     *
     * @param userId 用户ID
     * @param excludeType 需要排除的菜单类型，传 null 则不过滤
     * @return 用户有权限的菜单列表（平铺，不含树结构）
     */
    public List<MenuBO> selectMenusByUserId(
            TenantId tenantId,
            Long userId,
            String roleCode,
        ...
        if (parentSuperAdminSwitch) {
            QueryWrapper delegated =
                    QueryWrapper.create()
                            ...
                            .where(RoleDO::getCode)
                            .eq(roleCode)
                            .and(RoleDO::getTenantId)
                            .eq(tenant)
                            .and(RoleScopeMenuDO::getTenantId)
                            .eq(tenant)
                            .and(MenuDO::getTenantId)
                            .eq(tenant)
        ...
        QueryWrapper qw =
                QueryWrapper.create()
                        ...
                        .eq(userId)
                        .and(
                                column(RoleScopeMenuDO::getTenantId)
                                        .eq(column(UserScopeRoleDO::getTenantId)))
                        .and(MenuDO::getStatus)
                        .eq(1)
                        .and(MenuDO::getDelFlag)
                        .eq(0);
    ...
    public boolean existsByName(TenantId tenantId, String name, Long excludeId) {
        QueryWrapper qw = new QueryWrapper().where(MenuDO::getName).eq(name);
        addMenuTenantFilter(qw, tenantId);
        if (excludeId != null) {
    ...
    public boolean existsByPath(TenantId tenantId, String path, Long excludeId) {
        QueryWrapper qw = new QueryWrapper().where(MenuDO::getPath).eq(path);
        addMenuTenantFilter(qw, tenantId);
        if (excludeId != null) {
            qw.and(MenuDO::getId).ne(excludeId);
        }
    ...
    /** 添加当前角色过滤：role.code = currentRoleCode 确保只查询当前角色有权限的菜单 */
    ...
    /** 添加菜单表租户过滤：menu.tenant_id = currentTenantId 确保查询的菜单属于当前用户可见的租户范围 */
    ...
    /** 添加用户租户过滤：user_scope_role.tenant_id = currentTenantId 确保只查询当前租户下分配的角色菜单 */
