...
public class RoleServiceImpl implements RoleService {
    ...
    public PageData<RoleVO> getRoleList(
        ...
        requireActor(actor);
        log.info(
                "获取角色列表，pageNum: {}, pageSize: {}, namePresent: {}",
                pageNum,
                pageSize,
                name != null);

        Pair<Long, List<RoleBO>> result =
                roleRepository.selectPage(actor.tenantId(), pageNum, pageSize, name);
        List<RoleBO> roleBOs = result.getRight();

        // 批量查询菜单权限（修复 N+1 问题）
        List<Long> roleIds = roleBOs.stream().map(RoleBO::getId).toList();
        java.util.Map<Long, List<Long>> roleMenusMap =
                roleRepository.selectMenuIdsByRoleIds(actor.tenantId(), roleIds);
        for (RoleBO roleBO : roleBOs) {
            roleBO.setPermissions(
                    roleMenusMap.getOrDefault(roleBO.getId(), java.util.Collections.emptyList()));
        }
    ...
    private List<RoleBO> getAllRoles(ActorContext actor, String status, TenantId tenantId) {
        log.info("获取所有角色，status: {}, tenantSelected: {}", status, tenantId != null);
        if (tenantId == null) {
    ...
    private RoleBO getRoleDetail(ActorContext actor, Long id, TenantId tenantId) {
        if (tenantId == null
                || !isAssignableTenantScope(actor, tenantId)
        ...
        List<Long> menuIds =
                roleRepository.selectMenuIdsByRoleId(
    ...
    private boolean updateRole(ActorContext actor, Long id, RoleBO roleBO) {
        log.info("修改角色，roleIdPresent: {}", id != null);
        if (roleBO == null || roleBO.getTenantId() == null) {
            log.warn("拒绝缺少租户的角色更新，roleIdPresent={}", id != null);
            return false;
        }
        ...
        if (success && roleBO.getPermissions() != null) {
            ...
            // 先删除旧的关联
            roleRepository.deleteRoleMenus(
                    id, new TenantId(targetTenantId), new TenantId(targetTenantId));
            // 再插入新的关联，授权关系归属于目标角色租户。
            roleRepository.insertRoleMenus(
                    id,
                    new TenantId(targetTenantId),
                    new TenantId(targetTenantId),
                    roleBO.getPermissions());
        }
    ...
    public boolean replaceRoleMenus(
            ActorContext actor, Long id, TenantId tenantId, List<Long> menuIds) {
        requireActor(actor);
    ...
    public boolean deleteRole(ActorContext actor, Long id) {
        log.info("删除角色，roleIdPresent: {}", id != null);
    ...
    public boolean removeUserRoles(
            ActorContext actor, Long roleId, TenantId tenantId, List<Long> userIds) {
        requireActor(actor);
        log.info(
                "取消分配角色，roleIdPresent: {}, tenantSelected: {}, userCount: {},"
        ...
        if (!roleRepository.isRoleOwnedByTenant(roleId, tenantId)) {
            log.warn(
                    "角色不属于当前租户作用域或已停用，roleIdPresent={}, currentTenantSelected={}",
                    roleId != null,
                    actor.tenantId() != null);
            return false;
        }
        ...
        for (Long userId : userIds) {
            if (!userRepository.isUserInScope(userId, actor.tenantId())) {
                log.warn(
                        "目标用户不属于当前租户作用域，userIdPresent={}, currentTenantSelected={}",
                        userId != null,
                        actor.tenantId() != null);
                return false;
            }
            userScopeRoleRepository.deleteByUserIdRoleIdAndTenantId(userId, roleId, tenantId);
    ...
    public boolean assignUserRoles(
        ...
        requireActor(actor);
        log.info(
                "分配角色，roleIdPresent: {}, tenantSelected: {}, userCount: {}, currentTenantSelected:"
                        + " {}",
                roleId != null,
                tenantId != null,
                userIds == null ? 0 : userIds.size(),
    ...
    private boolean areMenusAssignable(ActorContext actor, List<Long> menuIds) {
        ...
        return roleRepository.areMenusInTenantScope(
                menuIds == null ? List.of() : menuIds, List.of(currentTenantId));
    }
    ...
    private Long resolveAssignableTenantId(ActorContext actor, Long requestedTenantId) {
        ...
        return isAssignableTenantScope(actor, targetTenantId) ? targetTenantId.value() : null;
    }
