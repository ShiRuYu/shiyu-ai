...
public class RoleServiceImpl implements RoleService {
    ...
    public PageData<RoleVO> getRoleList(ActorContext actor, Number pageNum, Number pageSize, String name) {
        requireActor(actor);
        log.info("获取角色列表，pageNum: {}, pageSize: {}, namePresent: {}", pageNum, pageSize, name != null);
    ...
    public boolean removeUserRoles(ActorContext actor, Long roleId, TenantId tenantId, List<Long> userIds) {
        requireActor(actor);
        log.info("取消分配角色，roleIdPresent: {}, tenantSelected: {}, userCount: {}, currentTenantSelected: {}",
                roleId != null, tenantId != null, userIds == null ? 0 : userIds.size(), actor.tenantId() != null);
        ...
        if (!roleRepository.isRoleOwnedByTenant(roleId, tenantId)) {
            log.warn("角色不属于当前租户作用域或已停用，roleIdPresent={}, currentTenantSelected={}",
                    roleId != null, actor.tenantId() != null);
            return false;
        }
        if (!isAssignableTenantScope(actor, tenantId)) {
            return false;
        ...
        for (Long userId : userIds) {
            if (!userRepository.isUserInScope(userId, actor.tenantId())) {
                log.warn("目标用户不属于当前租户作用域，userIdPresent={}, currentTenantSelected={}",
                        userId != null, actor.tenantId() != null);
                return false;
            }
    ...
    public boolean assignUserRoles(ActorContext actor, Long roleId, TenantId tenantId, List<Long> userIds) {
        requireActor(actor);
        log.info("分配角色，roleIdPresent: {}, tenantSelected: {}, userCount: {}, currentTenantSelected: {}",
                roleId != null, tenantId != null, userIds == null ? 0 : userIds.size(), actor.tenantId() != null);
        if (userIds == null || userIds.isEmpty()) {
            return true;
        }
