...
public class UserRepositoryImpl
    ...
    /** 分页查询用户列表 */
    public Pair<Long, List<UserBO>> selectPage(
            TenantId tenantId, Number pageNo, Number pageSize, String username) {
        if (tenantId == null || tenantId.value() <= 0) {
            throw new IllegalArgumentException("tenantId is required for user pagination");
        }
        QueryWrapper countWrapper = new QueryWrapper();
        countWrapper
                .from(UserDO.class)
                .in(UserDO::getId, buildVisibleUserIdsQuery(tenantId.value()));
    ...
    /**
     * 使用用户 ID 子查询代替直接 join 用户角色关系表。 一个用户在当前租户下有多个角色时，join 会把同一用户展开成多行， 进而导致列表重复、总数错误以及分页被重复记录占用。
     */
    private QueryWrapper buildVisibleUserIdsQuery(Long currentTenantId) {
        return QueryWrapper.create()
                .select(column(UserScopeRoleDO::getUserId))
    ...
    public boolean deleteById(Long id) {
        userScopeRoleMapper.deleteByQuery(
                QueryWrapper.create().where(UserScopeRoleDO::getUserId).eq(id));
    ...
    /** 根据用户名查询活跃用户（不含角色关联信息，方法名避免误导） 如需角色信息请额外调用 selectRolesByUserId() */
    public UserBO selectActiveUserByUsername(String username) {
        QueryWrapper qw =
                QueryWrapper.create()
                        .from(UserDO.class)
                        .where(UserDO::getUsername)
                        .eq(username)
                        .and(UserDO::getStatus)
                        .eq(1)
                        .and(UserDO::getDelFlag)
                        .eq(0);
        UserDO userDO = userMapper.selectOneByQuery(qw);
        return MapstructUtils.convert(userDO, UserBO.class);
    }

    /** 校验用户是否属于当前租户作用域。 该校验用于角色分配等写操作，不能只依赖前端传入的 userId。 */
    public boolean isUserInScope(Long userId, TenantId currentTenantId) {
        if (userId == null || currentTenantId == null || currentTenantId.value() <= 0) {
            return false;
        }
        QueryWrapper qw = QueryWrapper.create().where(UserDO::getId).eq(userId);
        qw.and(UserDO::getStatus).eq(1).and(UserDO::getDelFlag).eq(0);
        QueryWrapper assignmentQuery =
                QueryWrapper.create()
                        .from(UserScopeRoleDO.class)
                        .where(UserScopeRoleDO::getUserId)
                        .eq(userId)
                        .and(UserScopeRoleDO::getTenantId)
                        .eq(currentTenantId.value())
                        .and(UserScopeRoleDO::getStatus)
                        .eq(1)
                        .and(UserScopeRoleDO::getDelFlag)
                        .eq(0);
        return userMapper.selectCountByQuery(qw) > 0
                && userScopeRoleMapper.selectCountByQuery(assignmentQuery) > 0;
    }
}
