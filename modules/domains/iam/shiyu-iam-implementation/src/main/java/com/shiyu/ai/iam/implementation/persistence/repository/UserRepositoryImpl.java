 ...
 */
@Component
public class UserRepositoryImpl implements com.shiyu.ai.iam.implementation.port.repository.UserRepository {
     ...
     * 分页查询用户列表
     */
    public Pair<Long, List<UserBO>> selectPage(TenantId tenantId, Number pageNo, Number pageSize, String username) {
    ...
    public boolean isUserInScope(Long userId, TenantId currentTenantId) {
        ...
        QueryWrapper assignmentQuery = QueryWrapper.create()
                .from(UserScopeRoleDO.class)
                .where(UserScopeRoleDO::getUserId).eq(userId)
                .and(UserScopeRoleDO::getTenantId).eq(currentTenantId.value())
                .and(UserScopeRoleDO::getStatus).eq(1)
                .and(UserScopeRoleDO::getDelFlag).eq(0);
