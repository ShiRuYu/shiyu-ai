...
/**
 * 验证 插件 Market Service 相关功能、边界条件、异常路径和协作行为。
 */
...
class PluginMarketServiceTest {
    ...
    void permitsUnsignedDevelopmentPublishAndPersistsChecksum() {
        ...
        when(provider.getIfAvailable(
                        org.mockito.Mockito.<java.util.function.Supplier<PluginMarketStore>>any()))
                .thenReturn(store);
    ...
    void rejectsUnsignedProductionAndChecksumMismatch() {
        ...
        assertThrows(
                ...
                () -> new PluginMarketService(provider, false).publish(entry, true));

        PluginMarketEntry wrongChecksum =
                new PluginMarketEntry(
                        "demo2",
                        "1.0.0",
                        "local",
        ...
        assertThrows(
                SecurityException.class,
                () -> new PluginMarketService(provider, true).publish(wrongChecksum, true));
        assertThrows(
    ...
    void verifiesSignedTrustedPublisherAndNormalizesEntryDefaults() throws Exception {
        ...
        signer.update(manifest.getBytes(java.nio.charset.StandardCharsets.UTF_8));
        ...
        try {
            ...
            when(provider.getIfAvailable(
                            org.mockito.Mockito
                                    .<java.util.function.Supplier<PluginMarketStore>>any()))
                    .thenReturn(store);
            ...
            if (old == null) System.clearProperty("shiyu.plugin.trusted-publishers");
            else System.setProperty("shiyu.plugin.trusted-publishers", old);
        }
    }
}
