...
class McpToolAutoConfigurationTest {
    ...
    void createsRegistryAndSynchronizesBuiltinToolsOnlyForImplementation() {
        ...
        implementation.init();
        McpToolAutoConfiguration.McpToolSyncRegistrar registrar =
                configuration.mcpToolSyncRegistrar(implementation, registry);
        registrar.sync();
        assertEquals(5, registry.size());

        McpToolRegistry untouched = configuration.mcpToolRegistry();
        registrar =
                configuration.mcpToolSyncRegistrar(
                        new ToolService() {
                            @Override
                            public ToolExecutionResult execute(
                                    String toolName, java.util.Map<String, Object> parameters) {
                                return new ToolExecutionResult(false, null, "not used");
                            }
                        },
                        untouched);
        registrar.sync();
