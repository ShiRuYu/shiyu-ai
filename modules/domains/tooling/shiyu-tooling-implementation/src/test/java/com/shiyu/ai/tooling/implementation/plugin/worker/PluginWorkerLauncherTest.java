...
class PluginWorkerLauncherTest {
    ...
    void rejectsMissingExecutableAndUnsafeArguments() {
        PluginWorkerSpec missing =
                new PluginWorkerSpec(
                        null, Set.of("C:/trusted"), Set.of(), Set.of(), Duration.ofSeconds(1));
        assertThrows(
                SecurityException.class, () -> PluginWorkerLauncher.launch(missing, List.of()));
        Path executable = javaExecutable();
        PluginWorkerSpec spec = spec(executable, Duration.ofSeconds(1));
        assertThrows(
                SecurityException.class,
                () -> PluginWorkerLauncher.launch(spec, List.of("..\\escape")));
        assertThrows(
                SecurityException.class,
                () -> PluginWorkerLauncher.launch(spec, java.util.Arrays.asList((String) null)));
    }
