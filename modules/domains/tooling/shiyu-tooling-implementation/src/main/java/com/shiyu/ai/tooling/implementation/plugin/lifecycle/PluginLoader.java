...
public class PluginLoader {
    ...
    public Plugin loadFromJar(Path jarPath, PluginDescriptor descriptor) {
        ...
        try {
            ...
            log.error(
                    "插件加载失败: jarPathLength={}, errorType={}, errorMessageLength={}",
                    valueLength(jarPath),
    ...
    public void loadFromDirectory(Path dir, PluginManager manager) {
        ...
        for (File jar : jars) {
            try {
                ...
                log.warn(
                        "跳过加载插件 JAR: nameLength={}, errorType={}, errorMessageLength={}",
                        valueLength(jar.getName()),
                        e.getClass().getSimpleName(),
                        valueLength(e.getMessage()));
            }
        }
    ...
    public void closeClassLoader(String pluginId) {
        ...
        if (cl != null) {
            try {
                ...
                log.warn(
                        "关闭插件 ClassLoader 失败: pluginIdLength={}, errorType={},"
                                + " errorMessageLength={}",
    ...
    private PluginDescriptor readDescriptorFromJar(Path jarPath) {
        ...
        return new PluginDescriptor(
                id, id, "1.0.0", "Auto-discovered plugin: " + id, "unknown", null, Map.of());
    }
