...
class KnowledgeRelationServiceImplTest {
    ...
    void listsDirectionalRelationsAndFiltersMissingTargets() {
        ...
        KnowledgeBO source = knowledge(1L, 7L, "source");
        KnowledgeBO target = knowledge(2L, 7L, "target");
        KnowledgeRelationBO pre = relation(1L, 2L, "PRE");
        when(knowledge.findById(ACTOR.tenantId(), 1L)).thenReturn(source);
        when(knowledge.findById(ACTOR.tenantId(), 2L)).thenReturn(target);
        when(knowledge.findById(ACTOR.tenantId(), 9L)).thenReturn(null);
        when(relations.findBySourceId(ACTOR.tenantId(), 7L, 1L)).thenReturn(List.of(pre));
        when(relations.findByTargetId(ACTOR.tenantId(), 7L, 1L)).thenReturn(List.of(pre));
        when(relations.findBySourceIdAndType(ACTOR.tenantId(), 7L, 1L, "PRE"))
                .thenReturn(List.of(pre));
        when(relations.findByTargetIdAndType(ACTOR.tenantId(), 7L, 1L, "PRE"))
                .thenReturn(List.of(relation(9L, 1L, "PRE")));
        when(relations.findBySourceIdAndType(ACTOR.tenantId(), 7L, 1L, "RELATED"))
                .thenReturn(List.of(relation(1L, 2L, "RELATED")));
        KnowledgeRelationServiceImpl service =
                new KnowledgeRelationServiceImpl(relations, knowledge, graph, spaces);
        assertEquals(1, service.list(ACTOR, 1L).size());
        assertEquals(1, service.getPrerequisites(ACTOR, 1L).size());
        assertTrue(service.getSubsequent(ACTOR, 1L).isEmpty());
        assertEquals(1, service.getRelated(ACTOR, 1L).size());
        verify(spaces, atLeast(4))
                .requireAccess(eq(7L), eq(KnowledgeSpaceService.SpaceRole.VIEWER), eq(ACTOR));
    }
    ...
    void addsRejectsCyclesAndRemovesRelationsWithGraphEdges() {
        ...
        when(knowledge.findById(ACTOR.tenantId(), 1L)).thenReturn(source);
        when(knowledge.findById(ACTOR.tenantId(), 2L)).thenReturn(target);
        when(relations.exists(any(), anyLong(), anyLong(), anyLong(), anyString()))
                .thenReturn(false);
        ...
        service.addRelation(ACTOR, 1L, 2L, RelationType.PRE, null);
        verify(relations).insert(eq(ACTOR.tenantId()), any());
        verify(graph).addEdge(eq(ACTOR.tenantId()), eq(1L), eq(2L), eq("PRE"), eq(1.0));
        when(relations.exists(any(), anyLong(), anyLong(), anyLong(), anyString()))
                .thenReturn(true);
        assertThrows(
                ServiceException.class,
                () -> service.addRelation(ACTOR, 1L, 2L, RelationType.RELATED, 0.5));
        assertThrows(
                ServiceException.class,
                () -> service.addRelation(ACTOR, 1L, 1L, RelationType.PRE, 1.0));
        when(relations.exists(any(), anyLong(), anyLong(), anyLong(), anyString()))
                .thenReturn(false);
        when(graph.findPath(ACTOR.tenantId(), 2L, 1L)).thenReturn(List.of(2L, 1L));
        assertThrows(
                ServiceException.class,
                () -> service.addRelation(ACTOR, 1L, 2L, RelationType.PRE, 1.0));
        when(knowledge.findById(ACTOR.tenantId(), 2L)).thenReturn(null);
        assertThrows(
                ServiceException.class,
                () -> service.addRelation(ACTOR, 1L, 2L, RelationType.RELATED, 1.0));
        ...
        when(relations.findBySourceId(ACTOR.tenantId(), 7L, 1L))
                .thenReturn(List.of(relation(1L, 2L, "PRE")));
        when(relations.findByTargetId(ACTOR.tenantId(), 7L, 1L))
                .thenReturn(List.of(relation(2L, 1L, "RELATED")));
        service.removeRelation(ACTOR, 1L, 2L, RelationType.PRE);
        service.removeAllRelations(ACTOR, 1L);
        verify(relations).deleteBySourceIdOrTargetId(ACTOR.tenantId(), 7L, 1L);
        verify(graph, atLeast(2)).removeEdge(any(), anyLong(), anyLong(), anyString());
    }
    ...
    private static KnowledgeBO knowledge(Long id, Long space, String name) {
