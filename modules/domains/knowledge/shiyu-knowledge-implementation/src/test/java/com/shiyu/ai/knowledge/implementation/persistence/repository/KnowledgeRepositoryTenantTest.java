...
/**
 * 验证 知识 Repository 租户 相关功能、边界条件、异常路径和协作行为。
 */
...
class KnowledgeRepositoryTenantTest {
    ...
    void scopesKnowledgeReadsWritesAndTenantMismatch() throws Exception {
        ...
        inject(repository, "knowledgeMapper", mapper);
        ...
        try (MockedStatic<MapstructUtils> conversions = mockStatic(MapstructUtils.class)) {
            conversions
                    ...
                    .thenReturn(List.of(new KnowledgeBO()));
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(KnowledgeBO.class)))
                    .thenReturn(new KnowledgeBO());
            conversions
                    .when(() -> MapstructUtils.convert(any(Object.class), eq(KnowledgeDO.class)))
                    .thenReturn(new KnowledgeDO());
            assertNotNull(repository.findById(TENANT, 4L));
            assertNotNull(repository.findByCode(TENANT, "kb"));
            assertEquals(1, repository.findAll(TENANT).size());
            assertEquals(1, repository.searchByName(TENANT, "math", 5).size());
            assertEquals(1, repository.page(TENANT, 0, 5, "math", "math").size());
            assertEquals(1L, repository.count(TENANT, "math", "math"));
            KnowledgeBO bo = new KnowledgeBO();
            bo.setTenantId(31L);
            when(mapper.insert(any(KnowledgeDO.class)))
                    .thenAnswer(
                            i -> {
                                ((KnowledgeDO) i.getArgument(0)).setId(9L);
                                return 1;
                            });
            assertEquals(1, repository.insert(TENANT, bo));
            when(mapper.updateByQuery(any(KnowledgeDO.class), any(QueryWrapper.class)))
                    .thenReturn(1);
            assertEquals(1, repository.update(TENANT, bo));
            assertTrue(repository.existsByCode(TENANT, "kb"));
            assertTrue(repository.existsBySpaceAndCode(TENANT, 2L, "kb"));
            assertEquals(1, repository.findBySpace(TENANT, 2L).size());
            when(mapper.deleteByQuery(any(QueryWrapper.class))).thenReturn(1);
            assertEquals(1, repository.deleteById(TENANT, 4L));
            assertEquals(1, repository.deleteByIdAndSpace(TENANT, 4L, 2L));
        ...
        KnowledgeBO wrong = new KnowledgeBO();
        wrong.setTenantId(99L);
