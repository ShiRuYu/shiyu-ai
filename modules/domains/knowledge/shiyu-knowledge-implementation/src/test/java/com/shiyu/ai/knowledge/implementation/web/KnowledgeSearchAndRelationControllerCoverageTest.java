...
class KnowledgeSearchAndRelationControllerCoverageTest {
    ...
    private final KnowledgeDocumentRelationService relations =
    ...
    void mapsSearchDefaultsExplicitOptionsAndRebuild() {
        ...
        KnowledgeSearchController.SearchRequest defaults =
                new KnowledgeSearchController.SearchRequest(3L, "query", null, null, null, null);
        KnowledgeSearchController.SearchRequest explicit =
                new KnowledgeSearchController.SearchRequest(3L, "query", "vector", 10, 0.5, true);
        assertTrue(controller.search(defaults, "1").isSuccess());
        assertTrue(controller.search(explicit, "v1").isSuccess());
        assertTrue(
                controller
                        .rebuild(new KnowledgeSearchController.RebuildRequest(3L), "1")
                        .isSuccess());
        verify(spaces, times(2))
                .requireAccess(anyLong(), eq(KnowledgeSpaceService.SpaceRole.VIEWER), any());
    ...
    void mapsDocumentRelationsAndRejectsVersion() {
        KnowledgeDocumentRelationController controller =
                new KnowledgeDocumentRelationController(relations);
        ...
        KnowledgeDocumentRelationController.ReplaceRequest docs =
                new KnowledgeDocumentRelationController.ReplaceRequest(List.of(11L), "RELATED");
        KnowledgeDocumentRelationController.ReplacePointsRequest points =
                new KnowledgeDocumentRelationController.ReplacePointsRequest(
                        List.of(5L), "RELATED");
        KnowledgeDocumentRelationController.ReplaceDocumentRelationsRequest all =
                new KnowledgeDocumentRelationController.ReplaceDocumentRelationsRequest(
                        List.of(
