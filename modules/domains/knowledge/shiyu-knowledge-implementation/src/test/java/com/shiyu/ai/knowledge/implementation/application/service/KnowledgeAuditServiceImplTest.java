...
class KnowledgeAuditServiceImplTest {
    private static final ActorContext ACTOR =
            new ActorContext(new TenantId(9), new UserId(6), false);
    ...
    void recordsTenantAuditAndPagesMappedResults() {
        ...
        when(repository.pageAudit(ACTOR.tenantId(), 1, 10, 4L))
                .thenReturn(new PageData<>(List.of(audit), 1));
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(any(List.class), any(Class.class)))
                    .thenReturn(List.of());
    ...
    void rejectsIncompleteActor() {
        assertThrows(
                ServiceException.class, () -> service.record(null, 1L, "SPACE", 1L, "READ", null));
