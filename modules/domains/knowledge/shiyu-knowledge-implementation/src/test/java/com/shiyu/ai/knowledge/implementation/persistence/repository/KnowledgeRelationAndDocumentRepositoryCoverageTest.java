...
class KnowledgeRelationAndDocumentRepositoryCoverageTest {
    ...
    void exercisesDocumentRepositoryQueriesMutationsAndTenantGuards() throws Exception {
        KnowledgeDocumentMapper documents =
                mock(KnowledgeDocumentMapper.class, mapperAnswer(KnowledgeDocumentDO.class));
        KnowledgeDocRelationMapper relations =
                mock(KnowledgeDocRelationMapper.class, mapperAnswer(KnowledgeDocRelationDO.class));
        ...
        try (MockedStatic<MapstructUtils> conversions = conversionMock()) {
            ...
            document.setTenantId(TENANT.value());
            document.setId(1L);
            ...
            when(documents.selectOneByQuery(any()))
                    .thenAnswer(
                            invocation -> {
            ...
            assertEquals(
                    1,
                    repository
                            .pageBySpace(TENANT, 2L, 1, 10, "math", "PUBLISHED", "READY")
                            .getTotal());
        ...
        KnowledgeDocumentBO wrong = new KnowledgeDocumentBO();
        wrong.setTenantId(999L);
        wrong.setId(1L);
    ...
    void exercisesChunkRelationAndDocumentRelationRepositories() throws Exception {
        KnowledgeChunkMapper chunks =
                mock(KnowledgeChunkMapper.class, mapperAnswer(KnowledgeChunkDO.class));
        ...
        KnowledgeChunkDO chunkData = new KnowledgeChunkDO();
        chunkData.setId(4L);
        chunkData.setSpaceId(null);
        ...
        KnowledgeRelationMapper relationMapper =
                mock(KnowledgeRelationMapper.class, mapperAnswer(KnowledgeRelationDO.class));
        ...
        try (MockedStatic<MapstructUtils> conversions = conversionMock()) {
            ...
            assertEquals(
                    1, relationRepository.findBySourceIdAndType(TENANT, 2L, 1L, "RELATED").size());
            assertEquals(
                    1, relationRepository.findByTargetIdAndType(TENANT, 2L, 1L, "RELATED").size());
            assertEquals(1, relationRepository.insert(TENANT, relation));
            assertEquals(
                    1,
                    relationRepository.deleteBySourceAndTargetAndType(
                            TENANT, 2L, 1L, 2L, "RELATED"));
        ...
        KnowledgeDocRelationMapper docRelationMapper =
                mock(KnowledgeDocRelationMapper.class, mapperAnswer(KnowledgeDocRelationDO.class));
        KnowledgeDocRelationRepositoryImpl docRelationRepository =
                new KnowledgeDocRelationRepositoryImpl();
        ...
        assertThrows(
                IllegalArgumentException.class, () -> relationRepository.findBySpace(null, 2L));
    }
    ...
    private static Object newValue(Class<?> type) throws Exception {
        ...
        try {
            type.getMethod("setId", Long.class).invoke(value, 1L);
    ...
    private static MockedStatic<MapstructUtils> conversionMock() {
        ...
        conversions
                ...
                .thenAnswer(
                        invocation ->
                                convertValue(invocation.getArgument(0), invocation.getArgument(1)));
        return conversions;
    }
    ...
    private static Object convertValue(Object source, Class<?> type) throws Exception {
        ...
        if (value instanceof KnowledgeDocumentBO document) {
            document.setId(1L);
            document.setSpaceId(2L);
            document.setTitle("math");
            document.setContent("math content");
        }
        return value;
    }
