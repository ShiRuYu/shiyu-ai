...
class EmbeddedIngestionWorkerTest {
    ...
    void bindsJobTenantAndReleasesCapacityWhenInitialLookupFails() throws Exception {
        ...
        EmbeddedIngestionWorker worker =
                worker(
                        enterprise,
        ...
        when(enterprise.findJob(new TenantId(10L), 17L))
                .thenAnswer(
                        invocation -> {
        ...
        try {
            Exception failure =
                    assertThrows(
                            Exception.class, () -> invokeExecute(worker, new TenantId(10L), 17L));
        ...
        when(enterprise.findJob(new TenantId(10L), 18L))
                .thenAnswer(
                        invocation -> {
                            assertEquals(new TenantId(10L), TenantScope.require());
                            return null;
    ...
    void initializesRecoversStaleJobsAndPollsPendingJobs() throws Exception {
        ...
        doAnswer(
                ...
                .when(enterprise)
                .updateJob(new TenantId(10L), stale);
        worker.initialize();
        assertTrue(TenantScope.current().isEmpty());
    ...
    void executesSuccessfulJobWithTenantScopedDependencies() throws Exception {
        ...
        when(parser.parse(any(byte[].class)))
                .thenReturn(new DocumentParser.ParseResult("parsed", "hello", ""));
        when(storage.open("obj"))
                .thenReturn(
                        new ObjectStorage.ReadableObject(
        ...
        verify(ingestion)
                .ingest(
                        any(com.shiyu.ai.kernel.context.ActorContext.class),
                        eq(7L),
                        eq(8L),
                        eq(9L),
                        eq("hello"),
    ...
    void retriesStorageFailureThenMarksFinalFailureAndHonorsCancellation() throws Exception {
        ...
        assertEquals("文档处理失败，请稍后重试", retry.getErrorMessage());
        assertFalse(retry.getErrorMessage().contains("gone"));
        retry.setAttempts(3);
    ...
    void handlesBlankParserOutputAndCancellationAfterEmbedding() throws Exception {
        ...
        when(parser.parse(any(byte[].class)))
                .thenReturn(new DocumentParser.ParseResult(null, "  ", ""));
        when(storage.open("obj"))
                .thenReturn(
                        new ObjectStorage.ReadableObject(
        ...
        doAnswer(
                ...
                .ingest(
                        ...
                        anyList());
        when(parser.parse(any(byte[].class)))
                .thenReturn(new DocumentParser.ParseResult("title", "text", ""));
    ...
    void coversPollingAndExecutionBoundaryBranches() throws Exception {
        ...
        Set<Long> ids = (Set<Long>) inFlight.get(worker);
        ids.add(500L);
        worker.poll();
        ...
        when(storage.open("obj"))
                .thenReturn(
                        new ObjectStorage.ReadableObject(
                                ...
                                "text/plain",
                                3));
        EmbeddedIngestionWorker actorWorker =
                worker(
                        enterprise,
                        documents,
                        ingestion,
        ...
        when(parser.parse(any(byte[].class)))
                .thenReturn(new DocumentParser.ParseResult("Parsed title", "text", ""));
    ...
    void coversWorkerUtilityAndDuplicateClaimBranches() throws Exception {
        ...
        Method jobTenant =
                EmbeddedIngestionWorker.class.getDeclaredMethod(
                        "jobTenant", KnowledgeIngestionJobBO.class);
        ...
        Method parserFor =
                EmbeddedIngestionWorker.class.getDeclaredMethod("parserFor", String.class);
        parserFor.setAccessible(true);
        assertTrue(
                ((java.util.Optional<?>) parserFor.invoke(worker, new Object[] {null})).isEmpty());
        Method markRunning =
                EmbeddedIngestionWorker.class.getDeclaredMethod(
    ...
    private static EmbeddedIngestionWorker worker(
            KnowledgeEnterpriseRepository enterprise,
        ...
        return new EmbeddedIngestionWorker(
                enterprise, documents, ingestion, storage, scanner, parsers, scheduler, pools);
    }

    private static void invokeExecute(EmbeddedIngestionWorker worker, TenantId tenant, Long id)
            throws Exception {
        Method method =
                EmbeddedIngestionWorker.class.getDeclaredMethod(
                        "execute", TenantId.class, Long.class);
        method.setAccessible(true);
        method.invoke(worker, tenant, id);
    }

    private static Object invoke(
            EmbeddedIngestionWorker worker, String name, Class<?>[] types, Object... args)
            throws Exception {
    ...
    private static KnowledgeIngestionJobBO job(
            Long id,
            Long space,
            Long document,
            Long version,
            ...
            int attempts,
            int max) {
        KnowledgeIngestionJobBO value = new KnowledgeIngestionJobBO();
        value.setId(id);
        value.setTenantId(10L);
        value.setSpaceId(space);
        value.setDocumentId(document);
        value.setVersionId(version);
        value.setActorUserId(20L);
        value.setJobStatus(status);
        value.setAttempts(attempts);
        value.setMaxAttempts(max);
        return value;
    }

    private static KnowledgeDocumentBO document(Long id, String type) {
        KnowledgeDocumentBO value = new KnowledgeDocumentBO();
        value.setId(id);
        value.setTenantId(10L);
        value.setSpaceId(7L);
        value.setDocType(type);
        return value;
    }

    private static KnowledgeDocumentVersionBO version(Long id, String key) {
        KnowledgeDocumentVersionBO value = new KnowledgeDocumentVersionBO();
        value.setId(id);
        value.setTenantId(10L);
        value.setObjectKey(key);
        return value;
    }
}
