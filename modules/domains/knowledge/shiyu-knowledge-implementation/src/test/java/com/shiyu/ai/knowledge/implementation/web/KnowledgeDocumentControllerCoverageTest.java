...
class KnowledgeDocumentControllerCoverageTest {
    ...
    void rejectsInvalidVersionAndEmptyOrUnreadableUploads() throws Exception {
        ...
        ServiceException uploadFailure = assertThrows(ServiceException.class,
                () -> controller.upload(3L, broken, null, "1"));
        assertEquals("读取上传文件失败", uploadFailure.getMessage());
        when(documents.get(any(), eq(11L))).thenReturn(document);
        when(storage.open("docs/11")).thenThrow(new java.io.IOException("storage down"));
        ServiceException previewFailure = assertThrows(ServiceException.class,
                () -> controller.preview(11L, "1"));
