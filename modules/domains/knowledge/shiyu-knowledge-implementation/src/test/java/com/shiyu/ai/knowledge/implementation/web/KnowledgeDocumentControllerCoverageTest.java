...
class KnowledgeDocumentControllerCoverageTest {
    ...
    private final KnowledgeDocumentUploadService uploads =
            mock(KnowledgeDocumentUploadService.class);
    ...
    private final KnowledgeDocumentController controller =
            new KnowledgeDocumentController(documents, uploads, storage, resumable);
    private final EnterpriseDocumentService.DocumentView document =
            new EnterpriseDocumentService.DocumentView(
                    11L,
                    3L,
                    21L,
                    "Guide",
                    ...
                    LocalDateTime.now(),
                    LocalDateTime.now());
    private final EnterpriseDocumentService.UploadResult uploadResult =
            new EnterpriseDocumentService.UploadResult(document, 21L, 31L, false);
    ...
    void mapsDocumentLifecycleAndUploadEndpoints() throws Exception {
        MockMultipartFile file =
                new MockMultipartFile("file", "guide.pdf", "application/pdf", "data".getBytes());
        when(documents.page(any(), eq(3L), eq(1), eq(100), any(), any(), any())).thenReturn(null);
        when(uploads.upload(
                        any(), eq(3L), eq("Guide"), eq("guide.pdf"), eq("application/pdf"), any()))
                .thenReturn(uploadResult);
        when(uploads.importUrl(any(), eq(3L), eq("Guide"), eq("https://example.test/guide.pdf")))
                .thenReturn(uploadResult);
        ...
        assertTrue(
                controller
                        .importUrl(
                                3L,
                                new KnowledgeDocumentController.ImportUrlRequest(
                                        "https://example.test/guide.pdf", "Guide"),
                                "1")
                        .isSuccess());
    ...
    void mapsResumableUploadEndpointsAndPreview() throws Exception {
        ResumableUploadService.BeginRequest begin =
                new ResumableUploadService.BeginRequest(
                        "guide.pdf", "application/pdf", 4L, null, "Guide");
        ResumableUploadService.UploadSession session =
                new ResumableUploadService.UploadSession(
                        "session-1",
                        3L,
                        "guide.pdf",
                        4L,
                        1,
                        List.of(),
                        ResumableUploadService.CHUNK_SIZE);
        ...
        when(resumable.complete(any(), eq("session-1")))
                .thenReturn(new ResumableUploadHandler.RegistrationResult(uploadResult, false));
        when(documents.get(any(), eq(11L))).thenReturn(document);
        assertTrue(controller.beginUpload(3L, begin, "1").isSuccess());
        assertTrue(controller.uploadStatus("session-1", "1").isSuccess());
        MockMultipartFile chunk =
                new MockMultipartFile(
                        "file", "chunk", "application/octet-stream", "data".getBytes());
        ...
        ObjectStorage.ReadableObject readable =
    ...
    void rejectsInvalidVersionAndEmptyOrUnreadableUploads() throws Exception {
        assertThrows(
                RuntimeException.class, () -> controller.page(3L, 1, 20, null, null, null, "2"));
        ...
        ServiceException uploadFailure =
                assertThrows(
                        ServiceException.class, () -> controller.upload(3L, broken, null, "1"));
        assertEquals("读取上传文件失败", uploadFailure.getMessage());
        when(documents.get(any(), eq(11L))).thenReturn(document);
        when(storage.open("docs/11")).thenThrow(new java.io.IOException("storage down"));
        ServiceException previewFailure =
                assertThrows(ServiceException.class, () -> controller.preview(11L, "1"));
        assertEquals("文件预览失败", previewFailure.getMessage());
    }
}
