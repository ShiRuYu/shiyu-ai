...
class KnowledgePathServiceImplTest {
    ...
    private final KnowledgePathServiceImpl service =
            new KnowledgePathServiceImpl(graph, repository, spaces);

    @Test
    void generatesFindsAndFiltersPathsByTenantAndSpace() {
        KnowledgeBO from = point(1L, 10L);
        KnowledgeBO to = point(2L, 10L);
        when(repository.findById(ACTOR.tenantId(), 1L)).thenReturn(from);
        when(repository.findById(ACTOR.tenantId(), 2L)).thenReturn(to);
        ...
        when(graph.findMissingPrerequisites(ACTOR.tenantId(), 2L, Set.of(1L)))
                .thenReturn(List.of());
    ...
    void rejectsMissingPointsAndCrossSpacePaths() {
        ...
        when(repository.findById(ACTOR.tenantId(), 1L)).thenReturn(point(1L, 10L));
        when(repository.findById(ACTOR.tenantId(), 2L)).thenReturn(point(2L, 11L));
    ...
    private static KnowledgeBO point(Long id, Long spaceId) {
