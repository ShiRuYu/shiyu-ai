...
class KnowledgeDocumentRelationServiceImplTest {
    ...
    void listsAndReplacesPointDocumentRelationsWithTenantAndSpaceChecks() {
        ...
        KnowledgeDocumentRelationRepository documentRelations =
                mock(KnowledgeDocumentRelationRepository.class);
        ...
        KnowledgeDocumentRelationServiceImpl service =
                new KnowledgeDocumentRelationServiceImpl(
    ...
    void rejectsMissingActorCrossSpaceAndInvalidRelationTypes() {
        ...
        assertThrows(
                ...
                () -> service.replaceDocuments(ACTOR, 7L, List.of(11L), "invalid"));
        assertThrows(
                ServiceException.class,
                () ->
                        service.replaceDocumentRelations(
                                ACTOR,
                                11L,
                                List.of(
                                        new KnowledgeDocumentRelationService
                                                .DocumentRelationRequest(11L, "RELATED_TO"))));
        assertThrows(
                ServiceException.class,
                () ->
                        service.replaceDocumentRelations(
                                ...
                                List.of(
                                        new KnowledgeDocumentRelationService
                                                .DocumentRelationRequest(12L, "RELATED_TO"))));
        assertThrows(
                ServiceException.class,
                () ->
                        service.replaceDocumentRelations(
                                ...
                                List.of(
                                        new KnowledgeDocumentRelationService
                                                .DocumentRelationRequest(12L, "INVALID"))));
    ...
    void listsAndReplacesDocumentRelationsAndRemovesBothRelationKinds() {
        ...
        record.setId(9L);
        record.setTargetDocumentId(12L);
        record.setSourceDocumentId(11L);
        record.setRelationType("REFERENCES");
        when(documentRelations.selectBySource(ACTOR.tenantId(), 3L, 11L))
        ...
        service.replaceDocumentRelations(
                ACTOR,
                11L,
                List.of(new KnowledgeDocumentRelationService.DocumentRelationRequest(12L, null)));
        verify(documentRelations)
                .replace(
                        eq(ACTOR.tenantId()),
                        eq(3L),
                        eq(11L),
    ...
    void handlesEmptyListsNullSpacesAndFiltersForeignRelationRows() {
        ...
        assertEquals(
                List.of("matching"),
                service.listDocuments(ACTOR, 7L).stream()
                        .map(KnowledgeDocumentRelationService.DocumentSummary::title)
                        .toList());
        ...
        assertThrows(
                ServiceException.class, () -> service.replaceDocumentRelations(ACTOR, 20L, null));

        when(documents.selectById(ACTOR.tenantId(), 12L)).thenReturn(foreign);
        assertThrows(
                ServiceException.class,
                () -> service.replaceDocuments(ACTOR, 7L, List.of(12L), "RELATED"));
        KnowledgeBO foreignPoint = point(8L, 4L);
        when(points.findById(ACTOR.tenantId(), 8L)).thenReturn(foreignPoint);
        assertThrows(
                ServiceException.class,
                () -> service.replacePoints(ACTOR, 11L, List.of(8L), "RELATED"));
        ...
        when(documentRelations.selectBySource(ACTOR.tenantId(), 3L, 11L))
                .thenReturn(List.of(missingTarget));
        ...
        service.replaceDocumentRelations(
                ...
                11L,
                List.of(
                        new KnowledgeDocumentRelationService.DocumentRelationRequest(
                                12L, "REFERENCES")));
        verify(documentRelations, atLeastOnce())
                .replace(eq(ACTOR.tenantId()), eq(3L), eq(11L), anyList());
        assertThrows(
                ServiceException.class,
                () ->
                        service.replaceDocumentRelations(
