...
/**
 * {@code KnowledgeDocumentRelationRepositoryImpl} 实现知识模块的持久化端口，负责在领域对象与存储模型之间转换。
 */
@Component
public class KnowledgeDocumentRelationRepositoryImpl
        implements com.shiyu.ai.knowledge.implementation.domain.port.repository
                .KnowledgeDocumentRelationRepository {
    /**
     * 映射器，表示当前对象中的对应属性。
     */
    @Resource private KnowledgeDocumentRelationMapper mapper;

    public List<KnowledgeDocumentRelationBO> selectBySource(
    ...
    public List<KnowledgeDocumentRelationBO> selectByTarget(
            TenantId tenantId, Long spaceId, Long documentId) {
        requireTenant(tenantId);
        return convert(
                QueryWrapper.create()
    ...
    public void replace(
        ...
        if (!relations.isEmpty()) {
            ...
            mapper.insertBatch(
                    MapstructUtils.convert(relations, KnowledgeDocumentRelationDO.class));
        }
    }
    ...
    private List<KnowledgeDocumentRelationBO> convert(QueryWrapper wrapper) {
        return MapstructUtils.convert(
                mapper.selectListByQuery(wrapper), KnowledgeDocumentRelationBO.class);
    }
