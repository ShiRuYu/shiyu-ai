...
/**
 * 验证 模型 Manager 相关功能、边界条件、异常路径和协作行为。
 */
class ModelManagerTest {
    ...
    @Test
    void skipsPlatformWithMissingCodeWithoutDiscardingValidDatabaseAdapters() {
        AiPlatformRepository platforms = mock(AiPlatformRepository.class);
        AiModelRepository models = mock(AiModelRepository.class);
        PlatformProperties props = new PlatformProperties();
        props.setTenantId(9L);
        ...
        valid.setName("有效平台");
        valid.setBaseUrl("http://valid");
        AiPlatformBO missingCode = new AiPlatformBO();
        missingCode.setId(11L);
        missingCode.setName("缺少编码");
        missingCode.setBaseUrl("http://broken");

        when(platforms.selectAllEnabled(new TenantId(9L))).thenReturn(List.of(valid, missingCode));
