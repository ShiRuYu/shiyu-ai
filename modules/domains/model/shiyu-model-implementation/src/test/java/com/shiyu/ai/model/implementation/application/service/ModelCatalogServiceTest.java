...
import com.shiyu.ai.model.implementation.application.service.impl.AiModelServiceImpl;
import com.shiyu.ai.model.implementation.application.service.impl.AiPlatformServiceImpl;
...
/**
 * 验证 模型 Catalog Service 相关功能、边界条件、异常路径和协作行为。
 */
class ModelCatalogServiceTest {
    private static final ActorContext ACTOR =
            new ActorContext(new TenantId(81), new UserId(82), false);
    ...
    void platformServiceScopesQueriesAndHandlesDefaultsAndApiKeys() throws Exception {
        ...
        AiPlatformServiceImpl service =
                inject(new AiPlatformServiceImpl(), "aiPlatformRepository", repository);
        inject(service, "platformProperties", properties);

        AiPlatformBO platform = new AiPlatformBO();
        platform.setId(1L);
        platform.setName("DeepSeek");
        platform.setCode("DEEPSEEK");
        platform.setIsDefault("Y");
        AiPlatformResponse response = new AiPlatformResponse();
        response.setCode("DEEPSEEK");
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(
                            () ->
                                    MapstructUtils.convert(
                                            any(AiPlatformBO.class), eq(AiPlatformResponse.class)))
                    .thenReturn(response);
            mapper.when(
                            () ->
                                    MapstructUtils.convert(
                                            any(AiPlatformRequest.class), eq(AiPlatformBO.class)))
                    .thenReturn(platform);
            when(repository.selectPage(ACTOR.tenantId(), 1, 10, "deep", "deepseek"))
                    .thenReturn(Pair.of(1L, List.of(platform)));
            ...
            assertEquals(
                    "DEEPSEEK",
                    service.pageResponse(ACTOR, 1, 10, "deep", "deepseek")
                            .getRight()
                            .get(0)
                            .getCode());
            ...
            ArgumentCaptor<AiPlatformBO> createdPlatform =
                    ArgumentCaptor.forClass(AiPlatformBO.class);
            ...
            IllegalArgumentException missingPlatform =
                    assertThrows(
                            IllegalArgumentException.class,
                            () -> service.setDefaultResponse(ACTOR, 99L));
            assertEquals("平台不存在: 99", missingPlatform.getMessage());
        }
    }

    @Test
    void platformUpdatePreservesStoredApiKeyWhenRequestOmitsIt() throws Exception {
        AiPlatformRepository repository = mock(AiPlatformRepository.class);
        AiPlatformServiceImpl service =
                inject(new AiPlatformServiceImpl(), "aiPlatformRepository", repository);
        inject(service, "platformProperties", new PlatformProperties());

        AiPlatformBO mappedRequest = new AiPlatformBO();
        mappedRequest.setName("DeepSeek");
        mappedRequest.setCode("DEEPSEEK");
        mappedRequest.setAdapterType("OPENAI_COMPATIBLE");
        mappedRequest.setApiKey("");
        AiPlatformBO existing = new AiPlatformBO();
        existing.setId(1L);
        existing.setCode("DEEPSEEK");
        existing.setApiKey("stored-secret");
        AiPlatformResponse response = new AiPlatformResponse();

        when(repository.selectById(ACTOR.tenantId(), 1L)).thenReturn(existing);
        when(repository.update(eq(ACTOR.tenantId()), any(AiPlatformBO.class)))
        ...
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            ...
            service.updateResponse(ACTOR, 1L, request);
        }

        ArgumentCaptor<AiPlatformBO> updated = ArgumentCaptor.forClass(AiPlatformBO.class);
        verify(repository).update(eq(ACTOR.tenantId()), updated.capture());
        assertEquals("stored-secret", updated.getValue().getApiKey());
    }
    ...
    void modelServiceFillsPlatformAndClearsDefaultOnCreateAndUpdate() throws Exception {
        ...
        AiModelServiceImpl service =
                inject(new AiModelServiceImpl(), "aiModelRepository", repository);
        ...
        model.setId(3L);
        model.setPlatformId(2L);
        model.setModelName("gpt");
        model.setIsDefault("Y");
        AiModelResponse response = new AiModelResponse();
        response.setPlatformName("OpenAI");
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(
                            () ->
                                    MapstructUtils.convert(
                                            any(AiModelBO.class), eq(AiModelResponse.class)))
                    .thenReturn(response);
            mapper.when(
                            () ->
                                    MapstructUtils.convert(
                                            any(AiModelRequest.class), eq(AiModelBO.class)))
                    .thenReturn(model);
            when(platforms.selectById(ACTOR.tenantId(), 2L)).thenReturn(platform);
            when(repository.selectPage(ACTOR.tenantId(), 2L, 1, 10))
                    .thenReturn(Pair.of(1L, List.of(model)));
            ...
            assertEquals(
                    "OpenAI",
                    service.pageResponse(ACTOR, 2L, 1, 10).getRight().get(0).getPlatformName());
            ...
            service.deleteByIds(ACTOR, List.of(3L));
            service.getOptions(ACTOR, 2L);
            verify(repository, atLeastOnce())
                    .clearDefaultExcept(eq(ACTOR.tenantId()), eq(2L), any());
            when(repository.selectById(ACTOR.tenantId(), 99L)).thenReturn(null);
            IllegalArgumentException missingModel =
                    assertThrows(
                            IllegalArgumentException.class,
                            () -> service.setDefaultResponse(ACTOR, 99L));
            assertEquals("模型不存在: 99", missingModel.getMessage());
