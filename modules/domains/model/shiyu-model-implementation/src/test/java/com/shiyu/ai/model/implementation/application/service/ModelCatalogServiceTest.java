...
class ModelCatalogServiceTest {
    ...
    void platformServiceScopesQueriesAndHandlesDefaultsAndApiKeys() throws Exception {
        ...
        IllegalArgumentException missingPlatform = assertThrows(IllegalArgumentException.class,
    ...
    void platformUpdatePreservesStoredApiKeyWhenRequestOmitsIt() throws Exception {
        ...
        when(repository.update(eq(ACTOR.tenantId()), any(AiPlatformBO.class))).thenAnswer(invocation -> invocation.getArgument(1));
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(() -> MapstructUtils.convert(any(AiPlatformRequest.class), eq(AiPlatformBO.class)))
                    .thenReturn(mappedRequest);
            mapper.when(() -> MapstructUtils.convert(any(AiPlatformBO.class), eq(AiPlatformResponse.class)))
                    .thenReturn(response);
