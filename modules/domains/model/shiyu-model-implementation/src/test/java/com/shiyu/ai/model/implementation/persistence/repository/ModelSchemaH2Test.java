    ...
    void modelTablesSupportCrudAndTenantFiltering() throws Exception {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ScriptUtils.executeSqlScript(
                    connection, new ClassPathResource("db/baseline/h2/schema/model/04_model.sql"));
            try (Statement statement = connection.createStatement()) {
                statement.executeUpdate(
                        "INSERT INTO MODEL_AI_PLATFORM (ID,NAME,CODE,TENANT_ID) "
                                + "VALUES (1,'Tenant One','ONE',1)");
                statement.executeUpdate(
                        "INSERT INTO MODEL_AI_PLATFORM (ID,NAME,CODE,TENANT_ID) "
                                + "VALUES (2,'Tenant Two','TWO',2)");
                statement.executeUpdate(
                        "INSERT INTO MODEL_AI_MODEL (ID,PLATFORM_ID,MODEL_NAME,TENANT_ID) "
