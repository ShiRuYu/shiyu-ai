...
class DeepSeekHttpProviderTest {
    ...
    void emitsUsageBeforeCompletionWhenProviderSendsUsageAfterFinishFrame() {
        server.createContext("/v1/chat/completions", exchange -> {
            String response = "data: {\"id\":\"req-after-finish\",\"choices\":[{\"delta\":{\"content\":\"ok\"},\"finish_reason\":null}]}\n\n"
                    + "data: {\"id\":\"req-after-finish\",\"choices\":[{\"delta\":{},\"finish_reason\":\"stop\"}]}\n\n"
                    + "data: {\"id\":\"req-after-finish\",\"usage\":{\"prompt_tokens\":8,\"completion_tokens\":2,\"total_tokens\":10},\"choices\":[]}\n\n"
                    + "data: [DONE]\n\n";
            byte[] bytes = response.getBytes(StandardCharsets.UTF_8);
            exchange.getResponseHeaders().add("Content-Type", "text/event-stream");
            exchange.sendResponseHeaders(200, bytes.length);
    ...
    void estimatesMissingUsageAndMapsProviderFailuresWithoutLeakingRawErrors() {
        ...
        server.createContext("/v1/chat/completions", exchange -> {
            byte[] errorBody = "{\"error\":{\"message\":\"secret-api-key-should-not-escape\"}}".getBytes(StandardCharsets.UTF_8);
        ...
        DeepSeekHttpProvider.DeepSeekProviderException failure = assertThrows(
                DeepSeekHttpProvider.DeepSeekProviderException.class, () -> provider().chat(request()));
