...
import com.shiyu.ai.model.implementation.infrastructure.chat.ModelStreamEvent;
...
/**
 * 验证 模型 Controllers Coverage 相关功能、边界条件、异常路径和协作行为。
 */
class ModelControllersCoverageTest {
    ...
    void platformControllerCoversQueriesAndMutations() {
        ...
        response.setId(1L);
        response.setName("OpenAI");
        when(service.pageResponse(actor, 1, 10, "", "OPENAI"))
                .thenReturn(Pair.of(1L, List.of(response)));
        ...
        try (var ignored = mockStatic(ActorContextHttpAdapter.class);
            ...
            mapping.when(() -> MapstructUtils.convert(anyList(), eq(AiPlatformVO.class)))
                    .thenReturn(List.of());
            mapping.when(
                            () ->
                                    MapstructUtils.convert(
                                            any(AiPlatformResponse.class), eq(AiPlatformVO.class)))
                    .thenReturn(null);
    ...
    void platformControllerMapsNotFoundAndFailures() {
        ...
        when(service.updateResponse(any(), anyLong(), any()))
    ...
    void modelControllerCoversQueriesAndMutations() {
        ...
        try (var ignored = mockStatic(ActorContextHttpAdapter.class);
                var mapping = mockStatic(MapstructUtils.class)) {
            ignored.when(ActorContextHttpAdapter::currentActor).thenReturn(actor);
            mapping.when(() -> MapstructUtils.convert(anyList(), eq(AiModelVO.class)))
                    .thenReturn(List.of());
            mapping.when(
                            () ->
                                    MapstructUtils.convert(
                                            any(AiModelResponse.class), eq(AiModelVO.class)))
                    .thenReturn(null);
    ...
    void gatewayAndMediaControllersMapTenantRoutesAndProviderOperations() {
        ...
        when(provider.understandImage(any(), any(), any()))
                .thenReturn(new MediaProvider.VisionResult("seen", List.of("label")));
        when(provider.generateImage(anyString(), any()))
                .thenReturn(new MediaProvider.ImageResult("key", "image/png", 10, 20));
        MediaController media = new MediaController(new MediaProviderRegistry(List.of(provider)));
        MediaController.TtsRequest tts = new MediaController.TtsRequest();
        tts.text = "hello";
        assertTrue(media.tts(tts).isSuccess());
        MediaController.TranslateRequest translate = new MediaController.TranslateRequest();
        translate.text = "hello";
        assertTrue(media.translate(translate).isSuccess());
        MediaController.ImageRequest image = new MediaController.ImageRequest();
        image.imageBase64 = "aGk=";
        assertTrue(media.understand(image).isSuccess());
        MediaController.GenerateRequest generate = new MediaController.GenerateRequest();
        generate.prompt = "cat";
