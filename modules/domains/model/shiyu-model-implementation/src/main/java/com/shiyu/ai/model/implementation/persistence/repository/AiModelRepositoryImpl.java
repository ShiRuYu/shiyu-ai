...
public class AiModelRepositoryImpl
    ...
    public List<AiModelBO> selectByPlatformId(TenantId tenantId, Long platformId) {
        QueryWrapper queryWrapper = new QueryWrapper();
        queryWrapper
                .eq(AiModelDO::getTenantId, tenantId.value())
                .eq(AiModelDO::getPlatformId, platformId);
    ...
    public AiModelBO selectById(TenantId tenantId, Long id) {
        AiModelDO modelDO =
                aiModelMapper.selectOneByQuery(
                        new QueryWrapper()
                                .eq(AiModelDO::getTenantId, tenantId.value())
    ...
    public AiModelBO update(TenantId tenantId, AiModelBO bo) {
        AiModelDO modelDO = MapstructUtils.convert(bo, AiModelDO.class);
        modelDO.setTenantId(tenantId.value());
        aiModelMapper.updateByQuery(
                modelDO,
                new QueryWrapper()
                        .eq(AiModelDO::getTenantId, tenantId.value())
                        .eq(AiModelDO::getId, bo.getId()));
        return bo;
    }

    public void deleteById(TenantId tenantId, Long id) {
        aiModelMapper.deleteByQuery(
                new QueryWrapper()
                        .eq(AiModelDO::getTenantId, tenantId.value())
                        .eq(AiModelDO::getId, id));
    }
