...
/**
 * 验证 会话 Domain Invariant 相关功能、边界条件、异常路径和协作行为。
 */
class ConversationDomainInvariantTest {
    ...
    void validatesEventsAndPromptRevisions() {
        GenerationEvent event =
                new GenerationEvent("run", 0, GenerationEventType.DELTA, null, null);
        assertEquals("", event.payload());
        assertNotNull(event.createdAt());
        assertThrows(
                IllegalArgumentException.class,
                () -> new GenerationEvent("", 0, GenerationEventType.DELTA, "", null));
        assertThrows(
                IllegalArgumentException.class,
                () -> new GenerationEvent("run", -1, GenerationEventType.DELTA, "", null));
        assertThrows(
                IllegalArgumentException.class,
                () -> new GenerationEvent("run", 0, null, "", null));

        PromptTemplateVersion draft =
                new PromptTemplateVersion("v1", "prompt", 1, "DRAFT", "Hi", null, null, null, null);
        ...
        assertThrows(
                ...
                () ->
                        new PromptTemplateVersion(
                                "v0", "prompt", 0, "DRAFT", "", Map.of(), List.of(), null, null));
        assertThrows(
                IllegalArgumentException.class,
                () ->
                        new PromptTemplateVersion(
