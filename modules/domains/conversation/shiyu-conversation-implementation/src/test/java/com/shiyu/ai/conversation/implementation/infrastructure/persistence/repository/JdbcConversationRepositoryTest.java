...
/**
 * 验证 Jdbc 会话 Repository 相关功能、边界条件、异常路径和协作行为。
 */
class JdbcConversationRepositoryTest {
    ...
    void ordersConversationPagesByIdWhenUpdatedAtTies() throws Exception {
        JdbcConversationRepository repository =
                new JdbcConversationRepository(mock(DataSource.class));
        ...
        repository.listConversations(new TenantId(7), 8, 1, 0);

        var sql = org.mockito.ArgumentCaptor.forClass(String.class);
        verify(jdbc).query(sql.capture(), any(RowMapper.class), any(Object[].class));
        assertTrue(sql.getValue().contains("ORDER BY UPDATED_AT DESC,ID ASC"));
    }
    ...
    void persistsTenantScopedConversationsAndMessages() throws Exception {
        ...
        verify(jdbc, atLeastOnce())
                .update(startsWith("UPDATE CHAT_CONVERSATION"), any(Object[].class));
        ...
        assertEquals(
                "c1", repository.findConversation("c1", new TenantId(7), 8).orElseThrow().id());
        ...
        assertEquals(
                "hello",
                repository.findMessage("m1", new TenantId(7), 8).orElseThrow().textContent());
