...
class JdbcIdempotencyRepositoryTest {
    ...
    void findsAndClaimsTenantScopedKeysIdempotently() throws Exception {
        JdbcIdempotencyRepository repository =
                new JdbcIdempotencyRepository(mock(DataSource.class));
        JdbcTemplate jdbc = mock(JdbcTemplate.class);
        Field field = JdbcIdempotencyRepository.class.getDeclaredField("jdbc");
        ...
        when(jdbc.query(anyString(), any(RowMapper.class), any(Object[].class)))
                .thenAnswer(
                        invocation -> {
                            ...
                            when(row.getString(1)).thenReturn("resource-1");
