...
class JdbcGenerationRepositoryTest {
    ...
    void terminalGenerationUpdateRunsWithinOneTransaction() throws Exception {
        assertTrue(
                JdbcGenerationRepository.class
                        .getDeclaredMethod("update", GenerationRun.class, long.class)
    ...
    void persistsQueriesUpdatesAndMapsGenerationRuns() throws Exception {
        ...
        JdbcGenerationRepository repository =
                new JdbcGenerationRepository(mock(DataSource.class), runtime, 5);
        ...
        GenerationRun run =
                new GenerationRun(
                        ...
                        now,
                        "r1");
        when(jdbc.queryForObject(startsWith("SELECT TENANT_ID"), eq(Long.class), eq("c1")))
                .thenReturn(7L);
        repository.insert(run);
        verify(jdbc, times(2)).update(anyString(), any(Object[].class));
        when(jdbc.queryForObject(
                        contains("INPUT_MESSAGE_ID"), eq(Integer.class), any(), any(), any()))
                .thenReturn(1);
        when(jdbc.queryForObject(
                        contains("CONVERSATION_ID=? AND TENANT_ID=?"),
                        eq(Integer.class),
                        any(),
                        any()))
                .thenReturn(1);
        ...
        when(jdbc.query(anyString(), any(RowMapper.class), any(Object[].class)))
                .thenAnswer(
                        inv -> {
                            ...
                            when(event.getInt("SEQ")).thenReturn(2);
                            when(event.getString("TYPE")).thenReturn("MODEL_DELTA");
                            when(event.getString("PAYLOAD")).thenReturn("hello");
                            when(event.getTimestamp("CREATED_AT")).thenReturn(Timestamp.from(now));
                            return List.of(mapper.mapRow(event, 0));
                        });
        assertEquals(
                GenerationEventType.DELTA,
                repository.listEvents("g1", new TenantId(7), 0, 2000).get(0).type());
        assertThrows(
                IllegalArgumentException.class,
                () ->
                        repository.appendEvent(
                                new GenerationEvent("g1", 1, GenerationEventType.DELTA, "x", now),
                                new TenantId(7)));
    }
    ...
    void handlesMissingConversationDuplicateReservationAndStaleRecovery() throws Exception {
        AiRunRepository runtime = mock(AiRunRepository.class);
        JdbcGenerationRepository repository =
                new JdbcGenerationRepository(mock(DataSource.class), runtime, 1000);
        JdbcTemplate jdbc = mock(JdbcTemplate.class);
        Field field = JdbcGenerationRepository.class.getDeclaredField("jdbc");
        field.setAccessible(true);
        field.set(repository, jdbc);
        ...
        when(jdbc.update(startsWith("INSERT INTO CHAT_GENERATION_ACTIVE"), any(Object[].class)))
    ...
    void recoversStaleGenerationAndRepairsRuntimeProjection() throws Exception {
        ...
        when(jdbc.query(
                        startsWith("SELECT g.*,c.OWNER_USER_ID"),
                        any(RowMapper.class),
                        any(Object[].class)))
                .thenAnswer(
                        inv -> {
        ...
        when(jdbc.queryForObject(startsWith("SELECT TENANT_ID"), eq(Long.class), eq("g-stale")))
                .thenReturn(7L);
        when(jdbc.update(startsWith("UPDATE CHAT_GENERATION_RUN"), any(Object[].class)))
                .thenReturn(1);
        AiRun run =
        ...
        verify(runtime)
                .updateTerminalAndAppend(
                        any(AiRun.class),
                        eq(2L),
                        eq(AiRunEventType.RUN_FAILED),
                        contains("SERVICE_RESTART"),
                        eq(true));
    }
    ...
    void appendsEventsToLinkedRuntimeAndRejectsBlankLink() throws Exception {
        ...
        field.set(repository, jdbc);
        when(jdbc.query(
                        startsWith("SELECT g.RUNTIME_RUN_ID"),
        ...
        Instant now = Instant.now();
        repository.appendEvent(
                new GenerationEvent("g1", 1, GenerationEventType.USAGE, "{}", now),
                new TenantId(7));
        verify(runtime)
                .appendNextEvent(
                        eq("r1"),
        ...
        when(jdbc.query(
                ...
                .thenAnswer(
                        inv -> {
                            ...
                            ResultSet row = mock(ResultSet.class);
                            when(row.next()).thenReturn(true);
                            when(row.getString("RUNTIME_RUN_ID")).thenReturn(" ");
                            when(row.getLong("OWNER_USER_ID")).thenReturn(8L);
                            return extractor.extractData(row);
                        });
        assertThrows(
                ...
                () ->
                        repository.appendEvent(
                                new GenerationEvent("g2", 1, GenerationEventType.DELTA, "x", now),
                                new TenantId(7)));
    }
    ...
    void mapsEveryGenerationEventTypeAndHandlesEmptyQueries() throws Exception {
        ...
        verify(runtime, times(GenerationEventType.values().length))
                .appendNextEvent(
                        eq("r1"),
                        eq(new TenantId(7)),
                        eq(8L),
                        any(AiRunEventType.class),
                        eq("{}"),
                        eq(true),
                        eq(now));

        when(jdbc.query(
                        startsWith("SELECT e.SEQ,e.TYPE"),
                        any(RowMapper.class),
                        any(Object[].class)))
                .thenAnswer(
                        inv -> {
                            RowMapper mapper = inv.getArgument(1);
                            ResultSet row = mock(ResultSet.class);
                            when(row.getInt("SEQ")).thenReturn(1);
            ...
            when(jdbc.query(
                    ...
                    .thenAnswer(
                            inv -> {
                                ...
                                ResultSet row = mock(ResultSet.class);
                                when(row.getInt("SEQ")).thenReturn(1);
                                when(row.getString("TYPE")).thenReturn(type.name());
                                when(row.getString("PAYLOAD")).thenReturn("{}");
    ...
    private static ResultSet generationRow(GenerationRun run) throws Exception {
        ResultSet row = mock(ResultSet.class);
        when(row.getString("ID")).thenReturn(run.id());
        when(row.getString("CONVERSATION_ID")).thenReturn(run.conversationId());
        when(row.getString("INPUT_MESSAGE_ID")).thenReturn(run.inputMessageId());
        when(row.getString("ASSISTANT_MESSAGE_ID")).thenReturn(run.assistantMessageId());
        when(row.getString("SPEAKER_ID")).thenReturn(run.speakerId());
        when(row.getString("PLATFORM")).thenReturn(run.platform());
        when(row.getString("MODEL")).thenReturn(run.model());
        when(row.getString("STATUS")).thenReturn(run.status().name());
        when(row.getLong("PROMPT_TOKENS")).thenReturn(run.promptTokens());
        when(row.getLong("COMPLETION_TOKENS")).thenReturn(run.completionTokens());
        when(row.getLong("LATENCY_MS")).thenReturn(run.latencyMs());
        when(row.getString("ERROR_CODE")).thenReturn(run.errorCode());
        when(row.getInt("LAST_EVENT_SEQUENCE")).thenReturn(run.lastEventSequence());
        when(row.getBoolean("CANCEL_REQUESTED")).thenReturn(run.cancelRequested());
        when(row.getLong("VERSION")).thenReturn(run.version());
        when(row.getTimestamp("CREATED_AT")).thenReturn(Timestamp.from(run.createdAt()));
        when(row.getTimestamp("UPDATED_AT")).thenReturn(Timestamp.from(run.updatedAt()));
        when(row.getString("RUNTIME_RUN_ID")).thenReturn(run.runtimeRunId());
        return row;
    }
}
