...
class GenerationRunnerFailureTest {
    ...
    void synchronousProviderFailureLeavesAReplayedFailedRun() {
        ...
        GenerationRunner runner = new GenerationRunner(engine, generations, conversations,
                new com.shiyu.ai.conversation.contract.api.GenerationUsageSink() { }, admission,
                (AiRuntimePort) null, new ConversationPromptService(null));

        runner.start(created, new TenantId(1), 2);

        verify(admission).release(any(ActorContext.class), argThat(run -> run.status() == GenerationStatus.FAILED));
        verify(generations, atLeastOnce()).appendEvent(argThat(event -> event.type() == GenerationEventType.FAILED), eq(new TenantId(1L)));
        verify(generations, never()).appendEvent(argThat(event -> event.type() == GenerationEventType.COMPLETED), eq(new TenantId(1L)));
