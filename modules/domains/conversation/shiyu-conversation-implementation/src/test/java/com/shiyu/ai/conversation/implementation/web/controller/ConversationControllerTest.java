...
class ConversationControllerTest {
    ...
    private final ConversationController controller =
            new ConversationController(
                    service, conversations, runner, idempotency, generations, imports, prompts);
    ...
    void coversConversationQueriesMutationsAndExports() {
        ...
        when(conversations.listConversations(new TenantId(7), 8, 20, 0))
                .thenReturn(List.of(conversation));
        when(conversations.listMessages("c1", new TenantId(7), 8, 1000))
        ...
        ConversationController.UpdateConversationRequest update =
                new ConversationController.UpdateConversationRequest();
        ...
        when(conversations.listBranches("c1", new TenantId(7), 8))
                .thenReturn(List.of(conversation));
    ...
    void createsIdempotentlyAndPreviewsThenImportsPayload() {
        ConversationController.CreateConversationRequest create =
                new ConversationController.CreateConversationRequest();
        create.setTitle("Chat");
        when(service.create(new TenantId(7), 8, "general", "Chat", null, null, null))
                .thenReturn(conversation);
        assertEquals(conversation, controller.create(null, create).getData());

        String jsonl =
                "{\"role\":\"user\",\"contentParts\":[{\"type\":\"text\",\"text\":\"hello\"}]}";
        ConversationController.ImportRequest request = new ConversationController.ImportRequest();
        request.setContent(jsonl);
        ConversationImportPreviewStore.Preview preview =
                controller.importPreview(request).getData();
        ...
        when(service.create(eq(new TenantId(7L)), eq(8L), anyString(), any(), any(), any(), any()))
                .thenReturn(conversation);
        when(conversations.findConversation("c1", new TenantId(7), 8))
                .thenReturn(Optional.of(conversation));
    ...
    void rejectsMissingActorAndInvalidPayload() {
        ...
        installActor();
        ConversationController.MessageRequest messageRequest =
                new ConversationController.MessageRequest();
    ...
    void previewsPromptBranchesAndMapsImportFormats() {
        ...
        when(conversations.listMessages("c1", new TenantId(7), 8, 1000))
                .thenReturn(List.of(message));
        var context =
                new com.shiyu.ai.agent.contract.runtime.ContextItem(
                        "doc", "d1", "reference", 0.9, null, List.of(), "tenant", now);
        when(prompts.assemble(eq(rag), anyList(), eq(new TenantId(7L)), eq(8L)))
                .thenReturn(
                        new ConversationPromptService.PromptAssembly(
                                List.of(message),
                                List.of(
                                        com.shiyu.ai.model.contract.model.ChatMessage.text(
                                                "user", "hello")),
                                List.of(context),
                                new com.shiyu.ai.agent.contract.runtime.ContextTrace(
                                        "trace",
                                        new TenantId(7),
                                        "hello",
                                        List.of("d1"),
    ...
    void handlesCreateAndMessageIdempotencyAndAdmissionBranches() {
        ConversationController.CreateConversationRequest create =
                new ConversationController.CreateConversationRequest();
        create.setTitle("Chat");
        when(idempotency.find(new TenantId(7), 8, "conversation.create", "key"))
                .thenReturn(Optional.of("c1"));
        when(conversations.findConversation("c1", new TenantId(7), 8))
                .thenReturn(Optional.of(conversation));
        assertEquals(conversation, controller.create("key", create).getData());

        when(idempotency.find(new TenantId(7), 8, "conversation.create", "new"))
                .thenReturn(Optional.empty());
        when(service.create(new TenantId(7), 8, "general", "Chat", null, null, null))
                .thenReturn(conversation);
        when(idempotency.claim(new TenantId(7), 8, "conversation.create", "new", "c1"))
                .thenReturn(false);
        ...
        when(idempotency.claim(new TenantId(7), 8, "conversation.message:c1", "message-key", "g1"))
                .thenReturn(true);
        assertEquals(run, controller.message("c1", "message-key", request).getData());
        verify(runner).start(run, new TenantId(7), 8);

        when(idempotency.find(new TenantId(7), 8, "conversation.message:c1", "existing"))
        ...
        when(idempotency.find(new TenantId(7), 8, "conversation.message:c1", "denied"))
                .thenReturn(Optional.empty());
        when(service.createGeneration(any(), eq(appended), isNull(), isNull()))
                .thenThrow(new IllegalStateException("busy"));
        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.message("c1", "denied", request));
    }
    ...
    void coversIdempotencyConflictAndMutationFailureBranches() {
        ...
        when(service.create(new TenantId(7), 8, "general", "Chat", null, null, null))
                .thenReturn(conversation);
        when(idempotency.find(new TenantId(7), 8, "conversation.create", "claim-race"))
                .thenReturn(Optional.empty());
        when(idempotency.claim(new TenantId(7), 8, "conversation.create", "claim-race", "c1"))
                .thenReturn(false);
        ...
        when(idempotency.claim(new TenantId(7), 8, "conversation.message:c1", "race", "g-race"))
                .thenReturn(false);
        when(idempotency.find(new TenantId(7), 8, "conversation.message:c1", "race"))
                .thenReturn(Optional.empty());
        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.message("c1", "race", request));

        when(idempotency.claim(new TenantId(7), 8, "conversation.message:c1", "quota", "g-race"))
                .thenReturn(true);
        doThrow(new GenerationAdmissionException("QUOTA_EXCEEDED"))
                .when(runner)
                .start(run, new TenantId(7), 8);
        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.message("c1", "quota", request));
    }

    @Test
    void validatesActiveLeafOwnershipAndImportRoleFallback() {
        ConversationController.ActiveLeafRequest body =
                new ConversationController.ActiveLeafRequest();
        ...
        when(conversations.findMessage("other", new TenantId(7), 8))
                .thenReturn(
                        Optional.of(
                                new ConversationMessage(
                                        "other",
        ...
        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.activeLeaf("c1", "other", null));
        ...
        confirm.setPreviewToken(issued.token());
        confirm.setContent(unknownRole);
        when(service.create(eq(new TenantId(7L)), eq(8L), anyString(), any(), any(), any(), any()))
                .thenReturn(conversation);
        when(conversations.findConversation("c1", new TenantId(7), 8))
                .thenReturn(Optional.of(conversation));
        ...
        verify(service, atLeastOnce())
                .appendMessage(
                        eq(conversation), any(), eq(MessageRole.USER), eq("x"), isNull(), isNull());
    }

    @Test
    void mapsConversationMessageToModelMessage() throws Exception {
        ConversationMessage rich =
                new ConversationMessage(
                        "m-rich",
                        "c1",
                        null,
                        ...
                        List.of(
                                new ContentPart(
                                        "image", "caption", "https://img", "image/png", Map.of())),
                        Map.of(),
                        MessageStatus.COMPLETED,
                        0,
                        null,
                        ...
                        now);
        Method method =
                ConversationController.class.getDeclaredMethod(
                        "toModelMessage", ConversationMessage.class);
        method.setAccessible(true);
        com.shiyu.ai.model.contract.model.ChatMessage mapped =
                (com.shiyu.ai.model.contract.model.ChatMessage) method.invoke(controller, rich);
        assertEquals("assistant", mapped.role());
        assertEquals("image", mapped.content().getFirst().type());
        assertTrue(
                new ConversationController.PromptPreview(
                                List.of(), List.of(), 0, false, true, "local", Map.of(), "hash",
                                null, null)
                        .contextItems()
                        .isEmpty());
        GenerationEvent defaults =
                new GenerationEvent("g1", 0, GenerationEventType.DELTA, null, null);
        ...
        assertThrows(
                ...
                () ->
                        imports.consume(
                                new TenantId(7),
                                8,
                                mismatchPreview.token(),
                                "markdown",
                                "# changed"));
        var storePreview = imports.issue(new TenantId(7), 8, "md", "# title", List.of());
        assertTrue(
                imports.consume(new TenantId(7), 8, storePreview.token(), "markdown", "# title")
                        .isEmpty());
        assertThrows(
                IllegalArgumentException.class,
                () ->
                        imports.consume(
                                new TenantId(7), 8, storePreview.token(), "markdown", "# title"));
        try (org.mockito.MockedStatic<MessageDigest> digests = mockStatic(MessageDigest.class)) {
            ...
            assertThrows(
                    ...
                    () -> imports.issue(new TenantId(7), 8, "jsonl", "payload", List.of()));
        }
    }
}
