...
class JdbcChatProductRepositoryTest {
    ...
    void persistsAndReadsAllChatProductAssetsWithTenantFilters() throws Exception {
        JdbcChatProductRepository repository =
                new JdbcChatProductRepository(mock(DataSource.class));
        ...
        CharacterCardV2 card =
                new CharacterCardV2(
                        null, "Ava", "desc", "scene", "hello", List.of(), "system", Map.of(), 2);
        CharacterAsset character = new CharacterAsset("ch1", 7, 8, card, "PRIVATE", now, now);
        PersonaAsset persona =
                new PersonaAsset(
                        "p1",
                        7,
                        8,
                        new Persona("p1", 8, "Ava", "student", "warm", "PRIVATE", Map.of()),
                        now,
                        now);
        LorebookAsset lore =
                new LorebookAsset(
        ...
        GroupChat group =
                new GroupChat(
                        "g1",
                        "Study",
                        List.of(new GroupChat.Participant("ava", "Ava", "ch1")),
        ...
        repository.deleteCharacter(new TenantId(7), 8, "ch1");
        repository.deletePersona(new TenantId(7), 8, "p1");
        repository.deleteLorebook(new TenantId(7), 8, "l1");
        repository.deleteGroup(new TenantId(7), 8, "g1");
        ...
        when(row.getString("PARTICIPANTS_JSON"))
                .thenReturn("[{\"id\":\"ava\",\"displayName\":\"Ava\",\"characterId\":\"ch1\"}]");
        ...
        assertEquals(
                "Ava",
                repository.findCharacter(new TenantId(7), 8, "ch1").orElseThrow().card().name());
        assertEquals(1, repository.listCharacters(new TenantId(7), 8).size());
        assertEquals(
                "Ava",
                repository
                        .findCharacterForAccess(new TenantId(7), 9, "ch1")
                        .orElseThrow()
                        .card()
                        .name());
        assertEquals(
                "Ava",
                repository.findPersona(new TenantId(7), 8, "p1").orElseThrow().persona().name());
        assertEquals(1, repository.listPersonas(new TenantId(7), 8).size());
        assertEquals(
                "context",
                repository.findLorebook(new TenantId(7), 8, "l1").orElseThrow().entry().content());
        assertEquals(1, repository.listLorebooks(new TenantId(7), 8).size());
        assertEquals(
                "welcome", repository.listPrompts(new TenantId(7), 8, null).get(0).templateId());
        assertEquals(
                "Study",
                repository.findGroup(new TenantId(7), 8, "g1").orElseThrow().group().name());
