...
class ConversationServiceTest {
    ...
    void createsConversationWithDefaultTitleAndPersistsSystemPrompt() {
        ...
        Conversation created =
                service.create(new TenantId(7), 8, "chat", " ", "OPENAI", "gpt", "be concise");
                        ...
                        argThat(
                                message ->
                                        message.role() == MessageRole.SYSTEM
                                                && message.textContent().equals("be concise")));
    ...
    void generationRejectsWrongConversationInactiveLeafAndRunningDuplicate() {
        ...
        assertThrows(
                ...
                () -> service.createGeneration(conversation, wrong, "OPENAI", "gpt"));

        ConversationMessage stale = message("m0", "c1", 0);
        assertThrows(
                IllegalStateException.class,
                () -> service.createGeneration(conversation, stale, "OPENAI", "gpt"));

        ConversationMessage input = message("m1", "c1", 1);
        when(generations.hasRunning("c1", "m1", new TenantId(7))).thenReturn(true);
        assertThrows(
                IllegalStateException.class,
                () -> service.createGeneration(conversation, input, "OPENAI", "gpt"));
    }
    ...
    void recordsCompletedGenerationAndEvents() {
        ...
        GenerationRun completed =
                service.recordCompletedGeneration(
                        conversation, input, "answer", "OPENAI", "gpt", 4, 6);
                        ...
                        argThat(
                                message ->
                                        message.role() == MessageRole.ASSISTANT
                                                && message.textContent().equals("answer")));
        verify(generations, atLeast(4))
                .appendEvent(any(GenerationEvent.class), eq(new TenantId(7)));
    }
    ...
    void recordsConflictByDeletingAssistantAndFailingGeneration() {
        ...
        assertThrows(
                ...
                () ->
                        service.recordCompletedGeneration(
                                conversation, input, "answer", "OPENAI", "gpt", 1, 1));
        verify(conversations).deleteMessage(anyString(), eq(new TenantId(7)), eq(8L));
        verify(generations)
                .appendEvent(
    ...
    void recordsCompletedGenerationThroughRuntimeProjection() {
        ...
        GenerationRun completed =
                service.recordCompletedGeneration(
                        conversation, input, "answer", "OPENAI", "gpt", 4, 6, runtime, runtimeRun);
        assertEquals(GenerationStatus.COMPLETED, completed.status());
    ...
    void rejectsMissingConversationAndGenerationCompletionCasConflict() {
        ...
        assertThrows(
                ...
                () ->
                        service.recordCompletedGeneration(
                                conversation, input, null, "OPENAI", "gpt", 0, 0));
        ...
        assertThrows(
                ...
                () ->
                        service.recordCompletedGeneration(
                                conversation, input, "", "OPENAI", "gpt", 0, 0));
    }
