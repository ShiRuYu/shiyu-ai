...
/**
 * 验证 消息 Controller 相关功能、边界条件、异常路径和协作行为。
 */
class MessageControllerTest {
    ...
    private final MessageController controller =
            new MessageController(service, conversations, generations, runner, idempotency);
    ...
    private final Conversation conversation =
            new Conversation(
                    ...
                    now,
                    now);
    private final ConversationMessage input =
            new ConversationMessage(
                    ...
                    now,
                    now);
    private final ConversationMessage edited =
            new ConversationMessage(
                    "m2",
                    "c1",
                    null,
                    "m1",
                    MessageRole.USER,
                    List.of(ContentPart.text("updated")),
                    Map.of(),
                    MessageStatus.COMPLETED,
                    1,
                    null,
    ...
    void editsMessageAndSupportsIdempotentReplay() {
        ...
        when(idempotency.find(new TenantId(7), 8, "message.edit:m1", "key"))
    ...
    void retriesUserMessageAndStartsGeneration() {
        ...
        when(idempotency.claim(
                        any(TenantId.class), anyLong(), anyString(), anyString(), anyString()))
                .thenReturn(true);
    ...
    void rejectsInvalidEditAndNonUserRetry() {
        ...
        ConversationMessage assistant =
                new ConversationMessage(
                        ...
                        now,
                        now);
        when(conversations.findMessage("a1", new TenantId(7), 8))
    ...
    void handlesIdempotencyRaceAndMovesRetryLeafWithConflictMapping() {
        ...
        when(service.appendMessage(
                        ...
                        eq("m1")))
                .thenReturn(edited);
        when(idempotency.claim(new TenantId(7), 8, "message.edit:m1", "key", "m2"))
                .thenReturn(false);
        when(idempotency.find(new TenantId(7), 8, "message.edit:m1", "key"))
        ...
        Conversation branched =
                new Conversation(
                        ...
                        now);
        when(conversations.findConversation("c1", new TenantId(7), 8))
                .thenReturn(Optional.of(branched));
        GenerationRun run =
                new GenerationRun(
                        "g2",
                        "c1",
        ...
        verify(conversations)
                .updateConversation(argThat(c -> "m1".equals(c.activeLeafMessageId())), eq(2L));

        when(conversations.updateConversation(any(Conversation.class), eq(2L))).thenReturn(0);
        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.retry("m1", null, null));
    }
}
