...
class MessageControllerCoverageTest {
    ...
    private static final Conversation CONVERSATION =
            new Conversation(
                    ...
                    NOW,
                    NOW);
    private static final ConversationMessage USER =
    ...
    void editsMessagesAndRetriesGenerationsWithIdempotency() {
        ...
        MessageController controller =
                new MessageController(conversations, repository, generations, runner, idempotency);
        ConversationMessage edited =
                new ConversationMessage(
                        ...
                        null,
                        MessageRole.USER,
                        List.of(ContentPart.text("edited")),
                        Map.of(),
                        MessageStatus.COMPLETED,
                        0,
                        "m1",
        ...
        when(repository.findConversation("c1", new TenantId(7), 9))
        ...
        when(conversations.appendMessage(
                        ...
                        eq("m1")))
                .thenReturn(edited);
        when(idempotency.claim(new TenantId(7), 9, "message.edit:m1", "edit-key", "m2"))
                .thenReturn(true);
        when(conversations.createGeneration(any(), eq(USER), eq("OPENAI"), eq("gpt")))
                .thenReturn(generation);
        when(idempotency.claim(new TenantId(7), 9, "message.generation:m1", "gen-key", "g1"))
                .thenReturn(true);
        try (var ignored = mockStatic(ActorContextHttpAdapter.class)) {
            ...
            MessageController.EditRequest edit = new MessageController.EditRequest();
            edit.setContent("edited");
            assertEquals(edited, controller.edit("m1", "edit-key", edit).getData());
            MessageController.RetryRequest retry = new MessageController.RetryRequest();
            retry.setPlatform("OPENAI");
            retry.setModel("gpt");
    ...
    void coversExistingKeysAndValidationFailures() {
        ...
        GenerationRun generation =
                new GenerationRun(
                        ...
                        NOW,
                        NOW);
        when(idempotency.find(new TenantId(7), 9, "message.edit:m1", "existing"))
                .thenReturn(Optional.of("m2"));
        when(repository.findMessage("m2", new TenantId(7), 9)).thenReturn(Optional.of(USER));
        when(idempotency.find(new TenantId(7), 9, "message.generation:m1", "existing"))
        ...
        when(repository.findConversation("c1", new TenantId(7), 9))
                .thenReturn(Optional.of(CONVERSATION));
        when(conversations.appendMessage(
                        any(), any(), any(), anyString(), isNull(), isNull(), any()))
                .thenReturn(USER);
        when(conversations.createGeneration(any(), eq(USER), isNull(), isNull()))
                .thenReturn(generation);
        try (var ignored = mockStatic(ActorContextHttpAdapter.class)) {
            ignored.when(ActorContextHttpAdapter::tenantId).thenReturn(7L);
            ignored.when(ActorContextHttpAdapter::userId).thenReturn(9L);
            ...
            assertThrows(
                    ...
                    () -> controller.edit("m1", null, edit));
            when(repository.findMessage("missing", new TenantId(7), 9))
                    .thenReturn(Optional.empty());
            assertThrows(
                    org.springframework.web.server.ResponseStatusException.class,
                    () -> controller.edit("missing", null, edit));
            when(repository.findMessage("assistant", new TenantId(7), 9))
                    .thenReturn(
                            Optional.of(
                                    new ConversationMessage(
                                            "assistant",
            ...
            assertThrows(
                    org.springframework.web.server.ResponseStatusException.class,
                    () -> controller.retry("assistant", null, null));
            doThrow(new GenerationAdmissionException("QUOTA"))
                    .when(runner)
                    .start(generation, new TenantId(7), 9);
            when(idempotency.find(new TenantId(7), 9, "message.generation:m1", "quota"))
                    .thenReturn(Optional.empty());
            when(idempotency.claim(new TenantId(7), 9, "message.generation:m1", "quota", "g1"))
                    .thenReturn(true);
            assertThrows(
                    org.springframework.web.server.ResponseStatusException.class,
                    () -> controller.retry("m1", "quota", null));
        }
    }
}
