...
class ConversationControllerTest {
    ...
    void previewsPromptBranchesAndMapsImportFormats() {
        ...
        when(conversations.listMessages("c1", new TenantId(7), 8, 1000)).thenReturn(List.of(message));
        var context = new com.shiyu.ai.agent.contract.runtime.ContextItem("doc", "d1", "reference", 0.9, null, List.of(), "tenant", now);
        when(prompts.assemble(eq(rag), anyList(), eq(new TenantId(7L)), eq(8L))).thenReturn(new ConversationPromptService.PromptAssembly(
                List.of(message), List.of(com.shiyu.ai.model.contract.model.ChatMessage.text("user", "hello")),
                List.of(context), new com.shiyu.ai.agent.contract.runtime.ContextTrace("trace", new TenantId(7), "hello", List.of("d1"), "rag", now)));
    ...
    void mapsConversationMessageToModelMessage() throws Exception {
        ...
        com.shiyu.ai.model.contract.model.ChatMessage mapped = (com.shiyu.ai.model.contract.model.ChatMessage) method.invoke(controller, rich);
