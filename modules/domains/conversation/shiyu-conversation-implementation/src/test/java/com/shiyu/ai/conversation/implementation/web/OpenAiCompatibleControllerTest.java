...
class OpenAiCompatibleControllerTest {
    ...
    void rejectsEmptyOrNullChatMessagesBeforeStartingAProviderRun() {
        var empty = new OpenAiCompatibleController.ChatCompletionRequest("gpt", null,
                List.of(), false, false, null, null, null, null, null);
        assertThrows(org.springframework.web.server.ResponseStatusException.class,
                () -> controller.chatCompletions(empty));

        var withNull = new OpenAiCompatibleController.ChatCompletionRequest("gpt", null,
                java.util.Arrays.asList((Map<String, Object>) null), false, false, null, null, null, null, null);
        assertThrows(org.springframework.web.server.ResponseStatusException.class,
    ...
    void rejectsResponsesWithoutInputBeforeStartingAProviderRun() {
        var empty = new OpenAiCompatibleController.ResponsesRequest("gpt", null,
    ...
    void persistsExplicitStoreAndCoversRuntimeEventHelpers() throws Exception {
        ...
        ConversationMessage input = new ConversationMessage("m1", "c1", null, null, com.shiyu.ai.conversation.implementation.domain.model.MessageRole.USER,
                List.of(com.shiyu.ai.conversation.implementation.domain.model.ContentPart.text("hello")), Map.of(), com.shiyu.ai.conversation.implementation.domain.model.MessageStatus.COMPLETED, 0, null, Instant.now(), Instant.now());
    ...
    void comparesExistingStoredMessagesByRoleAndText() throws Exception {
        ConversationMessage message = new ConversationMessage("m1", "c1", null, null,
                ...
                List.of(com.shiyu.ai.conversation.implementation.domain.model.ContentPart.text("hello")), Map.of(),
                com.shiyu.ai.conversation.implementation.domain.model.MessageStatus.COMPLETED, 0, null, Instant.now(), Instant.now());
        assertTrue((Boolean) invoke("sameMessage", new Class<?>[]{ConversationMessage.class, com.shiyu.ai.conversation.implementation.domain.model.MessageRole.class, String.class},
                message, com.shiyu.ai.conversation.implementation.domain.model.MessageRole.USER, "hello"));
        assertFalse((Boolean) invoke("sameMessage", new Class<?>[]{ConversationMessage.class, com.shiyu.ai.conversation.implementation.domain.model.MessageRole.class, String.class},
                message, com.shiyu.ai.conversation.implementation.domain.model.MessageRole.ASSISTANT, "hello"));
        assertFalse((Boolean) invoke("sameMessage", new Class<?>[]{ConversationMessage.class, com.shiyu.ai.conversation.implementation.domain.model.MessageRole.class, String.class},
                message, com.shiyu.ai.conversation.implementation.domain.model.MessageRole.USER, "different"));
    }
    ...
    void streamsFailureAndDefaultDeltaEvents() {
        when(engine.stream(any())).thenReturn(Flux.fromIterable(List.of(
                ...
                ChatResponse.builder().eventType("FAILED").errorMessage("provider secret").build())));
        var request = new OpenAiCompatibleController.ChatCompletionRequest("gpt", null,
                List.of(Map.of("role", "user", "content", "hi")), true, false, null, null, null, null, null);
        List<?> events = ((Flux<?>) controller.chatCompletions(request)).collectList().block();
        assertEquals(3, events.size());
        String failurePayload = String.valueOf(((org.springframework.http.codec.ServerSentEvent<?>) events.get(1)).data());
