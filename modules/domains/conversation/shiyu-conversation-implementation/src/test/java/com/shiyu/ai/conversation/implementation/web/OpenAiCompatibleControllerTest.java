...
class OpenAiCompatibleControllerTest {
    ...
    private final OpenAiCompatibleController controller =
            new OpenAiCompatibleController(engine, models, conversations, repository, runtime);
    private final AiRun run =
            new AiRun(
                    "run1",
                    new TenantId(7),
                    new UserId(8),
    ...
    void servesModelsAndNonStreamingChatAndResponses() {
        when(models.availableModels())
                .thenReturn(List.of(new ModelRoutingPort.ModelDescriptor("gpt", "OPENAI")));
                ...
                new OpenAiCompatibleController.ChatCompletionRequest(
                        ...
                        List.of(
                                ...
                                Map.of(
                                        ...
                                        "assistant",
                                        "content",
                                        List.of(Map.of("type", "input_text", "text", "ok")))),
                        false,
                        false,
                        0.3,
                        0,
        ...
        var responses =
                new OpenAiCompatibleController.ResponsesRequest(
                        "gpt", "openai", "hello", false, false, null, 999999, null, null, null);
        Object output = controller.responses(responses);
        assertEquals("response", ((Map<?, ?>) output).get("object"));

        ChatResponse withToolCall =
                ChatResponse.builder()
                        .success(true)
                        .content("tool result")
                        .toolCalls(List.of(new ChatResponse.ToolCall("call-1", "search", "{}")))
                        .build();
    ...
    void rejectsFailedModelsAndInvalidTools() {
        ...
        assertThrows(
                ...
                () -> controller.chatCompletions(request));
        var invalid =
                new OpenAiCompatibleController.ChatCompletionRequest(
                        "gpt",
                        null,
                        ...
                        null,
                        null,
                        List.of(Map.of("function", Map.of("description", "missing name"))),
                        null,
                        null);
        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.chatCompletions(invalid));
    }

    @Test
    void rejectsEmptyOrNullChatMessagesBeforeStartingAProviderRun() {
        var empty =
                new OpenAiCompatibleController.ChatCompletionRequest(
                        "gpt", null, List.of(), false, false, null, null, null, null, null);
        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.chatCompletions(empty));
        ...
        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.chatCompletions(withNull));

        verify(runtime, never()).startRun(any(), any(), anyString(), anyString(), anyString());
        verifyNoInteractions(engine);
    }

    @Test
    void rejectsResponsesWithoutInputBeforeStartingAProviderRun() {
        var empty =
                new OpenAiCompatibleController.ResponsesRequest(
                        "gpt", null, null, false, false, null, null, null, null, null);

        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () -> controller.responses(empty));
        verify(runtime, never()).startRun(any(), any(), anyString(), anyString(), anyString());
        verifyNoInteractions(engine);
    }
    ...
    void streamsResponsesToolReasoningUsageAndCancellationBranches() {
                ...
                new OpenAiCompatibleController.ResponsesRequest(
                        ...
                        List.of(
                                Map.of(
                                        ...
                                        "user",
                                        "content",
                                        List.of(Map.of("type", "input_text", "text", "hi")))),
                        true,
                        false,
                        null,
                        null,
        ...
        when(engine.stream(any()))
                .thenReturn(Flux.just(ChatResponse.builder().eventType("CANCELLED").build()));
        List<?> cancelled = ((Flux<?>) controller.responses(responses)).collectList().block();
        assertEquals(2, cancelled.size());
        verify(runtime)
                .finish("run1", new TenantId(7), 8, AiRunStatus.CANCELLED, "MODEL_CANCELLED");
    }
    ...
    void mapsStructuredInputsToolsAndPrivateResponseHelpers() throws Exception {
        List<Map<String, Object>> raw =
                List.of(
                        ...
                        Map.of(
                                ...
                                "content",
                                List.of(
                                        Map.of("type", "input_text", "text", "answer"),
                                        Map.of(
                                                "type",
                                                "input_image",
                                                "image_url",
                                                Map.of("url", "https://img")))),
                        Map.of(
                                "role",
                                "assistant",
                                "content",
        ...
        assertThrows(
                ...
                () ->
                        invoke(
                                "tools",
                                new Class<?>[] {List.class},
                                List.of(Map.of("function", Map.of("description", "missing")))));

        ChatResponse withTools =
                ChatResponse.builder()
                        .content("answer")
                        .promptTokens(2)
                        .completionTokens(3)
                        .totalTokens(5)
                        .toolCalls(List.of(new ChatResponse.ToolCall("call1", "search", "{}")))
                        .build();
        @SuppressWarnings("unchecked")
        Map<String, Object> completion =
                (Map<String, Object>)
                        invoke(
                                "completion",
                                new Class<?>[] {String.class, ChatResponse.class},
                                "gpt",
                                withTools);
        assertTrue(completion.containsKey("choices"));
        assertEquals(
                2,
                ((Map<?, ?>) invoke("usage", new Class<?>[] {ChatResponse.class}, withTools))
                        .get("prompt_tokens"));
        assertEquals(
                "OPENAI",
                invoke("platform", new Class<?>[] {String.class, String.class}, "gpt", " openai "));
        assertEquals(
                "OPENAI",
                invoke("platform", new Class<?>[] {String.class, String.class}, "gpt", null));
        ...
        assertNull(
                invoke("safeMaxOutputTokens", new Class<?>[] {Integer.class}, new Object[] {null}));

        @SuppressWarnings("unchecked")
        List<ChatMessage> fromInput =
                (List<ChatMessage>) invoke("messageMaps", new Class<?>[] {Object.class}, raw);
        assertEquals(5, fromInput.size());
        assertTrue(
                ((List<?>) invoke("messageMaps", new Class<?>[] {Object.class}, "not-a-list"))
                        .isEmpty());
    }
    ...
    void persistsExplicitStoreAndCoversRuntimeEventHelpers() throws Exception {
        ...
        ConversationMessage input =
                new ConversationMessage(
                        ...
                        null,
                        null,
                        com.shiyu.ai.conversation.implementation.domain.model.MessageRole.USER,
                        List.of(
                                com.shiyu.ai.conversation.implementation.domain.model.ContentPart
                                        .text("hello")),
                        Map.of(),
                        com.shiyu.ai.conversation.implementation.domain.model.MessageStatus
                                .COMPLETED,
                        0,
                        null,
                        Instant.now(),
                        Instant.now());
        when(repository.findConversation("c1", new TenantId(7), 8))
                .thenReturn(Optional.of(conversation));
        when(repository.listMessages("c1", new TenantId(7), 8, 1000)).thenReturn(List.of());
        when(conversations.appendMessage(any(), any(), any(), anyString(), isNull(), isNull()))
                .thenReturn(input);
        invoke(
                "persistIfRequested",
                new Class<?>[] {
                ...
                true,
                "c1",
                List.of(ChatMessage.text("user", "hello")),
                "answer",
                "OPENAI",
                "gpt",
                run);
        ...
        invoke(
                "appendRuntimeEvent",
                new Class<?>[] {AiRun.class, ChatResponse.class, String.class},
                run,
                ChatResponse.builder().eventType("DELTA").content("x").build(),
                "DELTA");
        invoke(
                "appendRuntimeEvent",
                new Class<?>[] {AiRun.class, ChatResponse.class, String.class},
        ...
        invoke(
                ...
                new Class<?>[] {AiRun.class, ChatResponse.class, String.class},
                run,
                ChatResponse.builder().eventType("REASONING_DELTA").build(),
                "REASONING_DELTA");
        invoke(
                "appendRuntimeEvent",
                new Class<?>[] {AiRun.class, ChatResponse.class, String.class},
        ...
        invoke(
                ...
                new Class<?>[] {AiRun.class, ChatResponse.class, String.class},
                null,
                ChatResponse.builder().eventType("DELTA").build(),
                "DELTA");
    ...
    void coversOpenAiEdgeMappingsAndUnsuccessfulStreamTermination() throws Exception {
        ...
        List<ChatMessage> empty =
                (List<ChatMessage>)
                        invoke("structured", new Class<?>[] {List.class}, new Object[] {null});
        assertTrue(empty.isEmpty());
        @SuppressWarnings("unchecked")
        List<ChatMessage> mixed =
                (List<ChatMessage>)
                        invoke(
                                "structured",
                                new Class<?>[] {List.class},
        ...
        assertEquals(
                "text",
                ((ChatMessage.ContentPart) invoke("contentPart", new Class<?>[] {Object.class}, 9))
                        .type());

        assertTrue(
                ((List<?>) invoke("tools", new Class<?>[] {List.class}, new Object[] {null}))
                        .isEmpty());
        assertEquals(
                "OPENAI",
                invoke("platform", new Class<?>[] {String.class, String.class}, null, null));
        ...
        assertEquals(
                ...
                ((Map<?, ?>)
                                invoke(
                                        ...
                                        ChatResponse.builder().content("").build()))
                        .get("total_tokens"));
        assertThrows(
                org.springframework.web.server.ResponseStatusException.class,
                () ->
                        invoke(
                                "requireSuccessful",
                                new Class<?>[] {ChatResponse.class, AiRun.class},
        ...
        var incomplete =
                new OpenAiCompatibleController.ChatCompletionRequest(
                        ...
                        null);
        assertEquals(
                2, ((Flux<?>) controller.chatCompletions(incomplete)).collectList().block().size());
        when(engine.stream(any())).thenReturn(Flux.error(new IllegalStateException("stream down")));
        assertThrows(
                Exception.class,
                () -> ((Flux<?>) controller.chatCompletions(incomplete)).collectList().block());
        verify(runtime, atLeastOnce())
                .finish(
                        eq("run1"),
                        eq(new TenantId(7L)),
                        eq(8L),
                        eq(AiRunStatus.FAILED),
                        anyString());
    }
                ...
                (Boolean)
                        invoke(
                                ...
                                new Class<?>[] {
                                    ...
                                    com.shiyu.ai.conversation.implementation.domain.model
                                            .MessageRole.class,
                                    String.class
                                },
                                message,
                                com.shiyu.ai.conversation.implementation.domain.model.MessageRole
                                        .USER,
                                "hello"));
        assertFalse(
                (Boolean)
    ...
    void streamsFailureAndDefaultDeltaEvents() {
        ...
        String failurePayload =
                String.valueOf(
                        ((org.springframework.http.codec.ServerSentEvent<?>) events.get(1)).data());
        assertFalse(failurePayload.contains("provider secret"));
        assertTrue(failurePayload.contains("model call failed"));
        verify(runtime)
                .finish("run1", new TenantId(7), 8, AiRunStatus.FAILED, "IllegalStateException");
    }
