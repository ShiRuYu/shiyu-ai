...
/**
 * 验证 生成 Runner Edge 相关功能、边界条件、异常路径和协作行为。
 */
class GenerationRunnerEdgeTest {
    ...
    void rejectsMissingTenantBeforeAccessingConversationData() {
        ...
        assertThrows(
                ...
                () ->
                        runner(engine, generations, conversations)
                                .start(generation("g-null-tenant"), null, 2));
    ...
    void handlesNullAndUnknownProviderStreamsAsFailedRuns() {
        ...
        runner(engine, generations, conversations, usage, admission)
                .start(created, new TenantId(1), 2);
    ...
    private static void setupBasic(
        ...
        when(conversations.findMessage("m1", new TenantId(1), 2))
                .thenReturn(Optional.of(message("m1", "c1", 0)));
        when(conversations.findConversation("c1", new TenantId(1), 2))
                .thenReturn(Optional.of(conversation("c1", "m1")));
        when(conversations.listMessages("c1", new TenantId(1), 2, 1000))
                .thenReturn(List.of(message("m1", "c1", 0)));
