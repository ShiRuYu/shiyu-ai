    ...
    void previewAndExecutionAssemblyIncludeTheSameOptInContextMessage() {
        ContextItem item =
                new ContextItem(
                        ...
                        "algebra reference",
                        0.9,
                        new ContextCitation("Algebra", "doc-1", "1", null),
                        List.of("document:doc-1"),
                        "knowledge-access",
                        Instant.now());
        ContextAssemblyPort context =
                query -> new ContextAssemblyPort.ContextResult(List.of(item), null);
        ...
        ConversationMessage user =
                new ConversationMessage(
                        ...
                        null,
                        MessageRole.USER,
                        List.of(ContentPart.text("What is algebra?")),
                        Map.of(),
        ...
        ConversationPromptService.PromptAssembly assembly =
                service.assemble(conversation, List.of(user), new TenantId(1), 2);

        assertEquals(1, assembly.contextItems().size());
        assertEquals("system", assembly.modelMessages().get(0).role());
        assertTrue(
                assembly.modelMessages()
                        .get(0)
                        .content()
                        .get(0)
                        .text()
                        .contains("algebra reference"));
