...
class GenerationRunnerEventTest {
    ...
    void projectsEveryProviderEventAndUpdatesUsage() throws Exception {
        ...
        GenerationRunner runner =
                new GenerationRunner(
                        ...
                        admission);
        Method onEvent =
                GenerationRunner.class.getDeclaredMethod(
                        "onEvent",
                        ChatResponse.class,
        ...
        AtomicReference<com.shiyu.ai.agent.contract.runtime.AiRun> runtime =
                new AtomicReference<>();
        ...
        onEvent.invoke(
                runner,
                ChatResponse.builder().eventType("UNKNOWN").build(),
                state,
                runtime,
                answer,
                7L,
        ...
        verify(generations, atLeast(6))
                .appendEvent(any(GenerationEvent.class), eq(new TenantId(7L)));

        AtomicReference<GenerationRun> cancelled = new AtomicReference<>(running);
        onEvent.invoke(
                runner,
                ChatResponse.builder().eventType("CANCELLED").build(),
                cancelled,
                runtime,
                new StringBuilder(),
                7L,
    ...
    void finalizesAssistantMessageAndHandlesConversationCasLoss() throws Exception {
        ...
        GenerationRunner runner =
                new GenerationRunner(
                        mock(ChatEngine.class), generations, conversations, usage, admission);
        Method finish =
                GenerationRunner.class.getDeclaredMethod(
                        "finishSuccess",
                        AtomicReference.class,
                        ...
                        argThat(
                                message ->
                                        message.role() == MessageRole.ASSISTANT
                                                && "answer".equals(message.textContent())));
        verify(admission)
                .settle(any(), argThat(value -> value.status() == GenerationStatus.COMPLETED));
        ...
        finish.invoke(
                ...
                new AtomicInteger(),
                "answer");
        verify(conversations, atLeastOnce())
                .deleteMessage(anyString(), eq(new TenantId(7L)), eq(8L));
    }
    ...
    void recordsSynchronousProviderFailureAndDoesNotOverwriteLostCas() throws Exception {
        ...
        verify(admission)
                .release(any(), argThat(value -> value.status() == GenerationStatus.FAILED));
    ...
    void mapsRuntimeEventTypesAndUsesPersistedUsageWhenProviderOmitsCounts() throws Exception {
        ...
        GenerationRunner runner =
                new GenerationRunner(
                        ...
                        conversations,
                        new GenerationUsageSink() {},
                        mock(GenerationAdmission.class),
                        runtime,
        ...
        onEvent.invoke(
                runner,
                ChatResponse.builder().eventType("USAGE").build(),
                state,
                runtimeState,
                new StringBuilder(),
                7L,
        ...
        verify(runtime, atLeast(5))
                .append(eq(run), any(), anyString(), eq(true), anyString(), any(), any());
        verify(runtime)
                .recordUsage(
                        eq("runtime"),
                        eq(new TenantId(7L)),
