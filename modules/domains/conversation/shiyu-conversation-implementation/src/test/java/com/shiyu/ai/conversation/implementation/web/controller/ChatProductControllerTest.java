...
class ChatProductControllerTest {
    ...
    private final ChatProductController controller =
            new ChatProductController(
                    repository,
                    conversations,
                    conversationService,
                    generationRunner,
                    generations,
                    previews);
    private final CharacterCardV2 card =
            new CharacterCardV2(
                    null,
                    "Ava",
                    "desc",
    ...
    void characterPersonaLorebookAndImportEndpointsUseTenantActor() throws Exception {
        ...
        ChatProductController.CharacterRequest request =
                new ChatProductController.CharacterRequest();
        request.card = card;
        ...
        when(repository.findCharacterForAccess(new TenantId(7), 8, "ch1"))
        ...
        assertArrayEquals(
                CharacterCardCodec.toPng(card, null), controller.exportCharacter("ch1").getBody());

        byte[] json =
                CharacterCardCodec.toJson(card).getBytes(java.nio.charset.StandardCharsets.UTF_8);
        MockMultipartFile jsonFile =
                new MockMultipartFile("file", "card.json", "application/json", json);
        ...
        assertThrows(
                ...
                () -> controller.importCharacter(jsonFile, preview.token()));
        ...
        when(repository.findPersona(new TenantId(7), 8, "p1"))
                .thenReturn(Optional.of(personaAsset));
        ...
        LorebookEntry entry =
                new LorebookEntry("l1", List.of("magic"), "context", 1, "before", 50, true);
    ...
    void promptStudioValidatesVersionsVariablesAndRenders() {
        ...
        publish.version = 0;
        assertThrows(
                ResponseStatusException.class, () -> controller.publishPrompt("welcome", publish));

        when(repository.listPrompts(new TenantId(7), 8, "welcome"))
        ...
        ChatProductController.PromptPreviewRequest preview =
                new ChatProductController.PromptPreviewRequest();
        preview.body = "Hi {{name}}";
        preview.variables = Map.of("name", "Ava");
        assertEquals("Hi Ava", controller.preview(preview).getData().rendered());
        ChatProductController.PromptTestRequest test =
                new ChatProductController.PromptTestRequest();
        test.version = 1;
        test.variables = Map.of("name", "Ava");
        assertEquals(
                List.of("Hi Ava"),
                controller.testPrompt("welcome", test).getData().renderedCases());
    }
    ...
    void groupEndpointsPlanTurnsAndRejectInvalidRuns() {
        ...
        request.name = "Study";
        request.participants = List.of(ava);
        request.speakerPolicy = SpeakerPolicy.MANUAL;
        ...
        when(conversations.findConversation("c1", new TenantId(7), 8))
                .thenReturn(
                        Optional.of(
                                new Conversation(
                                        "c1",
    ...
    void startsGroupTurnAndReturnsExhaustedPlanWithoutStartingProvider() {
        GroupChat.Participant ava = new GroupChat.Participant("ava", "Ava", "ch1");
        GroupChat group =
                new GroupChat("g1", "Study", List.of(ava), SpeakerPolicy.ROUND_ROBIN, 1, 100);
        GroupChatAsset asset = new GroupChatAsset("g1", 7, 8, group, now, now);
        when(repository.findGroup(new TenantId(7), 8, "g1")).thenReturn(Optional.of(asset));
        ...
        GenerationRun completed =
                new GenerationRun(
                        ...
                        now,
                        now);
        when(generations.listConversation("c1", new TenantId(7), 1000))
                .thenReturn(List.of(completed));
        ChatProductController.TurnRunRequest exhausted = new ChatProductController.TurnRunRequest();
        exhausted.conversationId = "c1";
        exhausted.content = "next";
        ...
        GroupChat open =
                new GroupChat("g2", "Study", List.of(ava), SpeakerPolicy.ROUND_ROBIN, 2, 100);
        when(repository.findGroup(new TenantId(7), 8, "g2"))
                .thenReturn(Optional.of(new GroupChatAsset("g2", 7, 8, open, now, now)));
        ...
        GenerationRun created =
                new GenerationRun(
                        ...
                        now,
                        now);
        when(conversationService.createGeneration(conversation, input, "OPENAI", "gpt", "ava"))
                .thenReturn(created);
        exhausted = new ChatProductController.TurnRunRequest();
        exhausted.conversationId = "c1";
        exhausted.content = "next";
    ...
    void coversPngImportExportPromptEdgeCasesAndAdmissionErrors() throws Exception {
        ...
        when(repository.findCharacterForAccess(new TenantId(7), 8, "png"))
                .thenReturn(Optional.of(storedPng));
        assertArrayEquals(png, controller.exportCharacter("png").getBody());

        ChatProductController.PromptRequest publishedRequest =
                new ChatProductController.PromptRequest();
        publishedRequest.templateId = "published";
        publishedRequest.version = 1;
        publishedRequest.status = "PUBLISHED";
        publishedRequest.body = "hello";
        publishedRequest.testCases = List.of();
        publishedRequest.variableSchema = Map.of();
        when(repository.listPrompts(new TenantId(7), 8, "published")).thenReturn(List.of());
        when(repository.savePrompt(any(), eq(new TenantId(7L)), eq(8L)))
        ...
        test.version = 1;
        when(repository.listPrompts(new TenantId(7), 8, "published"))
                .thenReturn(
                        List.of(
                                new PromptTemplateVersion(
                                        "p",
        ...
        assertEquals(
                List.of("{{x}}"),
                controller.testPrompt("published", test).getData().renderedCases());
        ...
        when(repository.findGroup(new TenantId(7), 8, "g1"))
                .thenReturn(
                        Optional.of(
                                new GroupChatAsset(
                                        "g1",
    ...
    void rejectsMissingModelAndMapsGenerationAdmissionConflict() {
        GroupChat.Participant ava = new GroupChat.Participant("ava", "Ava", "ch1");
        GroupChat group =
                new GroupChat("g3", "Study", List.of(ava), SpeakerPolicy.ROUND_ROBIN, 2, 100);
        when(repository.findGroup(new TenantId(7), 8, "g3"))
                .thenReturn(Optional.of(new GroupChatAsset("g3", 7, 8, group, now, now)));
        Conversation noModel =
                new Conversation(
                        ...
                        now,
                        now);
        when(conversations.findConversation("c3", new TenantId(7), 8))
                .thenReturn(Optional.of(noModel));
        ...
        ChatProductController.TurnRunRequest request = new ChatProductController.TurnRunRequest();
        request.conversationId = "c3";
    ...
    void coversGroupTurnValidationPlatformOverridesAndAdmissionFailures() {
        GroupChat.Participant ava = new GroupChat.Participant("ava", "Ava", "ch1");
        GroupChat group =
                new GroupChat("g4", "Study", List.of(ava), SpeakerPolicy.ROUND_ROBIN, 3, 100);
        ...
        when(conversations.findConversation("c4", new TenantId(7), 8))
                .thenReturn(Optional.of(conversation));
        when(generations.hasRunningConversation("c4", new TenantId(7))).thenReturn(false);
        GenerationRun ignoredFailed =
                new GenerationRun(
                        "failed",
                        "c4",
                        "m",
                        ...
                        now,
                        now);
        GenerationRun ignoredNoSpeaker =
                new GenerationRun(
                        "no-speaker",
                        "c4",
                        "m",
                        ...
                        now,
                        now);
        when(generations.listConversation("c4", new TenantId(7), 1000))
                .thenReturn(List.of(ignoredFailed, ignoredNoSpeaker));

        ChatProductController.TurnRunRequest missingContent =
                new ChatProductController.TurnRunRequest();
        ...
        explicit.conversationId = "c4";
        explicit.content = "hello";
        explicit.platform = "ANTHROPIC";
        explicit.model = "claude";
        ConversationMessage input =
                new ConversationMessage(
                        "m4",
                        "c4",
                        ...
                        null,
                        MessageRole.USER,
                        List.of(ContentPart.text("hello")),
                        Map.of(),
                        MessageStatus.COMPLETED,
                        0,
                        null,
        ...
        when(conversationService.createGeneration(
                        conversation, input, "ANTHROPIC", "claude", "ava"))
                .thenReturn(run);
        assertEquals("run4", controller.runTurn("g4", explicit).getData().generationId());
        verify(generationRunner).start(run, new TenantId(7), 8);

        when(conversationService.appendUserMessage(conversation, "boom"))
        ...
        when(conversations.findConversation("c4", new TenantId(7), 8))
                .thenAnswer(
                        invocation ->
                                latestLookup.getAndIncrement() == 0
                                        ? Optional.of(conversation)
                                        : Optional.empty());
        when(conversationService.appendUserMessage(conversation, "missing latest"))
                .thenReturn(input);
        ...
        when(conversationService.createGeneration(
                ...
                .thenReturn(run);
        doThrow(
                        new com.shiyu.ai.conversation.implementation.application
                                .GenerationAdmissionException("QUOTA_EXCEEDED"))
                .when(generationRunner)
                .start(run, new TenantId(7), 8);
