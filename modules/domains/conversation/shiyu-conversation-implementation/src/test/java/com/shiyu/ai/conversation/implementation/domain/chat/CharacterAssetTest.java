...
/**
 * 验证 Character Asset 相关功能、边界条件、异常路径和协作行为。
 */
class CharacterAssetTest {
    ...
    void pngBytesAreDefensivelyCopiedAndVisibilityIsValidated() {
        ...
        CharacterAsset asset =
                new CharacterAsset(
                        "id", 1, 2, card, "PUBLIC", original, Instant.now(), Instant.now());
        original[0] = 9;
        assertArrayEquals(new byte[] {1, 2, 3}, asset.pngData());
        byte[] returned = asset.pngData();
        returned[1] = 9;
        assertArrayEquals(new byte[] {1, 2, 3}, asset.pngData());
        assertThrows(
                ...
                () ->
                        new CharacterAsset(
                                "id", 1, 2, card, "UNKNOWN", null, Instant.now(), Instant.now()));
    }
}
