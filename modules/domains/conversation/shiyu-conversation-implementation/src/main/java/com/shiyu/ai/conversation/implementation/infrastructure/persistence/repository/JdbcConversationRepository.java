...
public class JdbcConversationRepository implements ConversationRepository {
    ...
    public List<Conversation> listConversations(TenantId tenantId, long ownerUserId, int limit, int offset) {
        return jdbc.query("SELECT * FROM CHAT_CONVERSATION WHERE TENANT_ID=? AND OWNER_USER_ID=? AND STATUS<>'DELETED' ORDER BY UPDATED_AT DESC,ID ASC LIMIT ? OFFSET ?", this::mapConversation, tenantId.value(), ownerUserId, Math.min(Math.max(limit, 1), 100), Math.max(offset, 0));
    }

    @Override public List<Conversation> listBranches(String parentConversationId, TenantId tenantId, long ownerUserId) {
        return jdbc.query("SELECT * FROM CHAT_CONVERSATION WHERE PARENT_CONVERSATION_ID=? AND TENANT_ID=? AND OWNER_USER_ID=? AND STATUS<>'DELETED' ORDER BY UPDATED_AT DESC,ID ASC", this::mapConversation, parentConversationId, tenantId.value(), ownerUserId);
    }
    ...
    public List<ConversationMessage> listMessages(String conversationId, TenantId tenantId, long ownerUserId, int limit) {
        return jdbc.query("SELECT m.* FROM CHAT_MESSAGE m JOIN CHAT_CONVERSATION c ON c.ID=m.CONVERSATION_ID WHERE m.CONVERSATION_ID=? AND m.TENANT_ID=? AND c.OWNER_USER_ID=? ORDER BY m.SEQUENCE DESC,m.ID ASC LIMIT ?", this::mapMessage, conversationId, tenantId.value(), ownerUserId, Math.min(Math.max(limit, 1), 1000));
    }
