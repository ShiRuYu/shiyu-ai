...
public class GenerationController {
    ...
    @Autowired public GenerationController(GenerationRepository generations, GenerationAdmission admission, AiRuntimePort runtime) { this.generations = generations; this.admission = admission; this.runtime = runtime; }
    ...
    public Flux<ServerSentEvent<GenerationEvent>> stream(@PathVariable String id,
        ...
        return Flux.fromIterable(generations.listEvents(id, tenant(), cursor, 1000)).map(e -> ServerSentEvent.<GenerationEvent>builder().id(String.valueOf(e.sequence())).event(e.type().name()).data(e).build());
    }
    ...
    private Flux<ServerSentEvent<GenerationEvent>> runtimeEventStream(AiRun run, long afterSeq, boolean follow, int waitMs) {
        ...
        return Flux.interval(Duration.ZERO, Duration.ofMillis(500))
                .concatMap(tick -> Flux.defer(() -> {
                    List<com.shiyu.ai.agent.contract.runtime.AiRunEvent> events = runtime.events(run.id(), tenantId, ownerUserId, cursor.get(), 1000);
    ...
    @PostMapping("/{id}/cancel") public Result<Void> cancel(@PathVariable String id) {
        ...
        if (runtime == null) {
            generations.appendEvent(new GenerationEvent(g.id(), generations.nextEventSequence(g.id(), tenant()), GenerationEventType.CANCELLED, "{}", java.time.Instant.now()), tenant());
        }
        if (runtime != null) {
            AiRun run = runtime.requireGenerationRun(id, tenant(), user());
            runtime.finish(run.id(), tenant(), user(), com.shiyu.ai.agent.contract.runtime.AiRunStatus.CANCELLED, "CLIENT_CANCELLED");
        }
    ...
    private ActorContext actor(){return ActorContextHttpAdapter.currentActor();}
