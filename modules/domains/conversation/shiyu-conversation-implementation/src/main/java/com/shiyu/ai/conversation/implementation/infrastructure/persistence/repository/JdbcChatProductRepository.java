...
public class JdbcChatProductRepository implements ChatProductRepository {
    ...
    @Override public List<CharacterAsset> listCharacters(TenantId tenant, long owner) {
        long value = tenant(tenant);
        return jdbc.query("SELECT * FROM CHAT_CHARACTER_ASSET WHERE TENANT_ID=? AND OWNER_USER_ID=? ORDER BY UPDATED_AT DESC,ID ASC", (r,n) -> character(r, value, owner), value, owner);
    }
    ...
    @Override public List<GroupChatAsset> listGroups(TenantId tenant, long owner) {
        long value = tenant(tenant);
        return jdbc.query("SELECT * FROM CHAT_GROUP_CHAT WHERE TENANT_ID=? AND OWNER_USER_ID=? ORDER BY UPDATED_AT DESC,ID ASC", (r,n) -> group(r, value, owner), value, owner);
    }
