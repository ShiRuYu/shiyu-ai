...
public class ChatProductController {
    ...
    public Result<GroupTurnRun> runTurn(@PathVariable String id, @RequestBody TurnRunRequest request) {
        ...
        List<com.shiyu.ai.conversation.contract.model.GenerationRun> priorRuns = generationRepository.listConversation(request.conversationId, new com.shiyu.ai.kernel.context.TenantId(tenant()), 1000);
        List<String> completedSpeakers = priorRuns.stream()
                .filter(run -> run.status() == com.shiyu.ai.conversation.contract.model.GenerationStatus.COMPLETED && run.speakerId() != null)
                .map(com.shiyu.ai.conversation.contract.model.GenerationRun::speakerId).toList();
        int consumedTokens = priorRuns.stream()
                .filter(run -> run.status() == com.shiyu.ai.conversation.contract.model.GenerationStatus.COMPLETED && run.speakerId() != null)
                .mapToLong(com.shiyu.ai.conversation.contract.model.GenerationRun::completionTokens).mapToInt(value -> (int) Math.min(Integer.MAX_VALUE, Math.max(0, value))).sum();
