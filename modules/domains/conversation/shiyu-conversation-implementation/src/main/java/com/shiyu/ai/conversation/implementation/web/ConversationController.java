...
public class ConversationController {
    ...
    private Result<GenerationRun> messageInternal(String id, String idempotencyKey, MessageRequest request) {
        ...
        catch (com.shiyu.ai.conversation.implementation.application.GenerationAdmissionException denied) {
    ...
    public Object export(@PathVariable String id, @RequestParam(required = false) String format) {
        ...
        if ("jsonl".equalsIgnoreCase(format)) return org.springframework.http.ResponseEntity.ok().contentType(MediaType.TEXT_PLAIN).body(com.shiyu.ai.conversation.implementation.domain.chat.ConversationExchangeCodec.toJsonl(messages));
        if ("markdown".equalsIgnoreCase(format) || "md".equalsIgnoreCase(format)) return org.springframework.http.ResponseEntity.ok().contentType(MediaType.TEXT_MARKDOWN).body(com.shiyu.ai.conversation.implementation.domain.chat.ConversationExchangeCodec.toMarkdown(messages));
    ...
    public Result<Conversation> importConversation(@RequestBody ImportRequest request) {
        ...
        List<com.shiyu.ai.conversation.implementation.domain.chat.ConversationExchangeCodec.ImportedMessage> imported = importPreviews.consume(
    ...
    public Result<ConversationImportPreviewStore.Preview> importPreview(@RequestBody ImportRequest request) {
        ...
        List<com.shiyu.ai.conversation.implementation.domain.chat.ConversationExchangeCodec.ImportedMessage> imported = "markdown".equals(format) || "md".equals(format)
                ? com.shiyu.ai.conversation.implementation.domain.chat.ConversationExchangeCodec.fromMarkdown(request.content)
                : com.shiyu.ai.conversation.implementation.domain.chat.ConversationExchangeCodec.fromJsonl(request.content);
    ...
    public record PromptPreview(List<ConversationMessage> messages, List<PromptSegment> sources, long estimatedTokens,
                                ...
                                String promptHash, List<ContextItem> contextItems, com.shiyu.ai.agent.contract.runtime.ContextTrace contextTrace) {
