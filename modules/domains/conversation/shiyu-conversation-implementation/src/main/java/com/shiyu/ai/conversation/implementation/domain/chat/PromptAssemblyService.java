...
public class PromptAssemblyService {
    ...
    private ConversationMessage system(String source, String text) {
        return new ConversationMessage(source, "prompt-preview", null, null, MessageRole.SYSTEM, List.of(ContentPart.text(text)), java.util.Map.of(), com.shiyu.ai.conversation.implementation.domain.model.MessageStatus.COMPLETED, 0, null, java.time.Instant.now(), java.time.Instant.now());
    }
}
