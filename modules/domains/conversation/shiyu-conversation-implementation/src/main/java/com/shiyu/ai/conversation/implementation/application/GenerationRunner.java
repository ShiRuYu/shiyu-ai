...
public class GenerationRunner {
    ...
    public GenerationRunner(
        ...
        this(
                ...
                generations,
                conversations,
                new GenerationUsageSink() {},
                new GenerationAdmission() {},
                null,
                null);
    }
    ...
    public void start(GenerationRun created, TenantId tenantId, long ownerUserId) {
        ...
        ConversationMessage input =
                conversations
                        .findMessage(created.inputMessageId(), tenantId, ownerUserId)
                        .orElseThrow(() -> new IllegalArgumentException("input message not found"));
        Conversation conversation =
                conversations
                        .findConversation(input.conversationId(), tenantId, ownerUserId)
        ...
        ConversationPromptService.PromptAssembly assembly =
                promptService.assemble(
                        conversation,
                        conversations
                                .listMessages(conversation.id(), tenantId, ownerUserId, 1000)
                                .reversed(),
                        tenantId,
                        ownerUserId);
        ...
        if (runtime != null) {
            try {
                ...
                runtime.append(
                        runtimeState.get(),
                        AiRunEventType.PROMPT_ASSEMBLED,
    ...
    private void onEvent(
        ...
        try {
            eventType = GenerationEventType.valueOf(type);
        } catch (IllegalArgumentException ex) {
            eventType = GenerationEventType.DELTA;
        }
        if (eventType == GenerationEventType.DELTA && value.getContent() != null)
            answer.append(value.getContent());
        String payload =
                switch (eventType) {
                    ...
                    case TOOL_CALL ->
                            JSONUtils.toJsonString(
                                    java.util.Map.of(
                                            ...
                                            value.getToolName(),
                                            "arguments",
                                            Objects.toString(value.getToolArguments(), "{}")));
        ...
        if (runtime != null && runtimeState.get() != null) {
            ...
            runtime.append(
                    ...
                    "turn-" + state.get().id(),
                    "model-" + state.get().id(),
                    value.getProviderRequestId());
        }
        if (eventType == GenerationEventType.USAGE) {
            GenerationRun before = state.get();
            GenerationRun usage =
                    withUsageAndSequence(before, value, sequence, before.version() + 1);
            ...
            if (runtime != null && runtimeState.get() != null) {
                runtime.recordUsage(
                        ...
                        ownerUserId,
                        value.getPromptTokens() == null
                                ? usage.promptTokens()
                                : value.getPromptTokens(),
                        value.getCompletionTokens() == null
                                ? usage.completionTokens()
                                : value.getCompletionTokens(),
                        value.isEstimatedUsage(),
                        null);
            }
        } else {
    ...
    private void finishProviderTerminal(
            AtomicReference<GenerationRun> state,
            AtomicReference<AiRun> runtimeState,
            long tenantId,
            long ownerUserId,
        ...
        next =
                new GenerationRun(
                        ...
                        terminal == GenerationStatus.FAILED
                                ? Objects.toString(errorCode, "provider_failed")
                                : null,
    ...
    private void finishSuccess(
        ...
        ConversationMessage input =
                conversations
                        .findMessage(current.inputMessageId(), new TenantId(tenantId), ownerUserId)
                        .orElseThrow();
        Conversation conversation =
                conversations
                        .findConversation(
                                input.conversationId(), new TenantId(tenantId), ownerUserId)
                        .orElseThrow();
        ConversationMessage assistant =
                new ConversationMessage(
                        ...
                        List.of(ContentPart.text(answer)),
                        java.util.Map.of(),
        ...
        if (conversations.updateConversation(updated, conversation.version()) != 1) {
            ...
            finishFailure(
                    ...
                    ownerUserId,
                    nextSequence,
                    new IllegalStateException("conversation was modified"));
            return;
        }
        int sequence = nextSequence.getAndIncrement();
        ...
        completed =
                new GenerationRun(
                        ...
                        Math.max(completed.completionTokens(), Math.max(1, answer.length() / 4)),
                        java.time.Duration.between(current.createdAt(), Instant.now()).toMillis(),
                        null,
                        sequence,
                        false,
        ...
        if (generations.update(completed, current.version()) == 1) {
            ...
            if (runtime != null && runtimeState.get() != null) {
                runtime.append(
                        runtimeState.get(),
                        AiRunEventType.MODEL_COMPLETED,
                        "{}",
                        true,
                        "turn-" + completed.id(),
                        "model-" + completed.id(),
    ...
    private void finishFailure(
        ...
        GenerationRun persisted =
                generations.find(current.id(), new TenantId(tenantId), ownerUserId).orElse(current);
    ...
    private void appendProjection(
        ...
        generations.appendEvent(
                new GenerationEvent(run.id(), sequence, type, payload, Instant.now()),
                new TenantId(tenantId));
    }

    private GenerationRun withUsageAndSequence(
            GenerationRun s, ChatResponse value, int sequence, long version) {
