...
public final class RedisRateLimitStore implements RateLimitStore {
    ...
    public RedisRateLimitStore(StringRedisTemplate redis, RedisInfrastructureProperties properties) {
        this.redis = redis;
    ...
    public boolean consume(String key, long permits, long limit, Duration window) {
        if (key == null || key.isBlank() || permits <= 0 || limit <= 0 || window.isZero() || window.isNegative()) {
            return false;
        }
        long windowMillis = Math.max(1L, window.toMillis());
        Long result = redis.execute(CONSUME,
                List.of(properties.key("rate", key)),
                String.valueOf(permits), String.valueOf(limit), String.valueOf(windowMillis));
