...
public class MicrometerExecutorBinder implements MeterBinder {
    ...
    private void bindThreadPoolExecutor(ThreadPoolExecutor executor, MeterRegistry registry) {
        ...
        // 队列剩余容量
        Gauge.builder(
                        prefix + ".queue.remaining.capacity",
                        executor,
        ...
        Gauge.builder(
                        prefix + ".task.count",
                        executor,
                        e -> e.getCompletedTaskCount() + e.getActiveCount())
                .description("线程池总任务数")
                .register(registry);
    }
     ...
     * @return 带指标收集的执行器
     */
    public static Executor wrapWithMetrics(
            ExecutorService executor, String name, MeterRegistry registry) {
