...
/**
 * {@code TextbookRepositoryImpl} 实现教育模块的持久化端口，负责在领域对象与存储模型之间转换。
 */
@Component
public class TextbookRepositoryImpl
        implements com.shiyu.ai.education.implementation.domain.port.repository.TextbookRepository {

    /**
     * textbookMapper 属性，保存当前对象中的业务数据或协作依赖。
     */
    @Resource private TextbookMapper textbookMapper;

    public TextbookBO selectById(TenantId tenantId, Long id) {
        return MapstructUtils.convert(
                textbookMapper.selectOneByQuery(
                        QueryWrapper.create()
                                .eq(TextbookDO::getTenantId, tenantId.value())
                                .eq(TextbookDO::getId, id)),
                TextbookBO.class);
    }

    public PageData<TextbookBO> selectPage(TenantId tenantId, int pageNum, int pageSize) {
        com.mybatisflex.core.paginate.Page<TextbookDO> page =
                textbookMapper.paginate(
                        pageNum,
                        pageSize,
                        QueryWrapper.create()
                                .eq(TextbookDO::getTenantId, tenantId.value())
                                .orderBy("id", false));
        return new PageData<>(
                MapstructUtils.convert(page.getRecords(), TextbookBO.class), page.getTotalRow());
    }

    public List<TextbookBO> selectBySubjectAndGrade(
            TenantId tenantId, String subjectCode, Integer grade) {
        return MapstructUtils.convert(
                textbookMapper.selectListByQuery(
                        QueryWrapper.create()
                                .eq(TextbookDO::getTenantId, tenantId.value())
    ...
    public int update(TenantId tenantId, TextbookBO entity) {
        ...
        return EducationWriteGuard.require(
                textbookMapper.updateByQuery(
                        dataObj,
                        QueryWrapper.create()
                                .eq(TextbookDO::getTenantId, tenantId.value())
                                .eq(TextbookDO::getId, entity.getId())),
                "update textbook");
    }

    public int deleteById(TenantId tenantId, Long id) {
        return EducationWriteGuard.require(
                textbookMapper.deleteByQuery(
                        QueryWrapper.create()
                                .eq(TextbookDO::getTenantId, tenantId.value())
                                .eq(TextbookDO::getId, id)),
                "delete textbook");
