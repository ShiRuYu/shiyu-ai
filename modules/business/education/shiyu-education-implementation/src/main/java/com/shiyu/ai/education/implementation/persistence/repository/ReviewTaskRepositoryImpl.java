...
public class ReviewTaskRepositoryImpl
    ...
    /**
     * reviewTaskMapper 属性，保存当前对象中的业务数据或协作依赖。
     */
    @Resource private ReviewTaskMapper reviewTaskMapper;

    public ReviewTaskBO selectById(TenantId tenantId, Long id) {
        return MapstructUtils.convert(
                reviewTaskMapper.selectOneByQuery(
    ...
    public List<ReviewTaskBO> selectTodayTasks(TenantId tenantId, Long studentId) {
        return MapstructUtils.convert(
                reviewTaskMapper.selectListByQuery(
                        QueryWrapper.create()
    ...
    public List<ReviewTaskBO> selectByStudentAndStatus(
            TenantId tenantId, Long studentId, Integer status) {
        return MapstructUtils.convert(
                reviewTaskMapper.selectListByQuery(
                        QueryWrapper.create()
                                .eq("tenant_id", tenantId.value())
    ...
    public List<ReviewTaskBO> selectByStudentAndKnowledge(
    ...
    public int insert(TenantId tenantId, ReviewTaskBO entity) {
        ...
        int rows =
                EducationWriteGuard.require(reviewTaskMapper.insert(dataObj), "insert review task");
    ...
    public int update(TenantId tenantId, ReviewTaskBO entity) {
        ...
        return EducationWriteGuard.require(
                reviewTaskMapper.updateByQuery(
                        dataObj,
                        QueryWrapper.create()
                                .eq("tenant_id", tenantId.value())
                                .eq("id", entity.getId())),
    ...
    public int deleteById(TenantId tenantId, Long id) {
        return EducationWriteGuard.require(
                reviewTaskMapper.deleteByQuery(
                        QueryWrapper.create().eq("tenant_id", tenantId.value()).eq("id", id)),
                "delete review task");
    }
}
