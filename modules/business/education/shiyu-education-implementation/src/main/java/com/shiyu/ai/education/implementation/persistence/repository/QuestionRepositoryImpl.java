...
/**
 * {@code QuestionRepositoryImpl} 实现教育模块的持久化端口，负责在领域对象与存储模型之间转换。
 */
@Component
public class QuestionRepositoryImpl
        implements com.shiyu.ai.education.implementation.domain.port.repository.QuestionRepository {
    ...
    public QuestionBO selectById(TenantId tenantId, Long id) {
        return MapstructUtils.convert(
                questionMapper.selectOneByQuery(
    ...
    public PageData<QuestionBO> selectPage(TenantId tenantId, int pageNum, int pageSize) {
        com.mybatisflex.core.paginate.Page<QuestionDO> page =
                questionMapper.paginate(
                        pageNum,
                        pageSize,
                        QueryWrapper.create()
                                ...
                                .orderBy("id", false));
        return new PageData<>(
                MapstructUtils.convert(page.getRecords(), QuestionBO.class), page.getTotalRow());
    }

    public List<QuestionBO> selectBySubjectAndGrade(
    ...
    public List<QuestionBO> selectByType(TenantId tenantId, String type) {
        return MapstructUtils.convert(
                questionMapper.selectListByQuery(
                        QueryWrapper.create().eq("tenant_id", tenantId.value()).eq("type", type)),
                QuestionBO.class);
    }
    ...
    public QuestionBO selectByCode(TenantId tenantId, String code) {
        return MapstructUtils.convert(
                questionMapper.selectOneByQuery(
                        QueryWrapper.create().eq("tenant_id", tenantId.value()).eq("code", code)),
                QuestionBO.class);
    }
