...
/**
 * {@code KnowledgeTextbookRepositoryImpl} 实现教育模块的持久化端口，负责在领域对象与存储模型之间转换。
 */
@Component
public class KnowledgeTextbookRepositoryImpl
        implements com.shiyu.ai.education.implementation.domain.port.repository
                .KnowledgeTextbookRepository {

    /**
     * 映射器，表示当前对象中的对应属性。
     */
    @Resource private KnowledgeTextbookMapper mapper;
    ...
    public void deleteById(TenantId tenantId, Long id) {
        EducationWriteGuard.require(
                mapper.deleteByQuery(
                        QueryWrapper.create()
                                .eq(KnowledgeTextbookDO::getTenantId, tenantId.value())
                                .eq(KnowledgeTextbookDO::getId, id)),
