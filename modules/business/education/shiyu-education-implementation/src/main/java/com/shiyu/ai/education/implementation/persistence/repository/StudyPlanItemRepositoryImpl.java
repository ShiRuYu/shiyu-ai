...
/**
 * {@code StudyPlanItemRepositoryImpl} 实现教育模块的持久化端口，负责在领域对象与存储模型之间转换。
 */
@Component
public class StudyPlanItemRepositoryImpl
        implements com.shiyu.ai.education.implementation.domain.port.repository
                .StudyPlanItemRepository {

    /**
     * studyPlanItemMapper 属性，保存当前对象中的业务数据或协作依赖。
     */
    @Resource private StudyPlanItemMapper studyPlanItemMapper;

    public List<StudyPlanItemBO> selectByPlanId(TenantId tenantId, Long planId) {
        return MapstructUtils.convert(
                studyPlanItemMapper.selectListByQuery(
                        QueryWrapper.create()
        ...
        for (StudyPlanItemBO item : items) {
            ...
            count +=
                    EducationWriteGuard.require(
                            studyPlanItemMapper.insert(dataObj), "insert study plan item");
        }
        return count;
    }
}
