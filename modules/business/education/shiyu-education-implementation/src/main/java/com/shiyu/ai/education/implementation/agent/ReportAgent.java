...
public class ReportAgent {
    ...
    /**
     * studyRecordRepository 属性，保存当前对象中的业务数据或协作依赖。
     */
     ...
     * @return 报告内容
     */
    public String generateOverviewReport(ActorContext actor, Long studentId) {
        log.info("ReportAgent.generateOverviewReport: studentIdPresent={}", studentId != null);
        ...
        if (!resp.isSuccess()) {
            log.error(
                    "ReportAgent: LLM 报告生成失败: errorMessageLength={}",
                    resp.getErrorMessage() == null ? 0 : resp.getErrorMessage().length());
            return "报告生成失败，请稍后重试。\n统计数据：\n"
                    + "- 学习次数："
                    + totalStudySessions
                    + "\n"
    ...
    private String buildReportPrompt(
            Long studentId, int totalSessions, double avgScore, long totalDurationSec) {
