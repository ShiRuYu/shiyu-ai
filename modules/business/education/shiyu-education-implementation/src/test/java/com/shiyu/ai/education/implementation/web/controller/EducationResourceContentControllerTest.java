...
class EducationResourceContentControllerTest {
    ...
    void rejectsUnsafePathsAndUsesSafeMediaTypeFallback() throws Exception {
        ...
        EducationResourceContentController controller =
                new EducationResourceContentController(storage);
        try (MockedStatic<ActorContextHttpAdapter> actor =
            ...
            StoredFile noType =
                    new StoredFile(
                            "no-type", "no-type.txt", 1, null, Instant.now(), null, "memory");
            when(storage.list("tenant/7/education-resources")).thenReturn(List.of(noType));
            when(storage.open("no-type"))
                    .thenReturn(
                            new StorageObject(
                                    new ByteArrayInputStream(new byte[] {1}),
                                    "no-type.txt",
