...
class ReviewServiceImplTest {
    ...
    void listsTasksWithActorTenant() {
        ...
        when(repository.selectByStudentAndStatus(ACTOR.tenantId(), 10L, 0))
            ...
            mapper.when(
                            () ->
                                    MapstructUtils.convert(
                                            any(ReviewTaskBO.class), eq(ReviewTaskResponse.class)))
                    .thenReturn(response);
            mapper.when(() -> MapstructUtils.convert(any(List.class), eq(ReviewTaskResponse.class)))
    ...
    void createsUpdatesCompletesAndDeletesTasks() {
        ...
        ReviewTaskBO task = new ReviewTaskBO();
        task.setId(1L);
        when(repository.selectById(ACTOR.tenantId(), 1L))
                .thenReturn(task)
                .thenReturn(task)
                .thenReturn(null);
        request.setId(1L);
        ...
        explicit.setStudentId(12L);
        explicit.setKnowledgeId(22L);
        explicit.setReviewDate(LocalDate.of(2025, 1, 2));
        explicit.setReviewRound(3);
        explicit.setStatus(2);
        explicit.setResultScore(77.0);
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(
                            () ->
                                    MapstructUtils.convert(
        ...
        allFields.setId(1L);
        allFields.setStudentId(13L);
        allFields.setKnowledgeId(23L);
        allFields.setStatus(1);
        allFields.setReviewRound(4);
        allFields.setReviewDate(LocalDate.of(2025, 1, 3));
        allFields.setResultScore(80.0);
        when(repository.selectById(ACTOR.tenantId(), 1L)).thenReturn(task);
        ...
        ReviewRequest missing = new ReviewRequest();
        missing.setId(99L);
