...
class QuestionAndRecommendationServiceTest {
    private static final ActorContext ACTOR =
            new ActorContext(new TenantId(31), new UserId(32), false);
    ...
    void questionServiceScopesAllReadsAndWritesToTenant() {
        ...
        QuestionResponse response =
                new QuestionResponse(
                        9L, "q", "SINGLE", "math", 5, 2, null, "title", null, "A", null, null, 0L);
        ...
        when(repository.selectBySubjectAndGrade(ACTOR.tenantId(), "math", 5))
        ...
        when(repository.selectPage(ACTOR.tenantId(), 1, 10))
                .thenReturn(new PageData<>(List.of(question), 1));
        try (MockedStatic<MapstructUtils> mapper = mockStatic(MapstructUtils.class)) {
            mapper.when(
                            () ->
                                    MapstructUtils.convert(
                                            any(QuestionBO.class), eq(QuestionResponse.class)))
                    .thenReturn(response);
            mapper.when(() -> MapstructUtils.convert(any(List.class), eq(QuestionResponse.class)))
        ...
        service.update(ACTOR, request);
        service.incrementUsedCount(ACTOR, 9L);
        service.deleteById(ACTOR, 9L);
    ...
    void recommendationServiceRanksWeakKnowledgeQuestionsAndReviews() {
        ...
        RecommendationServiceImpl service =
                new RecommendationServiceImpl(abilities, questions, reviews);
        AbilityBO weak = new AbilityBO();
        weak.setKnowledgeId(101L);
        weak.setOverallMastery(30D);
        AbilityBO medium = new AbilityBO();
        medium.setKnowledgeId(102L);
        medium.setOverallMastery(50D);
        AbilityBO strong = new AbilityBO();
        strong.setKnowledgeId(103L);
        strong.setOverallMastery(80D);
        AbilityBO unknown = new AbilityBO();
        unknown.setKnowledgeId(104L);
        unknown.setOverallMastery(null);
        when(abilities.selectByStudent(ACTOR.tenantId(), 7L))
                .thenReturn(List.of(strong, medium, weak, unknown));
        QuestionResponse question =
                new QuestionResponse(
                        1L, "q", "SINGLE", "math", 5, 1, null, "title", null, "A", null, null, 0L);
        ...
        ReviewTaskResponse review =
                new ReviewTaskResponse(
                        1L, 7L, 101L, "math", 1, "today", 0, "due", null, LocalDateTime.now());
        when(reviews.listTodayTasks(ACTOR, 7L)).thenReturn(List.of(review));

        assertEquals(
                List.of(weak, medium).stream().map(AbilityBO::getKnowledgeId).toList(),
                service.recommendKnowledge(ACTOR, 7L, 5).stream()
                        .map(x -> x.knowledgeId())
                        .toList());
