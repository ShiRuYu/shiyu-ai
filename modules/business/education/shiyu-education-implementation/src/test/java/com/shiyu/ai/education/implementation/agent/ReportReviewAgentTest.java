...
/**
 * 验证 Report 复习 智能体 相关功能、边界条件、异常路径和协作行为。
 */
class ReportReviewAgentTest {
    ...
    void generatesSuccessfulAndFallbackLearningReports() {
        ...
        StudyRecordBO first = new StudyRecordBO();
        first.setScore(80d);
        first.setDurationSec(120);
        StudyRecordBO second = new StudyRecordBO();
        second.setScore(null);
        second.setDurationSec(null);
        when(records.selectByStudent(ACTOR.tenantId(), 11L)).thenReturn(List.of(first, second));
        when(chat.chat(any()))
                .thenReturn(ChatResponse.builder().success(true).content("report").build());
        ReportAgent agent =
                new ReportAgent(
                        chat, mock(AnalyticsService.class), records, mock(AbilityService.class));
    ...
    void schedulesCompletesAndFiltersOverdueReviewTasks() {
        ...
        when(scheduler.scheduleAfterLearning(eq(11L), eq(22L), any(Instant.class)))
                .thenReturn(
                        List.of(
                                new ReviewScheduler.ReviewTask(
                                        11L, 22L, LocalDate.now().minusDays(1), 1),
                                new ReviewScheduler.ReviewTask(
                                        11L, 22L, LocalDate.now().plusDays(2), 2)));
        ...
        task.setId(3L);
        task.setStatus(ReviewTaskStatus.PENDING.getCode());
        ...
        ReviewTaskBO overdue = new ReviewTaskBO();
        overdue.setReviewDate(LocalDate.now().minusDays(1));
        ReviewTaskBO future = new ReviewTaskBO();
        future.setReviewDate(LocalDate.now().plusDays(1));
        ReviewTaskBO undated = new ReviewTaskBO();
        when(tasks.selectByStudentAndStatus(
                        ACTOR.tenantId(), 11L, ReviewTaskStatus.PENDING.getCode()))
