...
负责 Spring Boot 启动、provider 校验、运行时锁、保留策略、启动事件和应用集成测试。

## 职责

- 负责 Spring Boot 启动、provider 校验、运行时锁、保留策略、启动事件和应用集成测试。
- 通过 `application-*.yml` 装配数据库、文件、向量、Redis 和事件 provider；外部地址、账号和密钥只从环境变量或密钥管理系统读取。
- PostgreSQL/MySQL 只在外部基线完成后启动；H2 由应用初始化器安装 v4 最终基线。
