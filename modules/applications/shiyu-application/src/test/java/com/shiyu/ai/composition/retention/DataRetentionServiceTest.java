...
class DataRetentionServiceTest {
    ...
    void removesOnlyExpiredOperationalRecordsOnH2() {
        ...
        jdbcTemplate.update(
                "INSERT INTO knowledge_ingestion_job (create_time, job_status) VALUES (?, ?)",
                Timestamp.from(old),
                "SUCCEEDED");
        jdbcTemplate.update(
                "INSERT INTO knowledge_ingestion_job (create_time, job_status) VALUES (?, ?)",
        ...
        assertEquals(
                ...
                jdbcTemplate.queryForObject(
                        "SELECT COUNT(*) FROM knowledge_ingestion_job WHERE job_status = 'RUNNING'",
                        Integer.class));
    }
    ...
    void doesNotReferenceRetiredMemoryEpisodicTable() {
        ...
        verify(jdbcTemplate, never())
                .update(contains("memory_episodic_memory"), any(Timestamp.class));
    }
    ...
    private void createOperationalTables(JdbcTemplate jdbcTemplate) {
        for (String table :
            ...
            jdbcTemplate.execute(
                    "CREATE TABLE "
                            + table
                            + " (id BIGINT AUTO_INCREMENT PRIMARY KEY, create_time TIMESTAMP)");
        }
        jdbcTemplate.execute(
                "CREATE TABLE knowledge_ingestion_job (id BIGINT AUTO_INCREMENT PRIMARY KEY, "
    ...
    private void insertTimestamped(JdbcTemplate jdbcTemplate, String table, Instant createTime) {
        jdbcTemplate.update(
                "INSERT INTO " + table + " (create_time) VALUES (?)", Timestamp.from(createTime));
    }
