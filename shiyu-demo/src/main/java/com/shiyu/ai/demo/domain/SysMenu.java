...
public class SysMenu extends BaseEntity {
    ...
    public String getRouterPath() {
        ...
        // 非外链并且是一级目录（类型为目录）
        if (0L == getParentId() && menuType.isDirectory()
                && !getFrame()) {
    ...
    public String getComponentInfo() {
        String component = ComponentTypeEnum.LAYOUT.getValue();
        if (StringUtils.isNotEmpty(this.component) && !isMenuFrame()) {
            component = this.component;
        } else if (StringUtils.isEmpty(this.component) && getParentId() != 0L && isInnerLink()) {
            component = ComponentTypeEnum.INNER_LINK.getValue();
        } else if (StringUtils.isEmpty(this.component) && isParentView()) {
            component = ComponentTypeEnum.PARENT_VIEW.getValue();
        }
        return component;
    }
    ...
    public boolean isMenuFrame() {
        return getParentId() == 0L && menuType.isMenu() && !getFrame();
    }
    ...
    public boolean isInnerLink() {
        return !getFrame() && StringUtils.isHttp(path);
    }
    ...
    public boolean isParentView() {
        return getParentId() != 0L && menuType.isDirectory();
    }
