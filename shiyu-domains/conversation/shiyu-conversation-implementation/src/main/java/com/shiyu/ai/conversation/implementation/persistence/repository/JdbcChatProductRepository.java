...
public class JdbcChatProductRepository implements ChatProductRepository {
    ...
    @Override public Optional<CharacterAsset> findCharacter(TenantId tenant, long owner, String id) {
        long value = tenant(tenant);
        return jdbc.query("SELECT * FROM CHAT_CHARACTER_ASSET WHERE TENANT_ID=? AND OWNER_USER_ID=? AND ID=?", (r,n) -> character(r, value, owner), value, owner, id).stream().findFirst();
    }
    @Override public Optional<CharacterAsset> findCharacterForAccess(TenantId tenant, long requester, String id) {
        long value = tenant(tenant);
        return jdbc.query("SELECT * FROM CHAT_CHARACTER_ASSET WHERE TENANT_ID=? AND ID=? AND (OWNER_USER_ID=? OR VISIBILITY IN ('PUBLIC','TENANT'))", (r,n) -> character(r, value, r.getLong("OWNER_USER_ID")), value, id, requester).stream().findFirst();
    }
    @Override public List<CharacterAsset> listCharacters(TenantId tenant, long owner) {
        long value = tenant(tenant);
        return jdbc.query("SELECT * FROM CHAT_CHARACTER_ASSET WHERE TENANT_ID=? AND OWNER_USER_ID=? ORDER BY UPDATED_AT DESC", (r,n) -> character(r, value, owner), value, owner);
    }
    ...
    @Override public List<LorebookAsset> listLorebooks(TenantId tenant, long owner) { long value = tenant(tenant); return jdbc.query("SELECT * FROM CHAT_LOREBOOK_ASSET WHERE TENANT_ID=? AND OWNER_USER_ID=? ORDER BY UPDATED_AT DESC", (r,n) -> new LorebookAsset(r.getString("ID"), value, owner, JSONUtils.parseObject(r.getString("ENTRY_JSON"), LorebookEntry.class), instant(r.getTimestamp("CREATED_AT")), instant(r.getTimestamp("UPDATED_AT"))), value, owner); }
    @Override public void deleteLorebook(TenantId tenant, long owner, String id) { jdbc.update("DELETE FROM CHAT_LOREBOOK_ASSET WHERE TENANT_ID=? AND OWNER_USER_ID=? AND ID=?", tenant(tenant), owner, id); }

    @Override public PromptTemplateVersion savePrompt(PromptTemplateVersion version, TenantId tenant, long owner) {
        long value = tenant(tenant);
        jdbc.update("INSERT INTO CHAT_PROMPT_TEMPLATE (ID,TENANT_ID,OWNER_USER_ID,TEMPLATE_ID,VERSION,STATUS,BODY,VARIABLE_SCHEMA,TEST_CASES,CREATED_AT,PUBLISHED_AT) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                version.id(), value, owner, version.templateId(), version.version(), version.status(), version.body(), JSONUtils.toJsonString(version.variableSchema()), JSONUtils.toJsonString(version.testCases()), ts(version.createdAt()), version.publishedAt() == null ? null : ts(version.publishedAt())); return version;
    }
    @Override public List<PromptTemplateVersion> listPrompts(TenantId tenant, long owner, String templateId) {
        long value = tenant(tenant);
    ...
    @Override public Optional<GroupChatAsset> findGroup(TenantId tenant, long owner, String id) {
        long value = tenant(tenant);
        return jdbc.query("SELECT * FROM CHAT_GROUP_CHAT WHERE TENANT_ID=? AND OWNER_USER_ID=? AND ID=?", (r,n) -> group(r, value, owner), value, owner, id).stream().findFirst();
    }
    @Override public List<GroupChatAsset> listGroups(TenantId tenant, long owner) {
        long value = tenant(tenant);
        return jdbc.query("SELECT * FROM CHAT_GROUP_CHAT WHERE TENANT_ID=? AND OWNER_USER_ID=? ORDER BY UPDATED_AT DESC", (r,n) -> group(r, value, owner), value, owner);
    }
    @Override public void deleteGroup(TenantId tenant, long owner, String id) { jdbc.update("DELETE FROM CHAT_GROUP_CHAT WHERE TENANT_ID=? AND OWNER_USER_ID=? AND ID=?", tenant(tenant), owner, id); }
