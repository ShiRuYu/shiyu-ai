...
class ConversationPromptServiceTest {
    ...
    void previewAndExecutionAssemblyIncludeTheSameOptInContextMessage() {
        ...
        ContextAssemblyPort context = query -> new ContextAssemblyPort.ContextResult(List.of(item), null);
        ...
        ConversationPromptService.PromptAssembly assembly = service.assemble(conversation, List.of(user), new TenantId(1), 2);
    ...
    void ordinaryChatDoesNotCallContextProviders() {
        ContextAssemblyPort context = query -> { throw new AssertionError("provider must not be called for chat"); };
        ConversationPromptService service = new ConversationPromptService(context);
        ...
        assertTrue(service.assemble(conversation, List.of(user), new TenantId(1), 2).contextItems().isEmpty());
    }
}
