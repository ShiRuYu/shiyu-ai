...
public class MemoryController {
    ...
    @PostMapping("/admin/indexes/rebuild") public Result<Void> rebuild(@RequestParam String namespace) { index.rebuild(tenant(), namespace); return Result.success(); }

    private TenantId tenant() { return ActorContextHttpAdapter.currentActor().tenantId(); }
    private void enforceSubject(String subjectType, String subjectId) {
        long user = ActorContextHttpAdapter.userId();
        if (!ActorContextHttpAdapter.platformAdmin() && (!("USER".equalsIgnoreCase(subjectType) || "STUDENT".equalsIgnoreCase(subjectType)) || !String.valueOf(user).equals(subjectId))) {
