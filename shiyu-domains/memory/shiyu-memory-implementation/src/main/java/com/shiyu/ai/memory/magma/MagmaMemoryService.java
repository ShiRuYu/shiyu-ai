...
public class MagmaMemoryService implements MemoryIngestionPort, MemoryQueryPort, MemoryGovernancePort {
    ...
    public MemoryEvent ingest(IngestMemoryCommand command) {
        ...
        MemoryEvent event = new MemoryEvent(
                UUID.randomUUID().toString(), tenantId, command.namespace(),
        ...
        MemoryEvent previous = repository.findPreviousEvent(
                tenantId, command.namespace(), command.subjectType(), command.subjectId(), command.occurredAt()).orElse(null);
        MemoryEvent next = repository.findNextEvent(
                tenantId, command.namespace(), command.subjectType(), command.subjectId(), command.occurredAt()).orElse(null);
        repository.insertEvent(event);
        if (previous != null && !previous.id().equals(event.id())) {
            insertTemporalEdgeIfMissing(tenantId, previous.id(), event.id(), "after", event.sourceId(), now);
        }
        if (next != null && !next.id().equals(event.id())) {
            insertTemporalEdgeIfMissing(tenantId, event.id(), next.id(), "before", event.sourceId(), now);
        }
        ...
        try { repository.enqueueConsolidation(tenantId, event.id()); } catch (RuntimeException ex) { log.warn("memory consolidation enqueue failed: {}", ex.getMessage()); }
        return event;
    }

    private void insertTemporalEdgeIfMissing(TenantId tenantId, String sourceId, String targetId,
    ...
    public List<MemoryEdge> relations(TenantId tenantId, String eventId, GraphType graphType, int limit) {
        requireEvent(tenantId, eventId);
     ...
     */
    public MemoryRetrievalTrace trace(TenantId tenantId, String traceId, String subjectType, String subjectId) {
    ...
    public MemoryRetrievalResult retrieveWithTrace(MemoryQuery query) {
        ...
        repository.recordRetrievalTrace(new MemoryRetrievalTrace(traceId, tenantId, query.namespace(), query.text(),
    ...
    @Override
    public MemoryEvent supersede(TenantId tenantId, String oldEventId, IngestMemoryCommand replacement) {
