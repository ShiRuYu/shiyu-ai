...
public class UserServiceImpl implements UserService {
    ...
    private UserBO getUserDetail(ActorContext actor, Long userId) {
        log.info("获取用户详情，userId: {}", userId);
        if (userId == null || !userRepository.isUserInScope(userId, actor.tenantId())) {
            return null;
        }
    ...
    private boolean updateUser(ActorContext actor, Long userId, UserBO userBO, Long[] roleIds, Long targetTenantId) {
        ...
        if (updated && roleIds != null) {
            syncUserRoles(actor, userId, roleIds, targetTenantId);
        }
        return updated;
    }
    ...
    public String resetUserPassword(ActorContext actor, Long userId, String password) {
        ...
        if (!canManageUser(actor, userId, currentTenantId)) {
            log.warn("拒绝越权重置用户密码，userId={}, tenantId={}", userId, currentTenantId);
            return null;
        }
        UserBO userBO = userRepository.selectById(userId);
        if (userBO == null) {
    ...
    private Map<String, Object> createUser(ActorContext actor, UserBO userBO, Long[] roleIds, Long targetTenantId) {
        ...
        if (roleIds != null) {
            syncUserRoles(actor, createdUser.getId(), roleIds, targetTenantId);
        }
    ...
    private boolean canManageUser(ActorContext actor, Long userId, Long currentTenantId) {
        ...
        return actor.homeTenantId() != null
    ...
    private void syncUserRoles(ActorContext actor, Long userId, Long[] roleIds, Long requestedTenantId) {
        ...
        userScopeRoleRepository.deleteByUserIdAndTenantId(
                userId, new com.shiyu.ai.kernel.context.TenantId(targetTenantId));
    ...
    public boolean changePassword(ActorContext actor, Long userId, String oldPassword, String newPassword) {
        ...
        if (!currentUserId.equals(userId)) {
            log.warn("拒绝为其他用户修改密码，requestUserId={}, currentUserId={}", userId, currentUserId);
            return false;
        }
        UserBO userBO = userRepository.selectById(userId);
        if (userBO == null) {
    ...
    private Long requireTargetTenant(Long targetTenantId) {
        if (targetTenantId == null || targetTenantId <= 0) {
            throw new IllegalArgumentException("target tenantId is required");
        }
