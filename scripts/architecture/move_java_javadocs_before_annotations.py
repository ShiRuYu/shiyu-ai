"""Generate an apply_patch patch that moves method Javadocs before annotations."""
...
def method_line(text: str) -> bool:
    value = text.strip()
    return "(" in value and not value.startswith(("if ", "for ", "while ", "switch ", "catch ", "try "))
...
def process(path: Path) -> str | None:
    ...
    while index < len(lines):
        ...
        while next_line < len(lines) and not lines[next_line].strip():
            next_line += 1
        if next_line >= len(lines) or not method_line(lines[next_line]):
            index = end + 1
            continue
        cursor = start - 1
        segment_start = start
        ...
        if annotation_lines:
            changes.append((start, end, min(annotation_lines), next_line))
        index = end + 1

    if not changes:
        return None
    for start, end, annotation_start, _ in sorted(changes, reverse=True):
...
def main() -> int:
    ...
    for line in chunks:
        if line.startswith("--- a/"):
            ...
            value = line.rstrip("\r\n")
            print(value[1:] if value.startswith(" ") else value)
    print("*** End Patch")
    return 0
