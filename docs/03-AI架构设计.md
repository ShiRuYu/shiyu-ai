...
<!-- 能力评估流程 -->
<chain name="abilityChain">
    THEN(
        loadQuestionsCmp,
        adaptiveTestCmp,
        calculateAbilityCmp,
        updateAbilityRecordCmp
    );
</chain>
```
...
```java
@LiteflowComponent("checkKnowledgeCmp")
public class CheckKnowledgeCmp extends NodeComponent {
    ...
    public void process() throws Exception {
        ...
        Knowledge knowledge = knowledgeEngine.getKnowledge(knowledgeId);
        List<Long> prereqs = knowledgeEngine.graph().parents(knowledgeId);
        Set<Long> mastered = ctx.getStudentProfile().getMasteredKnowledge();
        List<Long> missing = prereqs.stream()
            .filter(p -> !mastered.contains(p))
            .toList();
        ctx.setMissingPrerequisites(missing);
    }
}
...
public class TeacherCmp extends NodeComponent {
    ...
    public void process() throws Exception {
        LearningContext ctx = this.getContextBean(LearningContext.class);
        if (ctx.getMissingPrerequisites() != null) {
            for (Long missing : ctx.getMissingPrerequisites()) {
                AgentResponse response = teacherAgent.execute(
                    AgentRequest.builder()
                        .studentId(ctx.getStudentId())
                        .knowledgeId(missing)
                        .build());
                ctx.addTeachingResponse(response);
            }
        }
        AgentResponse response = teacherAgent.execute(
            AgentRequest.builder()
                .studentId(ctx.getStudentId())
                .knowledgeId(ctx.getKnowledgeId())
                .build());
        ctx.addTeachingResponse(response);
    }
}

@LiteflowComponent("scoreCmp")
public class ScoreCmp extends NodeComponent {

    @Override
    public void process() throws Exception {
        LearningContext ctx = this.getContextBean(LearningContext.class);
        PracticeResult result = ctx.getPracticeResult();
        double score = result.getCorrectCount() * 1.0 / result.getTotalCount();
        ctx.setScore(score);
    }
}

@LiteflowComponent("updateMasteryCmp")
public class UpdateMasteryCmp extends NodeComponent {
    ...
    public void process() throws Exception {
        LearningContext ctx = this.getContextBean(LearningContext.class);
        double score = ctx.getScore();
        abilityService.updateMastery(
            ctx.getStudentId(),
            ctx.getKnowledgeId(),
            score
        );
