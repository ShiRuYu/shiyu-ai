...
- [x] 从原方法提取行为矩阵：密码登录与验证码登录；注册；刷新/登出；切换角色/租户；权限查询；找回密码。
- [x] 补充行为测试：现有 AuthServiceImplTest、AuthServiceDefaultRoleTest 覆盖租户/角色拒绝、失效 Token、恢复失败和默认范围事务分支。
- [x] 先移动会话管理，再移动身份切换，再移动恢复流程，最后缩小认证协调；每次移动只改变委托位置。
- [x] 抽出的类通过构造器注入明确仓储/端口；禁止所有子服务反向依赖旧 AuthServiceImpl。
- [x] 特别验证 Spring 事务代理：用例拆分保持原事务注解入口与 ActorContext 显式传递。
- [x] 全部门禁通过后提交 refactor: separate iam authentication use cases。
...
- [x] AgentRuntimeImpl 区分运行协调、状态持久化、事件发布和框架适配，保留单一运行生命周期入口。
- [x] NodeFactory 按实际节点注册/构造职责拆分，验证未知类型、重复注册、缺少依赖的错误语义，避免引入新的反射全局注册表。
- [x] EmbeddedIndexRegistry 拆分索引生命周期与具体后端打开/恢复逻辑；现有覆盖和集成测试验证关闭、打开失败、恢复和租户检索分支。
- [x] 每个类先建立现有行为保护，再做职责移动；不改算法和性能参数，并保留职责与依赖证据。
- [x] 独立验证后按领域提交，完整门禁作为最终收口。
