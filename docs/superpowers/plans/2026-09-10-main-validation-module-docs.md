...
- [ ] Record all local and remote branches, latest commit, merged status, and worktree ownership.
- [ ] Record the 29 leaf modules and classify each as application, domain contract, domain implementation, infrastructure, shared, or test.
- [ ] Mark branch deletion as safe only when it is not checked out and has no unique commits; otherwise record the blocking reason.
...
- [ ] For every module, document purpose, responsibilities, public boundary, package roots, direct module dependencies, and focused verification command.
- [ ] For contract/implementation pairs, describe the contract as stable API/SPI and implementation as runtime adapter/service behavior.
- [ ] Keep examples and commands relative to the module or repository root and avoid secrets or environment-specific credentials.
...
- [ ] Run `mvn --batch-mode --no-transfer-progress -Pstrict-warnings verify -Ddependency-check.skip=true` from the backend root.
- [ ] Run architecture, documentation, fresh-startup, and Python test checks under `scripts/`.
- [ ] Start the application with the documented dev profile, confirm health/OpenAPI availability, then stop it cleanly.
...
- [ ] Run `git diff --check` and verify no package, source, or Maven module paths changed.
- [ ] Commit documentation and inventory changes as one focused commit.
- [ ] Push `master` to `origin` and confirm local and remote commit IDs match.
