...
flowchart LR
    BOOT["bootstrap\n启动与装配"] --> WEB["web\nController / DTO / SSE"]
    WEB --> PLATFORM["platform\nAgent / Auth / Model / Knowledge / Memory / Tool / Usage / Plugin / Vector"]
...
| 平台 | `platform/shiyu-ai-memory` | 记忆模型、查询、存储 SPI、短期/长期记忆实现 |
...
教育资源 Controller 位于教育包下，因此运行时实际路径带统一的 `/edu` 前缀，并受 `edu:resource:list` 权限约束。当前过滤器中保留的 `/education-resources/**` 排除项与实际 `/edu/education-resources/**` 不一致，不能把教育资源接口视为匿名公开接口。
...
| 领域 | 真实后端路径示例 |
| --- | --- |
| 认证与上下文 | `/auth/login`、`/auth/codes`、`/auth/tenants` |
| 用户和授权 | `/user/**`、`/tenant/**`、`/role/**`、`/menu/**`、`/auth-code/**` |
| Agent | `/agent/definition/**`、`/agent/version/**`、`/agent/execution/**` |
| 模型平台 | `/agent/platform/**`、`/agent/model/**`、`/chat/**` |
| 知识 | `/knowledge/**` |
| 教育 | `/edu/**`；由 `EduWebMvcConfig` 统一加前缀 |
| 记录 | `/record/**` |
| 文件 | `/system/file/**` |
| 用量/插件/工具 | `/usage/**`、`/plugin/**`、`/tool/mcp/**` |

`shiyu-ui` 的 `/api` 是浏览器/网关前缀：本地 Vite 把 `/api/auth/login` 代理成后端 `/auth/login`。生产环境必须配置等价的反向代理规则。
...
本轮审计使用 TypeScript AST 从前端提取 294 次调用（278 个唯一方法/路径），并与 2026-08-12 独立运行实例导出的 330 条路径、345 个 OpenAPI operation 交叉比较；未解析表达式为 0，前端调用都能找到结构对应。该检查覆盖 REST、动态路径、统一响应和 SSE 端点，但仍不能替代带真实模型供应商、生产对象存储和网关的全链路验证。

当前已确认的代码边界：

- 自动 Token 续期存在契约缺口：前端开启了自动刷新，并向 `/auth/refresh` 的 body 传入旧 access token；后端过滤器没有把 `/auth/refresh` 排除为可匿名访问路径。Token 已失效时，请求可能在进入 Controller 前被拦截。当前“过期会话跳回登录页”路径有测试，“失效 Token 成功续期”没有端到端证据。
...
- 教育资源真实路径为 `/edu/education-resources/{fileName}`，且有权限注解；过滤器排除项仍是无 `/edu` 前缀的旧路径，需要在修改访问策略时一并校正。
