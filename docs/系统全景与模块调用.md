...
| 模块 | 所有权 | 主要职责 | 对外依赖 |
| --- | --- | --- | --- |
| `modules/applications/composition` | 应用层 | 应用装配、跨域只读组合和启动所需端口 | Shared Kernel、领域 contract |
| `modules/applications/bootstrap` | 技术支持 | Spring Boot 入口、配置、H2/v4 初始化、健康检查 | 所有启用的 implementation |
| `modules/applications/web` | 技术支持 | Filter、统一异常、HTTP 基础配置、OpenAPI 暴露 | common、IAM 认证适配 |
| `modules/infrastructure-*` | 技术支持 | MyBatis、存储、线程、向量、Web 和核心工具 | 不依赖领域 implementation |
| `modules/shared/kernel` | Shared Kernel | 强类型 ID、`ActorContext`、分页、领域事件、错误类型 | 无业务领域依赖 |
| `tests/architecture` | 质量门禁 | ArchUnit、领域边界和依赖方向检查 | 测试依赖 |
