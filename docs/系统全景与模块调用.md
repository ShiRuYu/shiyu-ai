...
| 模块 | 所有权 | 主要职责 | 对外依赖 |
| --- | --- | --- | --- |
| `modules/application/shiyu-application` | 应用层 | 应用装配、跨域只读组合和启动所需端口 | Shared Kernel、领域 contract |
| `modules/application/shiyu-ai-bootstrap` | 技术支持 | Spring Boot 入口、配置、H2/v4 初始化、健康检查 | 所有启用的 implementation |
| `modules/application/shiyu-ai-web` | 技术支持 | Filter、统一异常、HTTP 基础配置、OpenAPI 暴露 | common、IAM 认证适配 |
