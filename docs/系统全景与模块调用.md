...
- `shiyu-ai-bootstrap` 负责启动、配置、provider 校验、数据库初始化和应用装配。
...
| 模块 | 所有权 | 主要职责 | 对外依赖 |
| --- | --- | --- | --- |
| `modules/applications/shiyu-application` | 应用层 | 应用装配、跨域只读组合和启动所需端口 | Shared Kernel、领域 contract |
| `modules/applications/shiyu-ai-bootstrap` | 技术支持 | Spring Boot 入口、配置、provider 校验、H2/v4 初始化和健康检查 | 所有启用的 implementation |
| `modules/applications/shiyu-platform-bootstrap` | 平台发行组合 | 仅平台能力的 Spring Boot 入口；不携带 Education business implementation | `shiyu-application`、平台领域实现 |
| `modules/applications/shiyu-ai-web` | 技术支持 | Filter、统一异常、HTTP 基础配置、OpenAPI 暴露 | common、IAM 认证适配 |
| `modules/infrastructure/{core,mybatis,storage,thread,vector,web}` | 技术支持 | MyBatis、存储、线程、向量、Web 和核心工具 | 不依赖领域 implementation |
| `modules/shared/shiyu-shared-kernel` | Shared Kernel | 强类型 ID、`ActorContext`、分页、领域事件、错误类型 | 无业务领域依赖 |
| `tests/shiyu-architecture-tests` | 质量门禁 | ArchUnit、领域边界和依赖方向检查 | 测试依赖 |

### 2.1 领域纵切

每个平台领域和独立业务边界都拥有一个 `contract` 和一个 `implementation` Maven 模块。平台领域位于 `modules/domains`，可选业务边界位于 `modules/business`；`contract` 放命令、查询、端口、事件和跨域 DTO，`implementation` 放服务、Controller、Repository、SQL、适配器和测试。
...
领域 implementation 自己维护 `src/main/resources/db/baseline/h2/schema/<domain>` 和 `seed/<domain>`。当前 v4 是空开发库的最终基线，不是 v3/v4 迁移脚本；MySQL/PostgreSQL 使用外部基线与迁移工具，应用启动只校验版本和表集合。
...
本地运行数据位于 `shiyu-ai/runtime/dev/data`，包含 H2 数据库、文件、日志和向量索引；备份目录不参与源码装配。使用外部数据库或对象存储时，数据库与文件内容离开该目录，应用只保留配置和必要的本地临时数据。

## 3.1 基础设施 provider

所有领域通过端口访问基础设施，provider 在启动时由配置选择：

| 能力 | 默认 | 可选实现 | 关键约束 |
| --- | --- | --- | --- |
| 数据库 | H2 | MySQL、PostgreSQL | 外部库必须先完成 v4 baseline，启动只校验不迁移 |
| 文件 | local | S3、MinIO | 元数据在数据库，迁移保留 object key |
| 向量 | JVector | InMemory、pgvector | pgvector 只允许 PostgreSQL，按维度建 HNSW 部分索引 |
| Redis 状态 | disabled | Redis | 租约、限流、幂等和短期缓存；不可用时快速失败 |
| 事件 | in-process | PostgreSQL outbox、Kafka | Kafka 仅显式开启，重试超限进入死信主题 |

provider 配置键分别是 `shiyu.infrastructure.database.provider`、`shiyu.infrastructure.file.provider`、`shiyu.infrastructure.vector.provider`、`shiyu.infrastructure.redis.provider` 和 `shiyu.infrastructure.event.provider`。旧 `shiyu.storage.type` 与 `shiyu.vector-store.type` 继续作为兼容别名。切换前先完成数据迁移、校验和回滚演练，不允许把空外部存储直接接入生产。
...
完整的 377 个路径和 413 个 operation 见 [API接口参考](参考/API接口参考.md)，不要在本文件重复维护全部参数表。
...
6. 节点开始/完成、模型调用、审计和失败事件发布给 Governance 等订阅方。
...
| 内容 | 事实来源 | 校验方式 |
| --- | --- | --- |
| 后端模块/边界 | Maven reactor、ArchUnit | `mvn clean verify -Dmaven.compiler.useIncrementalCompilation=false -Ddependency-check.skip=true` |
